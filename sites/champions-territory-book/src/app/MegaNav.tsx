...
/*
  ...
  Replies took the fourth slot when the venue screens came out. It is a
  queue on the same test the other three pass: a homeowner or a property
  manager who answered is work that arrived unasked, and an answer left
  sitting for a week is the lead gone to whoever replied that afternoon.
  Maps stayed as the featured key, which is a different instrument again
  and is argued for beside it below.
*/
const ITEMS: MegaItem[] = [
  ...
  { to: "/replies", label: "Replies", sec: "replies", rank: 4 },
];
  ...
  return (
    <div className={styles.bar}>
      ...
      */}
      ...
      {/*
        THE LOCKUP IS TEXT ON THIS ROUTE.

        The /me build carries the venue's own wordmark here because that
        application was built for one named venue and the mark identified
        it. This one is about a division of brands rather than a single
        business, and putting somebody else's trademark in the chrome of
        an unaffiliated work sample is a thing to do once, deliberately,
        with the disclaimer rewritten around it. It is not a thing to
        inherit by copying a folder.
      */}
      <div className={styles.brand}>
        ...
        >
          <span className={styles.brandText}>The Territory Book</span>
        </Link>
      </div>
        ...
        <ul className={styles.list}>
          {ITEMS.map((item) => {
            ...
            /* The same test NavLink runs, computed here as well because
               ...
               and the className callback cannot reach it. Every key on
               this strip is an exact match now: the one destination that
               carried a child route came out with the venue screens. */
            ...
            return (
              ...
              >
                <NavLink
                  to={item.to}
                  /* Exact on every key. Nothing on this strip has a
