...
import { PERIODS, VENUE, daysToOfferExpiry } from "@/data/venue";
 ...
 * has exactly one user, the Marketing Manager, and a switcher offering a view
 ...
 * offers, the coaching notes, the methodology. The field plan is a
 * working screen where hours outside the office are planned and
 ...
 * page a property manager opens from an email, and showing them the
 * division's own call sheet would be the digital equivalent of handing a
...
const GROUPS: RailGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        label: "Field",
        sec: "field",
        hint: "Tabling, networking and go-see runs outside the office",
      },
      {
        to: "/rivals",
        label: "Rivals",
        sec: "rivals",
        hint: "Published facts about the other operators, and why jobs were actually lost",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        label: "Capacity",
        sec: "capacity",
        hint: "What will actually fit on a date, in crew slots",
      },
      /**
       * WHERE THE RECURRING PROGRAMME ROWS BELONG, AND WHY THEY ARE NOT
       * DRAWN ON THIS BUILD.
       *
       * They belong under THE BOOK rather than under THE BOARD, and the
       * choice is worth a sentence because the board was the cheap place
       * to put them. Everything under THE BOARD is a view of the same two
       * hundred and eleven organisations: ranked on the desk, plotted on
       * the map, cut by lane, worked in the field. A membership programme
       * is not one of those. It is the only RECURRING thing the division
       * sells, it is sold to households rather than to organisations, and
       * a member signed in September is a commitment on next September's
       * calendar. That is exactly what the other rows in this group are
       * about: what is signed, what is planned, and what will still fit.
       *
       * The rows are absent while the screens behind them are rewritten.
       * A rail row pointing at a screen that still describes the wrong
       * business is worse than a gap, because a gap is visible and a
       * wrong row is trusted.
       */
    ],
  },
  {
  ...
  /*
    A SIXTH GROUP STOOD HERE AND IT IS GONE, WHICH IS THE HONEST STATE.

    It held three rows: co-marketing partners, promotional stock and the
    budget those two run on. All three were screens about the venue's own
    spend and all three came out with the venue subsystem, because none
    of them could be rebuilt truthfully. Champions Group publishes no
    marketing budget, no cost per lead, no agency roster and no spend by
    channel, and a budget screen for a company whose budget is not
    published is either three empty panels or three invented figures.

    An empty group heading with no rows under it is worse than either. It
    reads as a loading failure, and the rail spent a render drawing one.
    So the heading came out with the rows.

    The reasoning is kept here because the group is the right shape and
    should come back the day any of those figures becomes knowable, which
    for a person actually in the job is week one.
  */
];
...
export function useShellFigures(): ShellFigures {
   ...
   * programme enquiry, and the chrome re-renders on every keystroke in
   * the desk search box.
   */
  const queue = useMemo(() => {
   ...
   * three hundred and twenty nine records, joining threads, offers and statuses.
   */
  ...
  const counts = useMemo<Record<string, Count>>(() => {
    ...
    return {
      ...
      "/packages": { value: PACKAGES.length, unit: "published offers" },
...
export function SideRail() {
  ...
  */
  ...
  /*
     ONE DATE, ONE CLOCK, AND THE FIGURE IS THE SUBTRACTION.

     This was a number stored on each period, and it produced the worst
     defect this application has had: a rail reading "14 days to expiry"
     on a board dated three weeks AFTER the date it was counting to. Two
     halves of one fact, held in two files, each looking correct alone.
     data/venue.ts has the full argument beside PUBLISHED_OFFER_EXPIRY.
  */
  const daysLeft = daysToOfferExpiry(BOARD_DAY);
  const daysPast = -daysLeft;

  /**
   * HOW MUCH OF THE TERRITORY HAS BEEN WORKED. Carried over from the old
   * header, with its reasoning intact.
   *
   * The tempting figure was revenue against a target. There is no
   * target: none of the five brands publishes a lead volume, a budget or
   * a capacity plan, so a percentage against one would be a number this
   * app made up and then drew a picture of. What is real and countable is
   * how much of the territory has been worked at all, and it moves the
   * moment a reader changes a status anywhere in the application.
   */
  ...
  const markLabel =
    `The Territory Book. The dial round the mark shows how far the trade ` +
      ...
      : daysLeft === 1
        ...
        : daysLeft === 0
          ...
          : daysPast === 1
            ? "day past expiry"
            : "days past expiry";
  const countdownValue = daysLeft > 0 ? daysLeft : daysLeft === 0 ? 0 : daysPast;
  /*
     WHAT THE NEGATIVE NUMBER IS ALLOWED TO CLAIM, and it is narrower
     than it looks. Not that nothing has replaced the campaign today,
     because nobody read the site today. Only that on the day the fine
     print was read there was nothing published to follow it. The label
     says it that way and so does the offers card on the map.
  */
  const countdownLabel =
    daysLeft > 1
      ? `${daysLeft} days to the published offer expiry, 31 August 2026`
      : daysLeft === 1
        ? "1 day to the published offer expiry, 31 August 2026"
        : daysLeft === 0
          ? "The published offers expire today, 31 August 2026"
          : `${daysPast} days past the published offer expiry of 31 August 2026. Nothing scheduled to follow it was published on either brand site when they were read on 18 August 2026.`;
  ...
  return (
    <nav
      ...
      aria-label="Every screen in The Territory Book"
    >
      <div className={styles.head}>
        ...
        */}
        <div className={styles.brand}>
          ...
          */}
          <Link
            ...
            aria-label="The Territory Book, go to the desk"
          >
            <span className={styles.brandText}>
              <strong>The Territory Book</strong>
        ...
        {/*
          ...
          the one division it is about, and there is nothing to switch
          between because there is nothing else.
        */}
        <p className={styles.contextLabel}>Marketing manager, West</p>
        ...
        <div className={styles.context}>
          {/*
            ...
            the brand name here and stop. This one carries a number of
            days permanently, because the single fact that makes this
            application different from a CRM is that THE ONLY PUBLISHED
            DATE IN THE WHOLE PORTFOLIO IS 31 AUGUST 2026, and a fact
            that only appears on the screen a reader happens to be
            looking at is a fact half of them miss.

            The days come from the board day rather than from the
            machine's clock, so a screenshot of any screen is
            reproducible: the board day is a constant in the seeded
            world and the expiry is a published date, and the figure is
            the subtraction of the two. Once the date has passed the
            unit switches to "days past expiry" and counts up, because
            an offer that ran out three weeks ago with nothing published
            behind it is a louder fact than a zero.

            COLLAPSED, THE UNIT IS CLIPPED RATHER THAN DROPPED. A bare
            figure in a narrow column is the defect the owner photographed:
            "14" on its own is fourteen of nothing. The word stays in the
          ...
          >
            ...
            <ul className={styles.list}>
              {group.items.map((item) => {
                ...
                /* Every destination in this rail is an exact match now.
                   The one that carried a child route under it came out
                   with the venue screens. */
                ...
                return (
                  ...
                  >
                    <NavLink
                      to={href}
                      /* Exact on every row. Nothing in this rail has a
                         child route under it any more. */
                      end
                      data-rail-item=""
                      title={item.hint}
                      className={({ isActive }) =>
                      ...
                      {item.featured ? (
                        <>
                          {/*
                            ...
                            "Maps 329 organisations" rather than one run
                          ...
                          */}
                          ...
                          {/*
                            ...
                            says three hundred and twenty nine things are owed. The
            ...
            {group.id === "working" ? (
              <div className={styles.facets}>
                ...
                {/*
                  ...
                  it is WHERE THE DECISION IS: on the site itself, above it
                  at a region, or on a term calendar through procurement. Each tile carries the glyph, the word and
