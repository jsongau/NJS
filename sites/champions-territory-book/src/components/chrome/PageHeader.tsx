...
import { normalisePath, sectionFor } from "@/app/sections";
...
const SCREENS: Record<string, ScreenNode> = {
  ...
  "/calendar": { group: "The week", label: "Capacity", groupTo: "/calendar" },
  /* Reference is three screens that explain the other fifteen, and the
     group points at Method because Method is the one that answers the
     question a reader arrives with: can any of these numbers be
     trusted. The drift check below is a real check, and it is what
     caught the eight rows this table used to carry for screens that
     came out with the venue subsystem. */
  "/coaching": { group: "Reference", label: "Coaching", groupTo: "/method" },
  "/method": { group: "Reference", label: "Method", groupTo: "/method" },
  "/team": { group: "The crew", label: "The field crew", groupTo: "/team" },
  "/report": { group: "The crew", label: "Division weekly", groupTo: "/team" },
 ...
 */
export function crumbsFor(rawPathname: string, filter?: string): Crumb[] {
  /*
    NORMALISE BEFORE LOOKING UP, and this is a bug that only appears in
    the deployed build.

    Every route in this application is prerendered as a directory with an
    index.html in it, so a static host resolving /report will redirect to
    /report/ with a trailing slash before the application ever mounts.
    React Router hands that slash straight through, SCREENS["/report/"]
    is undefined, and the trail on every screen quietly degrades to "This
    screen". It reads as a missing feature and it is a missing character.

    normalisePath is the same function the section colours already run
    through, for exactly the same reason, so the two cannot disagree.
  */
  const pathname = normalisePath(rawPathname);
  const node = SCREENS[pathname];
  const out: Crumb[] = [];

  if (pathname !== "/") out.push({ label: "The Territory Book", to: "/" });
