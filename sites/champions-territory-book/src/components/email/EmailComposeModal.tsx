...
/**
 * THE COMPOSE WINDOW. THE ONE SCREEN IN THIS APP WHERE THE MARKETING
 * MANAGER ACTUALLY DOES THE JOB.
 *
 * Everything else here is preparation. The desk decides who to write to,
 * the map decides where they sit in the territory, the service line board
 * decides why they buy.
...
export interface EmailComposeModalProps {
  ...
  templateId?: string;
  /** Set by the membership board. Turns the fourth intent group on. */
  plan?: PlanContext;
  onClose: () => void;
}
...
> = {
  ...
  "reserve-party": {
    ...
    what: "The arrangement written down, at no cost and no commitment.",
  },
  "plan-enquiry": {
    glyph: "◇",
    label: "Membership enquiry",
    what: "What is published about the plans, and what is not.",
  },
  "plan-join": {
    glyph: "◈",
    label: "Enrol on a plan",
    what: "Ask for places on a programme that is open to enrolment.",
  },
  "plan-team": {
    glyph: "▤",
    label: "Enrol a portfolio",
    what: "A named portfolio and its doors, put down in writing.",
  },
  "plan-new": {
    glyph: "◉",
    label: "Build a programme",
    what: "Build one around this organisation rather than join one.",
  },
};

const CORE_INTENTS: ComposeIntent[] = ["outreach", "featured-promo", "reserve-party"];
const PLAN_INTENTS: PlanIntent[] = [
  "plan-enquiry",
  "plan-join",
  "plan-team",
  "plan-new",
...
function isPlanIntent(intent: ComposeIntent): intent is PlanIntent {
  return (PLAN_INTENTS as string[]).includes(intent);
}
 ...
 * package, which is a promo. An organisation with a written touch behind
 * it is past the opening and into agreeing terms, which is the third
 * intent. Everything else is a first touch, including every organisation
 * that publishes no address, because a first touch is what a marketer
 * reads out at a counter as easily as they send it.
 */
 ...
 * for one account can never be shown under another account's name.
 */
export function EmailComposeModal({
  ...
  return (
    <ComposeDialog
      key={`${prospect.id}:${intent ?? "auto"}:${plan?.planName ?? ""}`}
      prospect={prospect}
  ...
  */
  ...
  const drafts: EmailTemplate[] = useMemo(() => {
    if (isPlanIntent(intent)) {
      return plan ? planTemplatesFor(prospect, intent, plan) : [];
    }
  ...
  }, [intent, prospect, ctx, holdDate, guests, packageId, plan]);
   ...
   * the door count on an agreement, the date. A bare link is still a valid thing
      ...
      () => {
        ...
        outboxDispatch({
          ...
          message: {
            ...
            body: b,
            /*
              A package and a door count ride along ONLY where the message
  ...
  const lanesHeld = crewSlotsForDoors(guests);
  ...
  const shownIntents: ComposeIntent[] = plan
    ? [...CORE_INTENTS, ...PLAN_INTENTS]
    : CORE_INTENTS;
          ...
          <div className={styles.split}>
            ...
            <div className={styles.controls}>
                                ...
                                <Figure
                                  value={
                                    p.pricePerGuest === null
                                      ? null
                                      : `$${p.pricePerGuest.toFixed(2)} as published`
                                  }
                                  provenance={
                                    p.pricePerGuest === null
                                      ? "withheld"
              ...
              {intent === "reserve-party" ? (
                <fieldset className={styles.fieldset}>
                  <legend className={styles.legend}>What the terms cover</legend>
                  ...
                  <div className={styles.lanesBox}>
                    <p className={styles.lanesLead}>
                      <span className="num">{guests}</span> doors works out at{" "}
                      <strong className="num">{lanesHeld}</strong>{" "}
                      {lanesHeld === 1 ? "crew slot" : "crew slots"} against a
                      modelled Brea capacity of <span className="num">{floor}</span> a
                      day, at one slot per{" "}
                      <span className="num">{DOORS_PER_CREW_SLOT}</span> doors.
                    </p>
                    <ProvenanceBadge provenance="modeled" />

                    {overFloor ? (
                      <p className={styles.lanesWarn}>
                        <span aria-hidden="true">▲</span> Over the modelled crew
                        capacity. A phased programme rather than one push.
                      </p>
                    ) : null}
                    {overPackageMax ? (
                      <p className={styles.lanesWarn}>
