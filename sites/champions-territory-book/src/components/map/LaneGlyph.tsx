 ...
 * week. Then somebody nudges a droplet for the map, the legend keeps the
 ...
 * the service van lands in both surfaces in one edit, because there is
 * only one service van.
 *
 * THE PAINT GOES THROUGH `style` AND NOT THROUGH `fill`. A presentation
 * attribute does not resolve a custom property: `fill="var(--lane-hvac)"`
 * paints nothing, while `style={{ fill: "var(--lane-hvac)" }}` paints
 ...
 *   VenueGlyph     the dashed ring. The anchor branch is ours, not a row.
...
export interface MarkSvgProps {
   ...
   *
   * Then it is decoration, and a screen reader announcing "snowflake,
   * heating and air conditioning" immediately before the words "Heating
   * and air conditioning" is reading the row twice. The title is kept for
   * a sighted reader hovering it.
   */
  decorative?: boolean;
  className?: string;
}
 ...
 * calendar-locked: the season buys, not the buyer, and a compressor gives
 * out in July whether or not anybody has run a campaign. A square body
 * means discretionary: somebody has to decide there will be any work at
 * all. A reader learns that pair once on a chip and reads it for free on
 * three hundred and twenty nine pins, which only holds while the two agree. Both
 ...
 * The anchor, drawn.
 *
 * The ring is dashed because this one is OURS. Every other mark on the
 * board is a competitor, a partner or a prospect, and a solid pin at the
 * centre would file the division's own branch as the two hundred and
 * twelfth row on it.
 */
export function VenueGlyph({
  ...
  name: string;
  /** The words under the ring. "Our address" unless a caller insists. */
  label?: string;
  size?: number;
  decorative?: boolean;
  className?: string;
