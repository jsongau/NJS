...
/**
 * THE CENTRE PANE. The territory itself, and the one place on the screen
 * where the premise of the whole application is visible without reading a
 * word: one branch of ours in the middle, and three hundred and twenty nine
 * organisations round it that are somebody else's.
 *
 ...
 *
 * WHY THE ANCHOR MARK IS A DASHED RING
 * A solid pin on a map says "here is a business", and that is true of
 * every other mark on this screen. Service Champions Brea is ours, and a
 * reader who takes the centre of this map for one more row reads the
 * whole board wrong. So the ring is dashed, the plate under it says the
 * address is ours, and neither of those depends on the amber.
 *
 * WHY THE RINGS SAY "STRAIGHT LINE"
 * A circle drawn from a branch is the oldest trick on a territory map and
 ...
 * is walkable from the branch and can be dropped into a gap between two
...
const RINGS: { miles: number; note: string }[] = [
  { miles: 1, note: "Walkable from the branch. Fits in a gap." },
...
/**
 * How far from the branch a ring label has to fall before it is worth
 * drawing, in screen pixels.
 *
 * A RING LABEL IS ONLY HONEST IF IT CAN BE READ. The anchor mark is 104
 * pixels wide, because it carries the words "Our address" on a plate
 * under the ring, and that plate reaches 52 pixels either side of the
 * branch and 34 below it. A ring drawn at the territory zoom is 25 pixels
 * of radius per mile, so a one mile label lands 18 pixels from the
 * centre, which is underneath the plate, and a three mile label lands 54,
 * which clips its top edge. The phrase on the plate is one character
 * shorter than the one this was measured against, so the clearance is
 * unchanged or slightly better and has not been remeasured.
 *
 ...
 * "Send a featured promo", on the reasoning that the featured offer
...
/**
 * Where a ring's label sits: north west of the anchor, on the ring
 * itself.
 *
 ...
 * quadrant of the territory: Brea runs north east and due south of
 * Columbia Street, so a label placed there sat over the fewest pins. That is a true
 ...
 *
 * The anchor is forced into the bounds whatever is filtered. A map of the
 * drain lane that has scrolled the branch off the edge has lost the only
 * fixed point on it.
 */
function FitToFilter({
  ...
  useEffect(() => {
    ...
    /*
      ── THE ONE FILTER THAT DOES NOT KEEP THE BRANCH IN FRAME ──────

      Everywhere else the anchor is forced into the bounds, because a map
      of one service line that has scrolled the anchor address off the edge has
      lost the only fixed point on it. A field run is the exception and it
      is worth saying why rather than looking like an oversight.

      A run is six doors inside about half a mile, two or three miles out.
      Framing that against the branch means framing five miles to show
      ...
      not a claim about the anchor at all. It is a claim about six
      ...
      Nothing is lost: the branch is one press of the fit control away,
...
export interface MapCanvasProps {
   ...
   * is arithmetic: the strip says "31 of 329 on the board" and the list
   * shows thirty one cards, so a map showing three hundred and twenty nine marks
...
export function MapCanvas({
  ...
  */
   ...
   * unreadable fragments across the anchor plate.
   *
   * The rule now is one label, on the arc of the INNERMOST ring that can
   * clear the anchor plate at this zoom. Innermost rather than outermost is
  ...
  */
  ...
  /*
    The branch, and the one ring label, on the same principle. Both build
    a `divIcon` from a template, both are handed to a `Marker`, and both
    were rebuilt on every render of this pane for no reason at all. The
    anchor mark never changes; the label changes only when the ring it
    names changes.
  */
    ...
    >
      ...
      */}
      <p className="visually-hidden" role="status" aria-live="polite">
        {rows.length} organisations plotted around Service Champions Brea, the
        division's own branch. {summary.insideThree} of them are inside three
        straight line miles,{" "}
        ...
        straight line miles from the division anchor, and they are not drive
        times.
        ...
        */}
        ...
        {ringLabel ? (
          <Marker
            ...
            */
            title={`${ringLabel.ring.miles} straight line miles from ${VENUE.name}, not a drive time. ${ringLabel.ring.note}`}
            interactive={false}
            keyboard={false}
        ...
        {/*
          ...
          latitude, and there are organisations north of Columbia Street
          that would otherwise sit on top of the one address this entire
          application is measured from.
        */}
        <Marker
          position={VENUE_POINT}
          icon={venueMark}
          title={`${VENUE.name}, our own address`}
          zIndexOffset={1000}
        >
          <Popup
          ...
          >
            <div className={styles.popup}>
              <p className={styles.popupKicker}>Our branch</p>
              ...
              <p className={styles.popupNotOpen}>
                <span aria-hidden="true">◌</span> Ours, not a prospect. Service
                Champions publishes this branch alongside Colton and Murrieta,
                and one phone number covering all three, with no separate
                number published for any of them.
              </p>
      ...
      <div className={styles.overlay}>
        ...
        */}
        <p className={styles.ringNote}>
          ...
          <span>
            The circles are 1, 3 and 5 straight line miles from the branch.
        ...
        <div className={styles.zoomBar} role="group" aria-label="Map zoom">
          ...
          <button
            ...
            title="Fit everything that is plotted, including the branch"
          >
            <span aria-hidden="true">▣</span>
