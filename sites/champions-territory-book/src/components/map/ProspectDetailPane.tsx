...
import { DOORS_PER_CREW_SLOT, LANE_META } from "@/domain/lanes";
import { isReachableEmail, isWithheldEmail } from "@/domain/contactDoor";
 ...
 * somebody working a territory asks. The five here are the five
 * decisions in front of a marketer holding a phone:
 *
 *   Place         who and where they are, and every route to them. The
 *                 address set as an address, the phone as a phone, the
 *                 written door with the page it was read off, what they
 *                 sell and where, where they stand against this
 *                 division, and the facts that make the row checkable.
 ...
 *                 named evidence behind it, what they publish and what
 *                 they conspicuously do not, the angle, the signal, the
 *                 modelled reach, the crew it consumes and the offers
 *                 that fit.
 ...
 *                 whether it still stands and what it costs the brand.
 *
 * The offer catalogue did not disappear; it is the part of Potential
 * that answers "what do I lead with". The score breakdown moved to the
 * desk, which is where the ranking is made and argued.
 *
 ...
 *
 * It opens by default because it is the tab a marketer wants when they
 * have just pressed a pin: what is this place, where is it, and how do I
 * get hold of somebody. Status, which used to be the default, answers the
 ...
 * It does not invent a discount: the coupon war in this market has
 * already bottomed out at 47 dollars for a drain clearing, so there is
 * very little left to take a percentage off, and the leverage this desk
 * has is response time, coverage and a plan with a printed price on it.
 */
...
export interface ProspectDetailPaneProps {
   ...
   * panel because somebody ticked a service line chip loses their place
   * for a reason they did not ask for.
   */
  outOfFilter?: boolean;
  onClearFilters?: () => void;
...
const OFFER_STATE_META: Record<OfferState, StatusToken> = {
  ...
  declined: {
    ...
    label: "Declined",
    cssVar: "var(--risk)",
    note: "They said no to the offer, which is not always a no to the brand.",
  },
  lapsed: {
    glyph: "◌",
    label: "Lapsed",
    cssVar: "var(--warn)",
    note: "The date printed on it passed. Recorded rather than quietly reopened, because four dead campaign pages left live on one rival's site is the exact failure this state exists to catch.",
  },
  withdrawn: {
    glyph: "◍",
    label: "Withdrawn",
    cssVar: "var(--neutral)",
    note: "The brand pulled it, and the reason sits beside it.",
  },
};
 ...
 *
 * A SECOND COPY, and it is worth saying so out loud. The same map is in
 * the prospect drawer, for the same reason ORG_TYPE_META sits in the
 * record selector and OFFER_STATE_META sits here rather than in
 * domain/vocabulary.ts: that file belongs to another pass this week, and
 * a duplicate token map is a worse outcome than a temporarily misplaced
 * one only until somebody moves them all in one go. When they move,
 * delete both copies rather than leaving one behind.
 */
 ...
 *
 * IT STOPS AT "DATE HELD" ON PURPOSE. The step from a held date to
 * signed work is a signature and a deposit, and the money lives in
 * BookProvider where signed work cannot exist without a line to carry
 * it. Letting a map pane mark something booked would put a contract in
 * the pipeline with no revenue attached, which is the single failure the
 * two ledger model exists to prevent.
 */
...
export const ProspectDetailPane = memo(function ProspectDetailPane({
                    ...
                    return (
                      <li key={sp.id}>
                        <button
                          ...
                          aria-label={`Stop ${i + 1}, ${sp.name}. ${sdoor.label}. ${stop.line.miles.toFixed(1)} straight line miles from the Brea branch.`}
                        >
              ...
              <div className={styles.emptyDo}>
                ...
                <div className={styles.headActions}>
                  ...
                  <button
                    ...
                    aria-label={`Preview the proposal for ${topLine.prospect.name}`}
  ...
  const leadPackage = leadPackageOf(p);
  /* The value if it lands, and its arithmetic printed beside it. A
     published per door price multiplied by a modelled door count is a
     modelled figure, and it is badged as one. Where the lead offer is
     one of the plans whose owner withholds the price, there is no number
     to multiply and the row says so in the sentence the provenance
     system reserves for it. */
  /* No modeled room means no modeled value. Null all the way through
     rather than a zero that would print as a dollar figure. */
  const leadValue =
    leadPackage && leadPackage.pricePerGuest !== null && midpoint !== null
      ...
      >
        ...
        <header className={styles.head}>
          ...
          <div className={styles.headActions}>
            <button
              ...
              title={
                verified
                  ? "Opens on a template chosen for this service line and this buying window."
                  : "They publish no address, so the draft is a front desk script. The script is still the useful thing."
              }
        ...
        <div className={styles.scroller} ref={scrollRef}>
          ...
          {/* -------------------------------------------------------
              ...
              say on its own: a range of numbers is not self-evidently a
              door count and a string of characters is not self-evidently
          ...
          <Panel id="place" active={active === "place"}>
            ...
            */}
            ...
            <dl className={styles.grid}>
              <Cell label="From the branch">
                ...
                <span className={styles.cellHint}>
                  Straight line from {VENUE.address}, Brea, not drive time.
                </span>
              </Cell>
            ...
            {/*
              ...
              three hundred and twenty of the three hundred and twenty
              nine, which is more than any written door, and in this
              trade the phone is very nearly the product. A household
              with no cooling in August reads three listings and calls
              one of them inside two minutes.

              THREE STATES AND ALL THREE ARE SAID OUT LOUD. Ninety three
              publish an address, fifty publish only a form, and a
              hundred and eighty six publish neither. The published
              address is the smallest of the three, so neither the form
              branch nor the empty branch is an edge case to be guarded
              against. Between them they are the great majority of the
              board, and each one says what it is with the reason
              attached rather than rendering as a blank.
            */}
            {verified && isReachableEmail(p.email) ? (
              <a
            ...
            ) : verified && isWithheldEmail(p.email) ? (
              /*
                THE DOOR IS THERE AND IT IS NOT A LINK. A published
                address belonging to a named person is a door this
                application will not walk a stranger through, so the row
                keeps the domain and the finding and loses the mailto.
                See domain/contactDoor.ts.
              */
              <p className={`${styles.call} ${styles.door}`} data-door="withheld">
                <span aria-hidden="true">◈</span>
                <span className={`${styles.doorValue} num`}>{p.email}</span>
            ...
            {/* ---- Who they are ----
                 ...
                 FIVE FACTS AND NONE OF THEM HIDDEN WHEN ABSENT. What
                 they sell and where they sell it decide whether this row
                 is in the same market at all, and a blank against
                 "Founded" on a fifty year old business is worth reading:
                 longevity is free to print and a brand that has it and
                 does not say so is leaving a proof point on the floor.
                 ---- */}
            <h3 className={styles.blockTitle}>Who they are</h3>
            <dl className={styles.facts}>
              <Fact label="Service lines">
                {p.services && p.services.length > 0 ? (
                  p.services.join(", ")
                ) : (
                  <Absent what="services">Not published</Absent>
                )}
              </Fact>
              <Fact label="Service area">
                {p.serviceArea ? (
                  <>
                    {p.serviceArea}
                    <ProvenanceBadge provenance="public" compact />
                  ...
                  <Absent what="service-area">Not published</Absent>
                )}
                <span className={styles.hint}>
                  Their own claim about where they work, not a boundary
                  anybody has checked. One brand in this set claims three
                  counties and seventy one cities from a single address.
                </span>
              </Fact>
              <Fact label="Locations">
                {p.locationCount === undefined ? (
                  <Absent what="locations">Not published</Absent>
                ) : (
                  <>
                    <span className="num">{p.locationCount}</span> published
                    <ProvenanceBadge provenance="public" compact />
                  </>
                )}
              </Fact>
              <Fact label="Founded">
                {p.foundedYear === undefined ? (
                  <Absent what="founded">Not published</Absent>
                ) : (
                  <>
                    <span className="num">{p.foundedYear}</span>
                    <ProvenanceBadge provenance="public" compact />
                  </>
                )}
              </Fact>
            ...
            {/* ---- Where they stand ---- */}
            <h3 className={styles.blockTitle}>Where they stand</h3>
            <dl className={styles.facts}>
              <Fact label="In this market">
                <strong>
                  {MARKET_ROLE_LABEL[p.role ?? ""] ?? "Not decided yet"}
                </strong>
                <span className={styles.hint}>
                  A brand the division operates is here so the board can
                  say where its own coverage already is. A competitor is
                  here to be watched. A partner is not in this trade at
                  all.
                </span>
              </Fact>
              <Fact label="Service line">
                <LaneChip lane={p.lane} size="sm" />
                <span className={styles.hint}>
                ...
                {p.rating === undefined ? (
                  ...
                  <>
                    ...
                    <span className={styles.cellHint}>
                      ...
                      {p.reviewCount === 1 ? "review" : "reviews"}. A rating
                      with no volume behind it says very little, which is
                      exactly the problem with the anchor brand's own 4.9.
                    </span>
                  </>
                )}
              </Cell>
          ...
          <Panel id="status" active={active === "status"}>
            ...
            <div className={styles.controls}>
              ...
              <Button
                ...
                title={
                  advance === null
                    ? "The step from a held date to signed work is a signature and a deposit, so it happens on the Book page where the money lives."
              ...
              <Fact label="Buying window">
                ...
                <span className={styles.hint}>
                  {lane.occasionClass === "calendar-locked"
                    ? "The season or the failure buys, not the buyer, so this window is worked backwards from a date the weather sets."
          ...
          <Panel id="potential" active={active === "potential"}>
            ...
            {/* ---- What they publish ----
                 ...
                 FOUR ROWS, NONE OF THEM HIDDEN WHEN EMPTY, and the
                 blanks are the point. Eight of the fourteen brands
                 profiled name a membership plan and route the price to a
                 phone number, five publish no plan at all, and not one of
                 the fourteen publishes a price or an APR. An
                 absent price is a decision somebody made, and hiding the
                 empty row would hide the strongest pattern in the whole
                 data set.
                 ---- */}
            <h3 className={styles.blockTitle}>What they publish</h3>
            <dl className={styles.facts}>
              <Fact label="Current offer">
                {p.offer ? (
                  <>
                    {p.offer}
                    <ProvenanceBadge provenance="public" compact />
                  ...
                  <Absent what="offer">Nothing published</Absent>
                )}
              </Fact>
              <Fact label="Membership plan">
                {p.membership ? (
                  <>
                    {p.membership}
                    <ProvenanceBadge provenance="public" compact />
                  ...
                  <Absent what="membership">None published</Absent>
                )}
                <span className={styles.hint}>
                  Read the name, then look for a number beside it. A plan
                  with a printed monthly price is unoccupied ground in this
                  market, and it cannot be beaten by printing a smaller
                  coupon.
                </span>
              </Fact>
              <Fact label="Financing">
                {p.financing ? (
                  <>
                    {p.financing}
                    <ProvenanceBadge provenance="public" compact />
                  ...
                  <Absent what="financing">No lender named</Absent>
                )}
              </Fact>
              <Fact label="Online booking">
                {p.onlineBooking === undefined ? (
                  <Absent what="online-booking">Not checked</Absent>
                ) : p.onlineBooking === "yes" ? (
                  <>
            ...
            {/* ---- Why it matters, and what we can see ---- */}
            <h3 className={styles.blockTitle}>Why it matters</h3>
            <p className={styles.note}>
              {p.marketingAngle ?? (
                <Absent what="angle">
                  No angle written for this row yet, which is a gap in the
                  work rather than a fact about the organisation
                </Absent>
              )}
            </p>

            <h3 className={styles.blockTitle}>What we can see</h3>
            <p className={styles.note}>
              {p.competitiveSignal ?? (
                <Absent what="signal">
            ...
            <p className={styles.note}>
              A reading of a website rather than of a strategy. A brand can
              be doing something clever that its own site never mentions,
              and this line would not see it.
            </p>
            ...
            <dl className={styles.facts}>
              ...
              <Fact label="Crew slots">
                ...
                />
                <span className={styles.hint}>
                  This console plans at one crew slot per{" "}
                  <span className="num">{DOORS_PER_CREW_SLOT}</span>{" "}
                  doors. At the midpoint of{" "}
                  <span className="num">{midpoint}</span> this consumes{" "}
                  <span className="num">{line.lanesAtMidpoint}</span> of the{" "}
                  <span className="num">
                    {VENUE.crewSlotsModelledFloor}
                  </span>{" "}
                  a day the Brea branch is assumed to run. Both figures are
                  ours: no brand here publishes a technician count.
                </span>
              </Fact>
                ...
                {record.bookLines.length > 0 ? (
                  ...
                  <>
                    <Figure
                      ...
                      withheldReason={`${leadPackage ? leadPackage.name : "The lead offer"} carries no published price, which is the reason a marketing manager is looking at it.`}
                      compact
                    />
                    {leadValue === null ? null : (
                      <span className={styles.hint}>
                        ...
                        per door, times a modelled midpoint of{" "}
                        <span className="num">{midpoint}</span> doors. Not a
              ...
              {shownPackages.map((k) => (
                ...
                >
                  ...
                  <p className={styles.packMeta}>
                    <span className={styles.packMetaLabel}>Per door</span>
                    <Figure
                      value={
                      ...
                      compact
                    />
                    <span className={styles.packMetaLabel}>Doors</span>
                    {k.minGuests === null ? (
          ...
          {/* -------------------------------------------------------
              ...
              the brand. Rarely a discount: the published coupon prices
              in this market have already been competed down to a level
              nobody makes money at, and what is left to offer is
              response time, coverage and a plan with a printed price on
              it.
              ------------------------------------------------------- */}
          <Panel id="offers" active={active === "offers"}>
            ...
            <p className={styles.note}>
              Response time, coverage and a printed price, rather than money
              off a coupon. Every rival in this set is already at or below
              fifty dollars on a drain clearing, so a smaller number is not a
              position anybody can hold.
            </p>
          </Panel>
        </div>
      </aside>
