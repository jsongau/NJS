...
/**
 * THE RECORD. THE SCREEN A MARKETER OPENS TWENTY TIMES A DAY.
 *
 ...
 * that sentence, in that order, arranged so that all five answers are on
 ...
 * facts, the decay timer and the next action are the two a solo marketer
 ...
 * threshold, offers open, inbound leads, and what is signed.
 *
 * ── THE PROFILE IS A LIST OF THINGS THEY PUBLISH, AND THINGS THEY
 *    CONSPICUOUSLY DO NOT ─────────────────────────────────────────
 * Everything in the profile block was read off the organisation's own
 * website, which is the whole reason it is worth collecting: it is the
 * visible half of somebody else's marketing and a division marketer can
 * read all of it in an afternoon without buying a data product.
 *
 * NO ROW IN THAT BLOCK IS HIDDEN WHEN IT IS EMPTY. An absent membership
 * price is not a gap in the research, it is the finding: twelve of the
 * eight of the fourteen brands profiled name a plan and hide the number, five publish none at all, and every one of them routes the price question to
 * a phone number, and nobody in the market publishes an APR. Hide the
 * blanks and the strongest pattern in the data set disappears with them.
 *
 * ── THE QUOTE IS THE FIRST THING AFTER THE PROFILE ─────────────────
 ...
 * with nothing behind it is somebody's memory written down badly.
 *
 ...
 *
 * ── THE OFFERS BLOCK RARELY INVENTS A DISCOUNT ─────────────────────
 * Every rival in this set is already at or below fifty dollars on a
 * drain clearing and free camera inspections appear at four brands, so
 * a smaller coupon is not a position anybody can hold. What is extended
 * here is response time, coverage, a rate held for a year, a morning
 * with the crew. The cost to the brand is read from the offer catalogue
 * in the words the catalogue uses, rather than restated as a number
 * whose unit a reader would have to guess.
 */

/**
 * What this row is to the division, in a word a reader can act on.
 *
 * A THIRD COPY OF THIS MAP, and pretending otherwise would be worse than
 * saying it. It is also in the prospect drawer and in the map detail
 * pane, for the reason ORG_TYPE_META gives in the record selector: the
 * file where all of these belong is owned by another pass this week.
 * Three copies is a debt with a stated interest rate, not an accident.
 * When they move, delete all three.
 */
...
export interface ProspectRecordModalProps {
  ...
  prospectId: string;
  onClose: () => void;
  /** Branch-local day this record is held until, from RecordProvider. */
 ...
 * said no" and "the date on it passed" and "we pulled it" are three
 * different lessons, and a board that files all three as declined has
 * thrown two of them away. Lapsed earns its place in this market
 * particularly: one rival still has four campaign pages live whose
 * printed expiry dates ran out in March, September and December of last
 * year.
 */
const OFFER_STATE_META: Record<OfferState, StatusToken> = {
  ...
  lapsed: {
    ...
    label: "Lapsed",
    cssVar: "var(--warn)",
    note: "The date printed on it passed. Recorded rather than quietly reopened.",
  },
  withdrawn: {
    glyph: "◍",
    label: "Withdrawn",
    ...
    <>
      ...
      >
        ...
        <div className={styles.body}>
          ...
          <ul className={styles.highlights} aria-label="Highlights">
            ...
            <Highlight
              ...
              note={
                record.bookLines.length === 0
                  ? "No line in the book"
                  : `${record.bookLines.reduce((n, l) => n + l.guests, 0)} doors contracted`
              }
            />
          </ul>

          {/* ---------------------------------------------------------
              THE PROFILE. Who they are, where they stand, what they
              publish, why it matters and what we can see them betting
              on, in the order those questions actually get asked.

              Every row here is printed whether or not it has a value in
              it, and the empty ones carry the reason. See the header.
              --------------------------------------------------------- */}
          <section className={styles.section} aria-labelledby="rec-profile">
            <h3 className={styles.sectionTitle} id="rec-profile">
              The profile
            </h3>
            <p className={styles.sectionNote}>
              Read off their own website. The blanks are as informative as
              the values, so none of them is hidden.
            </p>

            <ul className={styles.headFacts}>
              <li>
                <span className={styles.factLabel}>Service lines</span>
                {p.services && p.services.length > 0 ? (
                  p.services.join(", ")
                ) : (
                  <span className={styles.absent}>Not published</span>
                )}
              </li>
              <li>
                <span className={styles.factLabel}>Service area</span>
                {p.serviceArea ?? (
                  <span className={styles.absent}>Not published</span>
                )}
              </li>
              <li>
                <span className={styles.factLabel}>Locations</span>
                {p.locationCount === undefined ? (
                  <span className={styles.absent}>Not published</span>
                ) : (
                  <span className="num">{p.locationCount}</span>
                )}
              </li>
              <li>
              ...
              <li>
                <span className={styles.factLabel}>In this market</span>
                {MARKET_ROLE_LABEL[p.role ?? ""] ?? (
                  <span className={styles.absent}>Not decided yet</span>
                )}
              </li>
              <li>
                <span className={styles.factLabel}>Current offer</span>
                {p.offer ?? (
                  <span className={styles.absent}>Nothing published</span>
                )}
              </li>
              <li>
                <span className={styles.factLabel}>Membership plan</span>
                {p.membership ?? (
                  <span className={styles.absent}>None published</span>
                )}
              </li>
              <li>
                <span className={styles.factLabel}>Financing</span>
                {p.financing ?? (
                  <span className={styles.absent}>No lender named</span>
                )}
              </li>
              <li>
                <span className={styles.factLabel}>Online booking</span>
                {p.onlineBooking === undefined ? (
                  <span className={styles.absent}>Not checked</span>
                ) : p.onlineBooking === "yes" ? (
              ...
              <li>
                <span className={styles.factLabel}>Why it matters</span>
                {p.marketingAngle ?? (
                  <span className={styles.absent}>
                    No angle written for this row yet
                  </span>
                )}
              </li>
              <li>
                <span className={styles.factLabel}>What we can see</span>
                {p.competitiveSignal ?? (
                  <span className={styles.absent}>
                    Nothing read off their marketing yet
                  </span>
                )}
              </li>
            </ul>
          ...
          {/* ---------------------------------------------------------
              LAST CONVERSATION. The thing he asked for by name, and the
              first thing the eye lands on after the profile.
              --------------------------------------------------------- */}
          ...
          {/* ---------------------------------------------------------
              ...
              when, whether it still stands, and what it costs the brand.
              --------------------------------------------------------- */}
          <section className={styles.section} aria-labelledby="rec-offers">
            ...
            <p className={styles.sectionNote}>
              ...
              off a coupon. The published prices in this market have already
              been competed down to a level nobody makes a margin at.
            </p>
                  ...
                  return (
                    <li key={extension.id} className={styles.offer} data-offer={extension.id}>
                      ...
                      <p className={styles.offerCost}>
                        <span className={styles.factLabel}>Cost to the brand</span>
                        {offer.costToVenue === 0 ? (
                ...
                {record.requests.map((r) => (
                  <li key={r.id} className={styles.request}>
                    ...
                    <span className={styles.requestFields}>
                      {r.headcount === null ? (
                        <span className={styles.absent}>No door count given</span>
                      ) : (
                        <>
                          <span className="num">{r.headcount}</span> doors
                        </>
                      )}
                      {r.desiredDate === null ? (
                ...
                {record.bookLines.map((l) => (
                  <li key={l.id} className={styles.request}>
                    ...
                    <span className={styles.requestAsk}>
                      <span className="num">{l.guests}</span> doors,{" "}
                      <span className="num">{l.lanesHeld}</span> crew{" "}
                      {l.lanesHeld === 1 ? "slot" : "slots"}
                    </span>
                    <span className={styles.requestFields}>
                      <Figure
