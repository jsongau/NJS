 ...
 *
 * WHAT IS PAGE LOCAL AND WHAT IS SHARED. The service line filter, the
 * search query and the emailable only toggle come from
 * `PipelineProvider` and are shared with the desk on purpose: filter to
 * plumbing on the map and the desk is filtered to plumbing when the
 * reader gets back to it, because a service line is a permanent property
 * of an organisation. The board segment ("all", "never touched", "live
 * conversations") is page local,
 ...
 *
 * NO SERVICE LINE IS NAMED IN THIS FILE. Every line keyed record is
 * built by iterating LANE_ORDER, so a tenth service line appears in the
...
export const BOARD_SEGMENTS: BoardSegmentMeta[] = [
  {
    ...
    short: "All",
    glyph: "●",
    what: "Every organisation left after the shared service line, search and written door filters.",
  },
  {
    value: "untouched",
    label: "Never touched",
    short: "Untouched",
    glyph: "○",
    what: "Nothing has been sent and nobody has been seen. The whole board started here.",
  },
  {
    value: "live",
    ...
    short: "Live",
    glyph: "◐",
    what: "A conversation is open or a date is held on the schedule. These are the ones with something to lose.",
  },
];
 ...
 *
 * The occasion segment on the left pane separates the demand that
 * arrives on its own from the demand somebody has to be persuaded into,
 * and it is implemented as a shortcut over the SHARED service line
 * filter rather than as a second parallel filter of its own. A class
 * segment that filtered locally would let the map show three lines while
 * the desk showed nine, which is precisely the incoherence the shared
...
/**
 * Which occasion class, if any, the shared filter currently equals.
 *
 * Returns null when the filter is empty (both classes showing) or when
 * the reader has narrowed past a whole class by ticking individual
 * service line chips. Null is correct in that second case: the segment
 * should stop claiming to describe a selection it no longer describes.
 */
 ...
 * "Non-discretionary 80, discretionary 249" is a description of the
 * territory. Wire it to the filtered rows and it becomes a description
 * of the last click.
 */
...
/**
 * Service line tally over whatever is currently plotted, every line
 * present.
 *
 * Built from LANE_ORDER so the legend can render a fixed set of rows and
 * so a service line with nothing on the board still shows its zero,
 * which is itself information: an empty line is a gap in the week, not
 * an absence of data.
 */
...
/**
 * Reachable doors as one number, for arithmetic only.
 *
 * Every door count in the data set is a RANGE with a stated basis, and
 * the range is the honest figure. The midpoint exists because you cannot
 * sum a range into a service line total, and it carries "modeled"
 * provenance everywhere it is shown so it can never be mistaken for a
 * measurement.
 */
export function headcountMidpoint(p: Prospect): number | null {
  /* Null where the row is a competitor or an operating brand and carries
     no modeled door count at all. Every caller has to decide what to draw
     for that case, which is the point of returning null over zero. */
  return groupProfile(p)?.mid ?? null;
}

/**
 * Crew slots that volume would consume at this console's own planning
 * rate of one slot per twenty doors, against the working assumption of
 * twenty six slots a day at the Brea branch.
 *
 * THE RATIO IS OURS AND THE CAPACITY IS OURS. No brand in this market
 * publishes a jobs-per-truck figure and none of the five West Division
 * sites publishes a technician count by branch, so both numbers are
 * planning assumptions rather than observations, and every screen that
 * divides by them says modelled beside the answer.
 *
 * Exported for the detail pane, which has to answer this for a prospect
 * that is not currently on the board.
 */
export function lanesAtMidpointFor(p: Prospect): number | null {
  const mid = headcountMidpoint(p);
  return mid === null ? null : crewSlotsForDoors(mid);
}
...
export interface MapRow extends DeskLine {
  /** Midpoint of the modeled door count range, rounded. */
  /** Null on rows that carry no modeled door count. See domain/booking.ts. */
  guestsMidpoint: number | null;
 ...
 * twenty seven plumbing rivals have never been read" is a sentence.
 * "Nine of the forty untouched organisations are plumbers" is a
 * different sentence about a different thing, and the stat bar is asking
 * the first one.
 */
...
export interface MapBoardTotals {
  ...
  /** Sum of midpoint door counts across plotted rows. */
  guestsInPlay: number;
  /** Sum of crew slots at midpoint across plotted rows. */
  lanesAtMidpoint: number;
  /** Plotted rows whose furthest status is unworked. */
  neverTouched: number;
  /** Plotted rows in conversation or with a date held. */
...
export function mapBoardTotals(
  ...
  for (const r of rows) {
    ...
    /* A competitor or an operating brand carries no door count, so it
       adds nothing to doors in play rather than adding a zero that would
       look like a measurement. */
    totals.guestsInPlay += headcountMidpoint(p) ?? 0;
    totals.lanesAtMidpoint += r.lanesAtMidpoint ?? 0;
 ...
 * denominator that means something, and neither Champions Group nor any
 * of its West Division brands publishes a lead target, a crew count or a
 * media budget for Brea. Inventing one would be the single most damaging
 * thing this screen could do, because every other number on it is real.
 *
...
export function mapBoardStats(totals: MapBoardTotals): MapBoardStat[] {
  return [
    ...
    /* The three written doors, in the vocabulary's own words. The
       labels are lifted from EMAIL_CONFIDENCE rather than reworded here,
       so the strip, the chips on the list cards and the drawer all call
       the same thing by the same name. They add up to the figure on
       their left, which is the whole reason the middle one exists. */
    {
      key: "written-door",
    ...
    },
    {
      ...
      provenance: "public",
      note: "Publish no email and no form. These are go-sees, and a go-see is the only route into an organisation that publishes no written door.",
    },
    {
      ...
      provenance: "modeled",
      note: "Straight line, not drive time. Measured from the Brea branch at 625 Columbia Street by this app's own haversine, so it is modeled rather than published.",
    },
    /*
    ...
    */
    ...
    },
    {
      ...
      provenance: "modeled",
      note: "Sum of the midpoint of every modeled door count range on the board. Every one of those is a range with a stated basis, so this is an order of magnitude and not a forecast. Competitors and the division's own brands carry no door count and add nothing to it.",
    },
    {
      key: "never-touched",
      label: "Never touched",
    ...
    },
    {
      ...
      provenance: "illustrative",
      note: "A conversation is open or a date is held on the schedule. Seeded for the prototype, and badged the same way the desk badges it.",
    },
  ];
}
...
export function mapSummarySentence(totals: MapBoardTotals): string {
  return (
    `${totals.plotted} organisations plotted around the Service Champions branch at 625 Columbia Street, Brea. ` +
 ...
 *
 * "12 of 329" beats "12", every time. A reader who cannot see how much
