 ...
 * for a held slot; and a desk that is being asked to prove it generates
 * calls cannot afford a plan that rewards paper.
 *
 ...
 *   the money was in.
 *
 *   THE BALANCE is treated as collected on the day the work runs,
 *   because that is when the crew turns up and the account settles.
 *
 * Both are stated on screen beside the figure. Neither is a claim about
 * the brand's own published terms.
 */
 ...
 * signs one large property portfolio while forty calendar locked
 * accounts go untouched through their only buying window has burned the
 * list for a year.
 *
...
// ---------------------------------------------------------------
// Gate two: hours out in the territory
// ---------------------------------------------------------------

/** The activity types this console counts as time out in the territory. */
 ...
 *
 * "Hours out in the territory at or above the planned share" can be read
...
export type BonusState =
  | { kind: "pre-opening"; gatesMet: number; gatesTotal: number }
  | { kind: "gate-missed" }
...
export interface PayReading {
  ...
  /** Collected over quota. Null in a planned quarter, which has none. */
...
export function payReading(input: {
  ...
  const attainment = quarter.preOpening ? null : collected.inQuarter / quota;
  ...
  if (quarter.preOpening) {
    /* No revenue quota inside the planned calendar, because there is no
       prior season to pace it against, so the quarter pays on the two
       leading gates, half each, all or nothing. */
    const gatesMet = (coverage.met ? 1 : 0) + (outside.met ? 1 : 0);
    state = { kind: "pre-opening", gatesMet, gatesTotal: 2 };
