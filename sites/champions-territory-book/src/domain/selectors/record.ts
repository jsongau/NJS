...
/**
 * Whole days between two instants, counted in branch-local calendar
 * days.
 *
 ...
 * Pitch status is drawn as a filling circle, offer family as a
 * patterned square, service line as a pointed or solid mark. A reader
 * scanning a dense row can tell which system a glyph belongs to before
 * working out which value it is. Within this family the fill pattern
 * carries the meaning: nothing known, a divided box for a decision split
 * between a site and a region, a half box for an institution that buys
 * half on a calendar and half on a purchase order, a solid box for one
 * site with the decision inside it.
 */
export const ORG_TYPE_META: Record<OrgType, StatusToken> = {
  ...
  independent: {
    ...
    label: "Independent",
    cssVar: "var(--ok)",
    note: "One site, and the person who can say yes works in it. An owner behind the counter, a practice manager, a single-branch contractor whose approval chain tops out on the premises.",
  },
  chain: {
    glyph: "◫",
    label: "Chain or franchise",
    cssVar: "var(--info)",
    note: "A branch, store, franchised unit or flagged property. The person on site can want it and still not approve it, so the useful outcome of a visit is often the role above them.",
  },
  unknown: {
    glyph: "◻",
    label: "Type not recorded",
    cssVar: "var(--neutral)",
    note: "The research on this row does not say where the approval sits. A handful of organisations on the board are honestly in this state rather than guessed into one of the other three.",
  },
};
 ...
 * mechanic available to a three hundred and twenty nine record
 * territory worked by one person, because the failure mode of a solo
 * marketer is not losing deals, it is forgetting them.
 *
 * THE THRESHOLDS ARE DIFFERENT PER STATUS BECAUSE THE COST OF SILENCE
 * IS DIFFERENT PER STATUS. A held date is the urgent one: it blocks a
 * crew day nobody else can be offered, so three days without contact is
 * already expensive to the branch and not merely impolite to the
 * customer. A live conversation goes cold fastest, so five. A first
 * touch with nothing back gets seven, which is the point at which a
 * second email is still reasonable and an eighth day of hope is not.
 *
 ...
 * because signed work is worked backwards from its own date on the
 * dispatch board, not forwards from the last email. "lost" has none
 * because a loss is not work; the next window is diarised instead, which
 * is a date rather than a decay.
 *
 * THESE ARE THIS DESK'S OWN INTERVALS AND THEY ARE ILLUSTRATIVE. Not one
 * brand in this market publishes a follow-up service level of any kind,
 * so nothing here is a claim about how any of them operates.
 */
...
export function stalenessOf(
  ...
  if (threshold === null) {
    return {
      ...
      note:
        status === "booked"
          ? `Booked. Worked backwards from the date the work runs rather than forwards from the last contact, which was ${days(elapsed)} ago.`
          : status === "lost"
 ...
 * browsers'. Naming a number of doors sits just under them, because it
 * costs nothing to say and is still the number every proposal is built
 * from. Asking who else uses us is worth more than it looks: it is a
 ...
 * already have somebody" outweighs three enthusiastic questions,
...
> = {
  ...
  "asked-for-a-price": {
    ...
    weight: 25,
    note: "The most common opening question in this territory and the hardest to answer, because the brand publishes a price for a tune-up and no price at all for the membership.",
  },
  "named-a-headcount": {
    label: "Named a number of doors",
    weight: 20,
    note: "Costs them nothing to say and is the number every proposal is built from. Worth having in writing.",
  },
  "asked-who-else-has-booked": {
    label: "Asked who else uses us",
    weight: 15,
  ...
  "held-a-date": {
    label: "Held a date",
    weight: 45,
    note: "A crew day blocked against no deposit. Worth nothing in the ledger and a great deal as a reading of intent.",
  },
  signed: {
    label: "Signed",
    weight: 60,
  ...
  "went-quiet-after-a-quote": {
    label: "Went quiet after a proposal",
    weight: -20,
    note: "Derived from silence rather than read off a message. A proposal out and a thread past its threshold is the most common way a live conversation dies.",
  },
  "booked-elsewhere": {
    label: "Went with somebody else",
    weight: -60,
  ...
  "said-no": {
    label: "Said no",
    weight: -60,
    note: "Recorded rather than hidden. A no to this offer is rarely a no to the organisation, which is why the record stays on the board.",
  },
};
 ...
 * result, which is exactly what a one person territory board must not be
 * used for.
 */
export type IntentLevel =
  ...
  | "signalled"
  /** A crew day is held, or it is signed. */
  | "committed"
...
export const INTENT_META: Record<IntentLevel, StatusToken> = {
  ...
  signalled: {
    ...
    label: "Signalled",
    cssVar: "var(--info)",
    note: "They asked for a date, a price, a number of doors or a meeting. Interest with evidence behind it and nothing held.",
  },
  cooling: {
    glyph: "◌",
    label: "Cooling",
  ...
  committed: {
    ...
    label: "Committed",
    cssVar: "var(--ok)",
    note: "A date is held or a contract is signed. A hold is a commitment of theirs and of the branch's schedule, and it is still not revenue.",
  },
  declined: {
    glyph: "✕",
    label: "Declined",
    cssVar: "var(--risk)",
    note: "They said no or they went elsewhere, in their own words. Kept on the board because a service line full of quiet losses is a finding.",
  },
};
...
function readIntent(
  ...
  if (hasOpenQuote && staleness.stale && positives.length > 0) {
    ...
    evidence.push({
      ...
      at: RECORD_AS_OF,
      quote: `A proposal or an offer is out and the thread has been quiet for ${days(staleness.daysSinceActivity ?? 0)}, which is past the ${staleness.threshold} day threshold for this status.`,
      weight: meta.weight,
      label: meta.label,
    });
  }
  ...
  if (status === "booked" || has("signed")) {
    ...
    headline = has("booked-elsewhere")
      ? "They told us they had already gone with somebody else."
      : "They said no, and the reason is in the thread.";
  } else if (status === "soft-hold" || has("held-a-date")) {
    level = "committed";
    headline = "A date is held against no deposit, which commits the schedule and not the money.";
 ...
 *
 * The cost to the brand lives on the offer and is read through here
 ...
 * cold follow-up because it is costing the branch a crew day. A requeue
...
function nextActionFor(
  ...
  if (status === "booked") {
    return {
      kind: "convert",
      label: "Confirm the dispatch",
      why: "Signed with a deposit taken. The remaining work is operational, closer to the date rather than now.",
    };
  }
  ...
  if (status === "lost") {
    return {
      ...
      why: `Lost this one. They buy in ${p.buyingWindow || "a window not recorded"}, so the next move is a date in the diary rather than another email.`,
    };
  }
  ...
  if (status === "soft-hold") {
    return {
      ...
      why: staleness.stale
        ? `A date is held against no deposit and the thread has been quiet ${days(staleness.daysSinceActivity ?? 0)}. It blocks a crew day nobody else can be offered.`
...
export interface ProspectRecord {
  ...
  /** Inbound leads, which carry their own response clock. */
 ...
 * for every id that is, including the great majority with no history at
 * all. An organisation with nothing recorded against it is not an error
 * case to be guarded against by four different callers; it is most of
 * this territory, and it gets the same shape as the rest.
 */
 ...
 * should call `prospectRecord` rather than building all three hundred
 * and twenty nine and picking one out.
 */
