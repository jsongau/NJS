 ...
 * to plot three hundred and twenty nine real organisations as a coloured body
 * with a single geometric character inside it. A reader could tell the
 * two occasion classes apart at a glance and could tell the lanes apart
 * only by consulting the key, because a diamond and a lozenge are not a
 * drain call and a panel upgrade, they are two lozenges. So each lane now
 * carries a drawing of WHAT IT TRADES IN: a snowflake, a droplet, a
 * breaker panel, a service van, a trap, a bed, a key, a schoolhouse, a
 * tank. The map is then readable without the key, and the key teaches it
 * in one pass instead of asking a reader to memorise nine abstract
 * marks.
 *
 ...
 *   ICON    what the lane trades in, so nine lanes stay separable rather
 *           than only the two classes. In field run mode the icon is
 ...
 *           because unworked is the honest default for most of a
 *           territory nobody has had the hours to work, and a hundred
 *           pips saying "nothing has happened here" is a hundred marks
 *           of noise.
 ...
 * because `fill="var(--lane-hvac)"` does not resolve while
 * `style="fill: var(--lane-hvac)"` does. Custom properties inherit
 ...
 * `var(--lane-hvac)`, never a colour. `sw` is the stroke width in
 * viewBox units.
 */
export interface MarkPaint {
  ...
  stroke?: string;
  sw?: number;
  /** `stroke-dasharray`, in viewBox units. The anchor's ring uses it. */
  dash?: string;
  opacity?: number;
 ...
 * questions. The title is what the drawing IS ("A snowflake, heating and
 * air conditioning"), which is what a reader hovering a legend swatch
 * wants. The aria label is what this particular mark MEANS in context
 * ("a Brea property management office, property and referral partners,
 * chosen"), which is what a screen reader user tabbing across a map
 * needs. A mark with only one of the two either reads as an unlabelled
 * picture or announces a shape rather than an organisation.
 */
export interface MarkScene {
  ...
  viewBox: number;
  /** Non-square marks state their own box. The anchor plate needs it. */
 ...
 * another, and keeping them apart is what lets the same droplet be drawn
 * in the lane's own hue in a legend, in a paper outline behind itself on
 * the map, and in plain ink on a printed contact sheet, without three
 * copies of the path.
 *
 * A note on what each one is, because the choices are not arbitrary. Six
 * of the nine draw the OBJECT the service line trades in and three draw
 * the PLACE the partner surface sits in, so that a reader who has never
 * opened the key can still tell a drain run from a property managers'
 * morning. Where two lanes risked the same silhouette, one of them moved:
 * property partners are a KEY rather than a house, because a house and a
 * schoolhouse are one gable apart at sixteen pixels.
 *
 * The two building marks that remain are deliberately far apart. The
 * community mark is a roof, a flag and a centred door; the employer mark
 * is a bed, which is furniture rather than architecture. Two buildings
 * with a centred door and a different hat are the same mark in greyscale,
 * and this set was checked for exactly that.
 *
 * WHAT IT GETS WRONG, said plainly. A snowflake is a cooling mark doing
 * duty for a line that sells heating for half the year, and there is no
 * single object that means "heating and air conditioning" at sixteen
 * pixels. The alternative was a flame beside a snowflake, which is two
 * drawings in one box and mush at map scale. The label under it carries
 * the other half of the meaning.
 */
const LANE_ICON_PATHS: Record<Lane, readonly string[]> = {
  /* A snowflake. Three axes and two chevrons, the climate mark. */
  "hvac": [
    "M12 3 V21",
    "M4.25 7.5 L19.75 16.5",
    "M19.75 7.5 L4.25 16.5",
    "M9 6 L12 3 L15 6",
    "M9 18 L12 21 L15 18",
  ],
  /* A water droplet over two ripples. */
  "plumbing": [
    "M12 2.75 C12 2.75 5.75 10 5.75 14.25 A6.25 6.25 0 0 0 18.25 14.25 C18.25 10 12 2.75 12 2.75 Z",
    "M7.5 20.25 H16.5",
    "M9.75 22.25 H14.25",
  ],
  /* A breaker panel with a bolt beside the breakers. */
  "electrical": [
    "M6 3.5 H18 A1.5 1.5 0 0 1 19.5 5 V19 A1.5 1.5 0 0 1 18 20.5 H6 A1.5 1.5 0 0 1 4.5 19 V5 A1.5 1.5 0 0 1 6 3.5 Z",
    "M7.5 8 H11",
    "M7.5 12 H11",
    "M7.5 16 H11",
    "M15.75 7.5 L13 12.75 H15.5 L14.75 16.5 L17.5 11.25 H15 Z",
  ],
  /* A service van in profile. One van, every line in the house. */
  "multi-service": [
    "M3.25 16.25 V7 H14.25 L18.5 12 H20.75 V16.25",
    "M14.25 7 V12",
    "M14.25 12 H20.75",
  ...
  /* A trap under a sink. The U-bend, flanged at both ends. */
  "drain-sewer": [
    "M6.5 3.5 V12.5 A5.5 5.5 0 0 0 17.5 12.5 V3.5",
    "M4.25 3.5 H8.75",
    "M15.25 3.5 H19.75",
    "M9.25 15.5 H14.75",
    "M10.25 17.25 H13.75",
  ],
  /* A bed. Headboard, frame, pillow, turned covers. Hospitality. */
  "partner-employer": [
    "M2.75 19.25 V7.5",
  ...
  /* A key. Whoever holds it decides who gets called. */
  "partner-property": [
    "M7.5 6.5 A3.5 3.5 0 1 0 7.5 13.5 A3.5 3.5 0 1 0 7.5 6.5",
    "M10.75 10 H20.5",
    "M16.5 10 V13.25 M19 10 V12.5",
  ],
  /* A schoolhouse. Gable roof, centred door, flag on the ridge. */
  "partner-community": [
    "M2.75 10.25 L12 4.5 L21.25 10.25 M4.75 10.25 V20.5 H19.25 V10.25",
    "M10 20.5 V15.25 H14 V20.5",
    "M12 4.5 V1.75 M12 2.25 L15 3 L12 3.75",
  ],
  /* A tank. Two pipes on top, a band, a gauge and a drain. */
  "water-heater": [
    "M7 6.75 A2.75 2.75 0 0 1 9.75 4 H14.25 A2.75 2.75 0 0 1 17 6.75 V17.5 A2.75 2.75 0 0 1 14.25 20.25 H9.75 A2.75 2.75 0 0 1 7 17.5 Z",
    "M7 9.5 H17",
    "M10 4 V1.75",
    "M14 4 V1.75",
    "M12 13 A1.75 1.75 0 1 0 12 16.5 A1.75 1.75 0 1 0 12 13",
    "M9 20.25 V22.25",
  ],
};
...
const LANE_ICON_NOUN: Record<Lane, string> = {
  ...
  "electrical": "A breaker panel",
  "multi-service": "A service van",
  "drain-sewer": "A trap under a sink",
  "partner-employer": "A hotel bed",
  "partner-property": "A key",
  "partner-community": "A schoolhouse with a flag",
  "water-heater": "A water heater tank",
};
...
export interface ProspectMarkerVisual {
  ...
  status?: PitchStatus;
  /**
   * Position in the field run, 1 based. Replaces the lane icon.
   *
   ...
   * row four to a pin does it instantly with a 4 and slowly with a van.
   */
  routeStep?: number;
  /** Run mode is on and this organisation is not on the route. */
  muted?: boolean;
  selected?: boolean;
}
 ...
 * heating lane look more important than the partner lanes for reasons of
 ...
 * because a droplet that shrinks when it becomes calendar-locked would be
 * encoding the occasion class twice and the lane not at all.
 */
 ...
 *
 * `unworked` deliberately draws NOTHING. Most of a territory has never
 * been worked, so a hundred pips all saying "no contact" would be a
 * hundred marks of noise over the one signal that matters, which is the
 * handful that have moved.
 */
const PIP_CX = 28.5;
const PIP_CY = 28.5;
const PIP_R = 5.5;
...
export function prospectMarkScene(v: ProspectMarkerVisual): MarkScene {
  ...
  /* A halo rather than a colour change, because the stroke is already
     ...
     composes: a selected, booked, calendar-locked heating row is a
     haloed pointed body with a snowflake in it and a full disc on its
     corner, and every one of those four facts is still legible. */
  if (v.selected) {
    shapes.push(
  ...
  /*
    THE PAPER OUTLINE IS NOT DECORATION. Three hundred and twenty nine
 ...
 *
 * The mark at the centre of the map is OURS. Service Champions Brea is a
 * trading branch and has been for twenty five years, and it sits on a
 * board of three hundred and twenty nine organisations that are competitors,
 * partners and prospects. A solid pin says "here is a business", which
 * is true of every other pin on the screen, so a solid pin here would
 * make the anchor the two hundred and twelfth of them.
 *
 * So the ring is dashed and the plate under it says the address is ours,
 * and neither of those depends on the amber. Nothing else on this map is
 ...
 *
 * ── THE LABEL HAS A TWELVE CHARACTER BUDGET, AND IT IS REAL ───────
 * The plate is sized from the label at 6.4 pixels a character with 22 of
 * padding, and it is clamped to the 100 pixel drawing. Twelve characters
 * lands at 98.8, which is the last width that fits, and "Our address" is
 * eleven. A caller passing anything longer gets a clamped plate with
 * centred text running past its rounded ends, so a longer phrase belongs
 * in the popup, which has a paragraph for it, rather than on the plate.
 *
 * Deliberately NOT a house or a van silhouette. `PinMark` owns the
 * product's own mark and its path is load-bearing there; a second copy of
 * it in this file is a second copy to keep in step, and the anchor does
 * not need one. What this mark has to say is "this one is ours", which
 * is geometry a circle can do.
 */
export function venueMarkScene(name: string, label = "Our address"): MarkScene {
  const w = 104;
  const cx = w / 2;
  const cy = 21;
  ...
  return {
    ...
    viewBox: w,
    viewBoxHeight: 60,
    title: `${name}, our own address`,
    ariaLabel: `${name}, our own address`,
    shapes: [
      {
        k: "circle",
        cx,
...
export function venueMarkerHtml(name: string, label = "Our address"): string {
 ...
 * claim the group does not support: seven marks reading "hvac" when
