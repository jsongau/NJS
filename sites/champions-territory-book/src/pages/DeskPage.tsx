...
/**
 * THE DESK. The front door of the console.
 *
 * WHAT THIS CONSOLE IS. A local marketing and market prospecting desk for
 * one home services brand in one defined territory: the accounts,
 * partners and community surfaces that produce incremental phone calls
 * and web leads, ranked so that a person with a budget and a week knows
 * which twenty to work. Everything else in this build is evidence for
 * that order.
 *
 ...
 * The territory holds a few hundred organisations and one person working
 * them. Alphabetical, by distance, or by Google rating all produce a list
 * that looks organised and wastes the week. The ranking in
 * domain/selectors/desk.ts weights reachability heaviest, then whether
 * the demand exists without us, then whether the season is open, and only
 * then size, because size is the softest figure on every row.
 *
 ...
 * Whether a season is open is the third heaviest criterion, and it needs
 * a "now". Reading the system clock would mean this page ranks
 ...
 * booked work is a signature, and the money lives in BookProvider where
 * a booking cannot exist without a line to carry it. Letting the desk
 * mark something "Booked" would put revenue in the pipeline with nothing
 * underneath it, which is the single failure this console's two-ledger
 ...
 * "Non-discretionary" in the rail narrows this board in the same render,
 * and it narrows the map and the offers board too, which is the
 * behaviour the territory page already promises in prose.
 *
 ...
 * the reason the rail gives for doing the same: a tenth service line
 * added to the union has to land in one of the two classes, and a
 * hand-written list would quietly leave it out of both.
 */
 ...
 * than three. A reader who has toggled two service line chips by hand is
 * in a state none of the three named choices describes, and a control
 * that snapped back to "Every organisation" in that case would be showing
 * a value that disagrees with the board behind it.
 */
    ...
    >
      ...
      <div className={styles.actions}>
        ...
        <Button
          ...
          title={
            advance === null
              ? "The step from a held date to booked work is a signature, so it happens on the Book page where the money lives."
      ...
      {expanded ? (
        <div className={styles.breakdown} id={whyId}>
          ...
          <p className={styles.breakdownFoot}>
            <code className={styles.code}>domain/selectors/desk.ts</code>. Crew
            days at the midpoint door count, at this console's own planning rate
            of one slot per twenty doors:{" "}
...
export function DeskPage() {
      ...
      : laneClass === "discretionary"
        ...
        : laneClass === "other"
          ? pipeline.laneFilter.length === 1
            ...
            : `${pipeline.laneFilter.length} service lines`
          : undefined;
   ...
   *
   * Named off the same filter the chips write, so the block above the
   * board and the chips above it cannot describe different sets. A
   * hand-picked pair of service lines is named as the two lines rather
   * than as "filtered", because "filtered" tells a reader that something
   * was removed and not what is left.
   */
  const cut = useMemo(() => {
    if (laneClass === "locked") {
      return {
    ...
    if (laneClass === "other") {
      return {
        label:
          pipeline.laneFilter.length === 1
            ? LANE_META[pipeline.laneFilter[0]].label
            : `${pipeline.laneFilter.length} service lines, hand-picked`,
        glyph: "◆",
        tone: "var(--accent)",
      };
    }
   ...
   * than only the service line. Searching "property management" narrows
   * the board just as hard as pressing Plumbing does, and a block that
   * only redrew for one of the two would be telling the reader that the
   * other did nothing.
   */
  const cutKey = [
    laneClass,
   ...
   * this position counted the whole territory, so it reported the untouched
   * count for every organisation on the board while the rows underneath
   * were a single service line. These are computed off `lines`, which is
   * the same array the board draws, so they cannot disagree with what is
   * on screen.
   */
  ...
  return (
    <div className={styles.page}>
      <div className={styles.inner}>
        {/*
          The band. The context control is the demand class, which is the
          same filter the rail's second level sets and the same one the
          map and the offers board read, so choosing it here moves all
          three. It is offered again in the band because a reader
        ...
        */}
            ...
            <ContextSelect
              ...
              options={[
                ...
                /* The hand-picked state is offered as a reading and
                   ...
                   service line chips below are where that state is made. */
                ...(laneClass === "other"
                  ? [
                      {
                        value: "other",
                        label: `Hand-picked service lines (${pipeline.laneFilter.length})`,
                      },
                    ]
                  : []),
              ]}
        ...
        <header className={styles.head}>
          <p className={styles.eyebrow}>
            Stage one, {period.label}, West Division
          </p>
          <h1 className={styles.h1}>
            The desk: who to work today for calls and leads
          </h1>
          {/*
            EVERY COUNT IN THIS HEADER IS DERIVED, AND THAT IS A FIX
            RATHER THAN A FLOURISH. These sentences used to carry the
            totals as words: a spelled-out board size in the lede, "All
            eight lanes" on the first filter, the same total again in the
            result line. A ninth service line landed and a later research
            pass grew the board, and the prose stayed where it was, so the
            first sentence on the front door disagreed with the stat card
            directly beneath it. On a console whose whole argument is that
            its numbers are derived rather than asserted, that is the
            cheapest possible reason to doubt every other figure on the
            site. Reading the length of the array removes the class of
        ...
        */}
        {/* -----------------------------------------------------------
            FILTERS. Service line chips are toggles rather than a
            multi-select, because the service line is the thing a marketer
            actually plans a week around and a dropdown hides which ones
            are on.
            ----------------------------------------------------------- */}
          ...
          >
            ...
            >
              <span className={styles.allLanes}>
                All {LANE_ORDER.length} service lines
              </span>
            </button>
            {LANE_ORDER.map((lane) => (
              <button
        ...
        {/*
          ...
          Typing in the search box or toggling a service line changes this
        ...
        */}
        ...
        {/* -----------------------------------------------------------
            ...
            figures that are true of THIS cut rather than of the whole
            territory. The strip of standing figures moved under the board to
        ...
        */}
        ...
        {/* -----------------------------------------------------------
            WHAT THE TERRITORY HOLDS.

            THIS STRIP USED TO SIT ABOVE THE FILTERS AND IT DID NOT MOVE
            WHEN THEY DID. Five figures over the whole territory, drawn in
            the position a reader looks to for the answer to the thing
            they just pressed, and three of the five do not narrow with a
            service line at all. So choosing one line left the top of the
            screen reading the whole board while the rows underneath
            showed twenty two of them.
        ...
        <section className={styles.kpis} aria-labelledby="desk-strip-heading">
          ...
          <Kpi
            ...
            note="Real organisations inside the territory this brand publishes as its service area. Sixty nine carry the Google place id they came from; the rest carry no place id and were geocoded with the US Census Bureau geocoder."
            provenance="public"
          />
          <Kpi
          ...
          <Kpi
            ...
            label="Never touched"
            note="What an unworked territory actually looks like on day one. Softening this would have been the dishonest move."
            provenance="illustrative"
          />
          <Kpi
            value={liveConversationCount(pipeline)}
            label="Live conversations"
            note="Replied and want to talk, or holding a date on the schedule. A hold is worth nothing until it converts."
            provenance="illustrative"
          />
          <Kpi
