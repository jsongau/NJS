...
import { PACKAGES, GATED_PACKAGES, LOWEST_PUBLISHED_PLAN_PRICE } from "@/data/packages";
 ...
 * times is a marketing problem, and the two want completely different
 * work. The first is answered by a better sentence on the next call.
 * The second is answered by changing what the brand actually publishes,
 * or by rewriting the landing page the campaign points at, or by
 * accepting that a whole service line cannot be sold at the price the
 * market has set. Nothing tells those two apart except writing them
 * down, and a team that does not write them down relearns the same
 * lesson every season with a different rep.
 *
 * So this page is a register and not a script. Each row carries the
 * objection IN THE CUSTOMER'S OWN VOICE, why they are right to raise
 * it, the answer that works, WHAT THAT ANSWER COSTS THE BRAND, and a
 * disposition the reader can set. The cost line is the one most
 * objection-handling documents leave out and the one a division
 * director reads first, because every answer on this page gives
 * something away: doing the repair that was asked for gives away the
 * replacement ticket, telling a homeowner where the cheaper drain price
 * is gives away the drain, and answering the rebate question honestly
 * gives away money the customer had already counted.
 *
 * ----- WHY ONE CARD IS TWICE THE SIZE OF THE OTHERS -------------------
 *
 * "Somebody quoted me hundreds less and nothing you publish explains
 * the difference" is not one objection among seven. It is true of every
 * conversation in every service line, it is what this market has
 * trained every homeowner to say, and the honest answer to it is the
 * entire argument for filling this role. Thirteen rival brands were
 * profiled across these five counties and not one publishes what its
 * membership costs, so the
 * only number a customer can compare is the coupon, and on the coupon
 * the smallest operator always wins.
 *
 * A reader who reads exactly one card on this page should read that
 * one, so it is lifted out of the list, given the page's only amber
 * rule and the largest pull quote in the application. Ranking by
 * severity alone would have left it as the first of seven identical
 * cards, which is a layout that has quietly decided all seven are the
 ...
 * because nobody has been standing in the territory with a tally
 ...
 * account on the desk moves the figure here, and it carries a modeled
  ...
  return (
    <article className={styles.card} data-tone={tone} aria-labelledby={`obj-${objection.id}`}>
      ...
      {/*
        ...
        customer's sentence deserves the same treatment for the same
        reason: every other word on this card is the brand talking to
        itself, and the reader needs to be able to tell at a glance
        which is which.
      */}
      ...
      <div className={styles.lanes}>
        <p className={styles.lanesLabel}>
          {everyLane
            ? "Raised in every service line on the board"
            : "Raised in these service lines"}
        </p>
      ...
      <div className={styles.prose}>
        ...
        <section className={styles.block} data-key="cost">
          <h4 className={styles.blockHead}>What that answer costs the brand</h4>
      ...
      <div className={styles.foot}>
        <Button
          ...
          aria-label={`Filter the desk to the ${lanes.length} service lines that raise ${objection.short}`}
        >
          {everyLane
            ? "Work the whole board on the desk"
            : `Work these ${lanes.length} service lines on the desk`}
        </Button>
...
export function ObjectionsPage() {
   ...
   * Two of the seven objections cover every service line on the board
   * and the other five overlap heavily, so summing the card figures
   * would count the same property several times over and produce a
   * number larger than the territory contains. This is the count
      ...
      <div className={styles.inner}>
        <header className={styles.head}>
          ...
          {/* A register rather than a script: every row carries the answer
              and what the answer costs the brand. */}
          <p
            className={styles.provenanceNote}
            title="The facts underneath these rows are sourced: the published rival coupon prices, the partnership one rival declares with another, the terminated federal credit, the reserved state programme, the live utility money. The sentences in quotation marks were written for this work sample."
          >
        ...
        <section className={styles.standing} aria-labelledby="standing-h">
          ...
          <p className={styles.standingUnion}>
            ...
            the territory sit behind at least one objection this register still
            calls open.
            <ProvenanceBadge provenance="modeled" compact />{" "}
            <span
              className={styles.unionNote}
              title="Two rows on this page cover every service line on the board and the rest overlap heavily, so adding the card figures together would count the same property several times over."
            >
              A union, not a sum.
            </span>
          </p>
          ...
          <section className={styles.feature} aria-labelledby="feature-h">
            <div className={styles.featureHead}>
              ...
              <h2 className={styles.h2} id="feature-h">
                The objection this whole market created
              </h2>
              <p className={styles.featureLede}>
                True of every conversation in every service line.
              </p>
            </div>
            ...
            {cardFor(
              ...
              <div className={styles.gate}>
                ...
                <p className={styles.gateText}>
                  Of the fourteen brands profiled, eight name a membership
                  and refuse to price it and five publish no plan at all, so
                  not one of them puts a price where a household can find it.
                  So the only
                  number a household can compare in ninety seconds is the
                  coupon, and on the coupon the two van outfit wins.
                </p>

                <div className={styles.gateFigures}>
                  <div className={styles.gateFigure}>
                    <span className={styles.gateFigureLabel}>
                      What the club costs a month
                    </span>
                    <WithheldFigure
                      reason={
                        <>
                          The plan page names the benefits and prints no price,
                          so the figure comes from a phone call. Two brands in
                          this division's own family publish theirs and the
                          rest of the market publishes none, which is the
                          unoccupied ground this work sample is arguing for.
                        </>
                      }
                    />
                  </div>
                  ...
                  <div className={styles.gateFigure}>
                    <span className={styles.gateFigureLabel}>
                      Offers carrying no published price
                    </span>
                    <Figure
                  ...
                  <div className={styles.gateFigure}>
                    <span className={styles.gateFigureLabel}>
                      The lowest figure published anywhere on the menu
                    </span>
                    <Figure
                ...
                {/* Never invent a figure to get off the phone. A number a
                    customer takes to a landlord or a board and then has
                    withdrawn costs more than the job was worth. */}
                <p className={styles.gateFoot}>
                  <Link to="/lanes">
                    Every offer, and what this brand publishes for it
                  </Link>
                  .
                </p>
              </div>,
          ...
          <section className={styles.notable} aria-labelledby="notable-h">
            <div className={styles.notableHead}>
              ...
              <h2 className={styles.h2} id="notable-h">
                When the cheapest two prices turn out to be partners
              </h2>
            ...
            {cardFor(
              ...
              <div className={styles.gate} data-variant="notable">
                ...
                <p className={styles.gateText}>
                  The brand publishing the fifty seven dollar drain calls
                  itself a proud partner of the brand publishing the seventy
                  seven dollar one, on its own About page. Volunteering that
                  turns a demand to match into a conversation about what the
                  price actually includes.
                </p>
                <p className={styles.gateFoot}>
                  If the cheapest possible clear is what they want, tell them
                  where it is and keep the household.{" "}
