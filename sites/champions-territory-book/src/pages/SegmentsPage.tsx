 ...
 *   "Develop marketing plans/strategies: You will be responsible for
 *    crafting tailored marketing plans that cater to the unique needs of
 *    the brand as well as the various service lines."
 *
 * The service line board already answers WHICH LINE the work belongs to.
 * This screen answers WHICH SEGMENTS ARE WORTH THE NEXT QUARTER OF
 * BUDGET, which is a different document written in a different month,
 * and the only version of that answer worth anything is one a reader can
 * argue with.
 *
 * ── WHAT THIS CUT IS AND IS NOT ───────────────────────────────────
 * This is the commercial and light industrial half of the territory,
 * plus the referral surfaces that feed the residential half. The
 * residential cut a home services marketer actually spends against is
 * not an industry at all: it is the age of the housing stock and, for
 * multifamily, the door count. That argument is made in the lede rather
 * than left implicit, because leaving it out would let seventeen federal
 * sector codes pose as the whole segmentation, and it is not encoded in
 * the ranking because there is no housing-stock field on a row to rank
 * it by. Saying that is better than pretending otherwise.
 *
 ...
 * cost of the donut is the whole reason this page exists, because a
 * segmentation with no stated rule is an opinion with a number typed
 * next to it.
 *
 * So the ranking is a stated rule, its three inputs are counts off rows
 * a reader can click, and the rule's one judgement, the weighting, is a
 * CONTROL rather than a constant. Four presets re-rank the board live
 * and print their own numbers next to the answer they produce. A reader
 * who thinks certainty matters more than volume in the tail of a cooling
 * season does not have to take my word for the consequence; they press
 * "Peak season" and watch thirteen sectors move.
 *
 ...
 * weighting is `?weights=pre-opening` and the open row is `?open=61`,
...
type WeightsKey = "balanced" | "pre-opening" | "volume" | "reach";
...
const PRESETS: Record<WeightsKey, WeightPreset> = {
  balanced: {
    ...
    note: "The board's published weighting. Volume leads because doors are the closest thing to revenue that can be computed without inventing a price, and nobody in this market publishes a job price above a coupon.",
  },
  "pre-opening": {
    key: "pre-opening",
    label: "Peak season",
    ...
    certainty: 0.6,
    reach: 0.2,
    note: "In the tail of a cooling season the only demand worth a budget is the demand that arrives on its own. A compressor fails in August. A planned panel upgrade is a decision a household can simply postpone until spring.",
  },
  volume: {
    key: "volume",
    label: "Volume",
    ...
    certainty: 0.1,
    reach: 0.1,
    note: "Chase the doors. Everything is volume, and a hard sector with four hundred doors in it beats an easy one with forty.",
  },
  reach: {
    key: "reach",
    label: "Reach",
    ...
    certainty: 0.2,
    reach: 0.6,
    note: "One manager, four counties, and an hour in the van is the scarce resource. Sectors you can write to win; sectors that have to be driven to lose.",
  },
};

const PRESET_ORDER: WeightsKey[] = ["balanced", "pre-opening", "volume", "reach"];
...
const COMPONENTS: ComponentSpec[] = [
  {
    ...
    glyph: "▦",
    cssVar: "var(--info)",
    basis: "Doors in play as a share of the largest sector on the board.",
  ...
  },
  {
    ...
    glyph: "◷",
    cssVar: "var(--ok)",
    basis: "The share of the sector sitting in service lines where the call happens whether or not anybody advertises.",
...
const REACH_SPLIT: {
...
}[] = [
  {
    ...
    note: "Read off their own page. A message reaches a named inbox.",
  },
  {
    key: "formOnly",
    glyph: "◐",
  ...
  },
  {
    ...
    glyph: "○",
    label: "Go-see only",
    note: "No written door at all. This one costs an hour in the van.",
  },
];
 ...
 * doors, then label, because two orderings of the same board that
    ...
    >
      ...
      <div className={styles.inner}>
        <header className={styles.head}>
          ...
          <blockquote className={styles.posting}>
            <p className={styles.postingText}>
              "Develop marketing plans/strategies: You will be responsible for
              crafting tailored marketing plans that cater to the unique needs
              of the brand as well as the various service lines."
            </p>
            <footer className={styles.postingFoot}>
              One line of the Champions Group Marketing Manager posting for the
              West Division, Brea. This screen is the answer to it.
            </footer>
          </blockquote>
          <p className={styles.lede}>
            ...
            nobody can argue with is a ranking nobody should trust. This is the
            commercial and light industrial half of the territory plus the
            referral surfaces that feed the other half: property managers and
            multifamily owners by door count, HOAs and community associations,
            builders and agents. The residential half is not an industry at
            all, and the cut that matters there is the age of the housing
            stock. A 1960s tract in north Orange County is a replacement
            market, where the argument is a failing system, financing and
            whatever rebate is still live. A 2015 build is a maintenance
            market, where the argument is a membership plan. The same offer
            sent to both wastes half the spend, and no field on these rows
            carries the year a house was built, so this board cannot rank that
            cut and does not pretend to.
          </p>
          <p className={styles.provNote}>
            <ProvenanceBadge provenance="modeled" />
            <span>
              Every score and every door figure on this screen is computed
              from counts. Organisation counts are counts. Nothing here is a
              number somebody typed into a template.
            </span>
          </p>
        </header>
        ...
        {/* =========================================================
            ...
            leave with the half-the-board sentence and nothing else, so
        ...
        <section className={styles.answer} aria-labelledby="answer-h">
          ...
          <h2 className={styles.h2} id="answer-h">
            Three sectors hold half the doors on the board
          </h2>
          ...
          <div className={styles.tiles}>
            ...
            <div className={styles.tile}>
              ...
              <span className={styles.tileLabel}>
                doors in play, summed from modelled midpoints
          ...
          <div className={styles.headline}>
            <p className={styles.headlineLabel}>
              What you would keep if the budget were cut in half
            </p>
            <p className={styles.headlineText}>
              ...
              <span className="num">{count(totals.seatsInPlay)}</span> doors on
              this board between them:{" "}
              <span className="num">{count(halfSeats)}</span> doors, in{" "}
        ...
        <section className={styles.ranking} aria-labelledby="ranking-h">
          <div className={styles.sectionHead}>
            <p className={styles.sectionEyebrow}>The ranking</p>
                ...
                >
                  <h3 className={styles.rankHeading}>
                    ...
                    >
                      ...
                      {/* Mark, word and number. Any two of the three can
                          fail, in greyscale, on a screen reader, or with a
                          glyph the font does not carry, and the row still
                      ...
                      <span className={styles.rankStat}>
                        ...
                        <span className={styles.rankStatWord}>doors</span>
                      </span>
                  ...
                  {open ? (
                    ...
                    >
                      <div className={styles.panelProse}>
                        <div className={styles.prose}>
                          <p className={styles.proseLabel}>
                            What makes them raise a call
                          </p>
                          <p className={styles.proseBody}>{row.occasion}</p>
                        </div>
                        <div className={styles.prose}>
                          <p className={styles.proseLabel}>
                            What marketing into this sector looks like
                          </p>
                      ...
                      <div className={styles.panelFacts}>
                        <div className={styles.factBlock}>
                          <p className={styles.factLabel}>
                            Service lines this sector occupies on this board
                          </p>
                          ...
                          <p className={styles.factNote}>
                            {row.lanes.length === 1
                              ...
                              : `${row.lanes.length} service lines, most common first. The way in changes inside the sector, which is why an industry is not a service line.`}
                          </p>
                        </div>
                        ...
                        <div className={styles.factBlock}>
                          ...
                          <p className={styles.factPair}>
                            ...
                            <span className={styles.factWord}>
                              median miles from the branch, straight line
                              <ProvenanceBadge provenance="modeled" compact />
                            </span>
                          </p>
                          <p className={styles.factPair}>
                      ...
                      <div className={styles.entries}>
                        <p className={styles.factLabel}>
                          The three largest accounts in the sector
                        </p>
        ...
        {/* =========================================================
            ...
            build already uses for a fact somebody withheld: the same
            withheld glyph, a dashed edge, no rank and no score. A reader
            who has seen a withheld membership price on the offers board
            recognises what this is before reading a word of it.
            ========================================================= */}
            ...
            <div className={styles.gap} key={row.id}>
              ...
              <p className={styles.gapOccasion}>
                <span className={styles.gapOccasionLabel}>
                  What would make one raise a call, when one is found
                </span>
                {row.occasion}
              </p>
        ...
        <section className={styles.execute} aria-labelledby="execute-h">
          <div className={styles.sectionHead}>
            ...
            <p className={styles.sectionLede}>
              What the manager does on Monday morning in each of them. Change
              the weighting above and these three change with it, which is the
                ...
                <li className={styles.moveCard} key={row.id}>
                  ...
                  <p className={styles.moveReachNote}>
                    {row.doorOnly > row.emailable
                      ? `More of this sector has to be driven to than written to, so Monday is a route sheet and not an inbox. Median ${row.medianMiles.toFixed(1)} miles out.`
                  ...
                  <p className={styles.moveEntryLabel}>
                    Start with the largest accounts
                  </p>
                  <ul className={styles.moveEntries}>
                    {row.entryPoints.map((entryPoint) => (
                      <li key={entryPoint.id}>
                        ...
                        <span className={styles.entryWord}>doors</span>
                      </li>
                    ))}
                  </ul>
                </li>
          ...
          <p className={styles.executeFoot}>
            The sector says which industries and why. The service line says
            what is being sold to the people in one.{" "}
            <Link className="tap" to="/lanes">
              The nine service lines and what runs into each
            </Link>
          </p>
        </section>

        <p className={styles.pageFoot}>
          Every count on this screen is counted off the rows. Every score and
          every door figure is computed from those counts by the formula
          printed above the ranking, with the weights printed beside the answer
          they produced. Nothing here is projected revenue and nothing here is
          a price.{" "}
          <Link className="tap" to="/method">
