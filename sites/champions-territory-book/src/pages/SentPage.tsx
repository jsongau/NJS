 ...
 * Five messages are seeded so the log has a pattern in it: four service
 * lines, two kinds of message, four different outcomes and only one of
 * them good, which is the honest ratio for cold outreach into a
 * territory with no history behind it. But a seeded log and a log the
 * reader wrote are
...
/*
  The sample link carries an offer and a door count because a bare quote
  link is a different page from a filled one, and the empty state is
  arguing that these rows have somewhere to land. The door count sits
  inside the modelled range on this organisation's own board row rather
  than being a figure anybody gave us, and it renders with an
  illustrative badge on both screens for that reason.
*/
const SAMPLE_PACKAGE_ID = "asi-rewards-1995";
      ...
      */}
      ...
      {message.guests || pkg || message.attachmentName ? (
        <div className={styles.quoteFacts}>
          ...
          {message.guests ? (
            <span className={styles.quoteFact}>
              <span className={styles.fieldLabel}>Doors discussed</span>
              <Figure
        ...
        <div className={styles.emptyCol}>
          ...
          <div className={styles.door}>
            ...
            <p className={styles.doorText}>
              The template forks on the service line and on the published
              campaign running to a printed expiry.
            </p>
          ...
          <div className={styles.door}>
            <p className={styles.doorHead}>
              ...
              <span>A request to hold the terms, from a group quote page</span>
            </p>
            <p className={styles.doorText}>
              The organisation-facing page. A request to hold the terms writes
              a row from the buyer's direction.
            </p>
            <p className={styles.doorLink}>
              <button
                type="button"
              ...
              <span className={styles.doorLinkNote}>
                Written against the{" "}
                {PACKAGE_BY_ID[SAMPLE_PACKAGE_ID]?.name ?? "school offer"} and
                the door count on their board row.{" "}
                <ProvenanceBadge provenance="illustrative" compact />
              </span>
            </p>
          </div>
