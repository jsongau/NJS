 ...
 * reading. Nothing left in the queue at four in the afternoon is the
 * best outcome the whole product has; the screen was reporting it in the
 * typography of a missing record.
 *
 * ── THE MARK IS A STRIKE, AND IT IS NOT DECORATION ────────────────
 * A bowling scoresheet marks a cleared rack with an X in a box. This
 * building is a bowling alley, the reader keeps score for a living, and
 * it is the one symbol in the domain that already means "everything
 * went down". So the mark is that box, drawn at forty four units with
 * the same two unit stroke language every other mark in this set uses,
 * and lit in the section's own colour.
 *
 ...
 *
 * COLOUR IS THE THIRD SIGNAL. The strike, the word and the figure all
 * say the same thing. Take the colour out and every reading survives.
 */
export function ClearedBoard({
  /** Two or three words. "Board cleared". "Nothing waiting". */
  headline,
...
/**
 * The strike box. A square with its corners eased and a heavy cross
 ...
 * as a stroke rather than as a fill because a stroked X at this weight
 * reads as something somebody wrote on the sheet, which is what it is.
 */
