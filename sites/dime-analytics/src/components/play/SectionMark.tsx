 ...
 * The solid shape is also the part that carries the meaning: the lit
 * plinth under the trophy, the pins at the end of the lane, the claimed
 * cell in the week, the coin on top of the stack. Read only the solid
 * shapes down the rail and the product still makes sense.
 *
 * ── THE SAME PRIMITIVE FAMILY AS EVERYTHING ELSE ──────────────────
 * PinMark draws a regulation pin from its own path constants, LaneChip
 * encodes the occasion class in the SHAPE of its cap, PackageGlyph draws
 * its families rather than importing them. Nothing in this application
...
const MARKS: Record<SectionId, Mark> = {
  ...
  /* Outside the building. A flag planted somewhere nobody has been. */
  field: {
  ...
  /* The only recurring product the building sells, and the only mark in
     the set that is a prize. The plinth is lit because a forming league
     is a thing being built up to. */
  /*
  ...
  */
  ...
  /*
    THE FLOOR. Three seats at a lane, the first one filled. The solid
    shape is deliberately the leftmost of three rather than a whole row,
    because two of the three seats are open and the mark should say that
  ...
  */
  ...
  /*
    RIVALS. Two pins standing on the same line, one of them ours. Not a
    crosshair and not a target: this register holds published facts about
    other venues, and a mark that reads as aiming at somebody would
    overstate what the screen actually does.
  */
  rivals: {
