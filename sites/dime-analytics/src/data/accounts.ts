 ...
 * DIME is an unaffiliated work sample, no promotion has been run out of
 * it and no group has been served, so a populated retention board would
 * be a claim about DIME's customers that nobody could check and that
 * DIME itself would know to be wrong.
 *
 ...
 * govern most of the Irvine trade area and they govern it in opposite
...
export const ACCOUNTS: Account[] = [
  {
    ...
    segmentBasis:
      "A comprehensive campus about half a mile from the Irvine office, buying its own programme events out of its own activities office. Not grad night, which is a different clock: a committee that turns over every year and a date settled a year out.",
 ...
 * record should carry. DIME publishes no price for any package, so
