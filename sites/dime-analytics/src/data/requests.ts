 ...
 * desk in the Irvine trade area, so the mix is weighted the way that
 ...
 * illustrative rather than public, because DIME publishes the contents
 ...
 * inventing a fact about DIME, which is the one thing this application
 * never does.
 *
 ...
 *   price. DIME publishes none. That is not a gap in the research, it
 ...
 * Irvine" and not an invented club. A made-up organisation in a real
 * trade area is a claim about somebody who exists, and there are real
 * youth sports clubs in Irvine.
 *
...
export const SEED_REQUESTS: GroupRequest[] = [
  ...
  },
  {
    ...
    channel: "phone",
    prospectId: null,
    organisationName: "Youth sports club, Irvine",
  ...
  },
  {
    ...
    channel: "party-page",
    prospectId: null,
    organisationName: "Insurance brokerage, Irvine",
    contactRole: "Office manager",
    lane: "auto-finance",
    email: "office-manager@demo.invalid",
    phone: null,
    note: "Holiday party for the office and a few partner agents. Picked the nearest location on the list and asked whether the corporate office in Irvine handles this or whether it all goes through the store.",
    askSummary:
  ...
  },
  {
    ...
    channel: "go-see",
    prospectId: null,
    organisationName: "Boba franchise unit, Los Irvine Center",
    contactRole: "Store manager",
    lane: "local-retail-food",
    email: "store-team@demo.invalid",
    phone: null,
  ...
  // -------------------------------------------------------------
  {
    ...
    agreedNextStepAt: null,
    closeReason:
      "Signed on a per head figure this desk set. DIME publishes no price for any package, so the number is an illustrative one and the book line says so.",
  ...
  },
  {
    ...
    phone: null,
    note: "Wanted a staff night and needed an all in figure to put in front of the partners at their meeting on the Monday. DIME publishes no price for any package, so nothing could be put in writing inside their week.",
 ...
 * attached. Nothing here is a DIME product, a DIME term or a DIME
 ...
 * party. DIME publishes no league programme on any page this
 ...
 *
 * WHAT IS NOT IN THIS FILE: a DIME league, a DIME league price, and
...
export const OPEN_LANE_SOCIALS: LeagueProgramme = {
  ...
  unpublished: [
    {
      field: "Price",
      note: "No dollar amount appears anywhere on the leagues page. Bowlero and Lucky Strike withhold league pricing in the same way and both route the question to an enquiry form, so this is a category-wide withholding rather than something particular to one operator. DIME publishes no league at all, so it has no price to withhold.",
      provenance: "withheld",
    },
    {
      field: "Season length",
 ...
 * show what a competitor publishes and what DIME does not, and carry
...
export const SEED_LEAGUE_INTEREST: LeagueInterest[] = [
  {
    ...
    standingAnswer:
      "The programme they found belongs to a different operator, plays Tuesday to Thursday, and publishes no price, no season length and no team size. DIME publishes no league at any location. There is nothing to sign up to, and the interest is being recorded rather than answered with a maybe.",
    answeredAt: null,
  ...
  },
  {
    ...
    answerable: "not-published-here",
    standingAnswer:
      "A brand-wide midweek programme exists in this category and it belongs to somebody else. What can be offered today is the published detail of that one, the plain statement that DIME publishes no league, and a note of the interest. Not a place in something that does not exist.",
    answeredAt: null,
    provenance: { request: "illustrative", programme: "public" },
  },
  {
    ...
    answerable: "not-published-here",
    standingAnswer:
      "Answered on 15 September with what one operator publishes, what DIME publishes, and the plain statement that no league price is published anywhere. Recorded as standing midweek demand for whenever something midweek is announced.",
