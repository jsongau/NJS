 ...
 * transactional book, and a transactional book is the book a new desk is
 * most tempted to build, because each line closes fast.
 *
 ...
 * ---------------------------------------------------------------
 *
 * THIS LEAGUE HAS NEVER BEEN RUN. Not a frame of it has been bowled. So
 ...
 * ---------------------------------------------------------------
 *
 * ROUND1 PUBLISHES NO LEAGUE AT ALL. Across the corporate profile, the
 * party booking page, the party room page and the nearest store's own
 * page there is no programme name, no night, no season length, no team
 * size and no price. A competitor does publish a brand wide league
 * programme, and that is recorded as the competitor's in
 * `data/requests.ts`, never as this operator's.
 *
 * So the two leagues in `data/leagues.ts` are a PROPOSAL, not an
 * announcement, and every field on them is illustrative. The price is
 * the one figure that could not be anything else: no league price is
 * published anywhere, so it renders as the withheld sentence and never
 * as a number.
 *
 * ---------------------------------------------------------------
 ...
 * matches is sixteen lanes, not eight. Whether sixteen lanes is most of
 * the house or a corner of it is a question nobody outside the store can
 * answer, because no lane count is published for any location.
 *
 ...
 * a team asked for under a standing offer in `venue.ts`, which holds a
 * place without a deposit until the team either confirms or lets it go.
 * It is real enough to keep somebody else out and it is not
...
export type SlotState =
  ...
  | "confirmed"
  /** Place held under a standing offer. No deposit, and it can lapse. */
  | "held"
...
export interface League {
  ...
  seasonWeeks: number;
  /** Bowlers per team. No team size is published, so this is ours. */
  teamSize: number;
