 ...
 * reserved address, and the leagues use handles. A team screen is the
 * one place where the temptation to invent three people is almost
 * irresistible, because three names with three photographs would look
 * like a product and would cost nothing to type.
 *
 * It would also be the only fact on the site a reader cannot check, and
 * it would be sitting on the screen that argues the floor is run
 * honestly. So a rep is a SEAT: an id, a published job title, an
 ...
 * One seat is filled and there is no team behind it. A screen showing
 * three staffed desks would be a mock-up. A screen showing which seats
 * are empty, what each empty seat was going to carry, and what is not
 * being worked while it stays empty is the honest picture, and it is the
 * only one that lets a reader judge what a single seat can reach.
 *
 * ── TITLES COME FROM THE PUBLISHED POSTING, LIKE EVERY OTHER TITLE ─
 * `Prospect.decisionMakerTitle` carries only titles found on published
 * staff directories. A seat carries only the grades printed in the
 * posting this work sample was written for, with the source on the row.
 * Manager and Senior Manager are both in its heading. Two seats share
 * the junior title, which is exactly why a seat has an ordinal: two
 * identical titles have to be distinguishable without anybody being
 * given a name.
 */
...
export interface Seat {
  ...
  seatNumber: number;
  /** A grade printed in the published posting. Nothing else. */
  title: string;
  titleSource: string;
  state: SeatState;
  ...
  /**
   * Territory by lane, not by geography.
   *
   * The trade area is four miles across, so splitting it by postcode
   * would hand two people the same street and two different halves of
   * the same conversation. The whole model is lanes: a lane decides the
   * motion, the door, the buyer class and the month, so a lane is the
   * only unit of territory that carries a way of working with it.
   */
  lanes: Lane[];
