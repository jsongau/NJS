 ...
 * knowable from here, since DIME publishes no lane count anywhere, so
...
export interface CupNightView {
   ...
   * published floor count. DIME publishes no bowling lane count for
...
export function taleOfTheTape(
  ...
  const rows: TapeRow[] = [
    ...
    row(
      ...
      "none",
      false,
      `Will be ${HANDICAP_FACTOR_PCT} per cent of ${HANDICAP_BASIS} once there are averages to compute one from. The basis is this proposal's own and DIME publishes no handicap system.`,
    ),
    row(
      "form",
      "Form, last five",
