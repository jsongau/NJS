 ...
 * ---------------------------------------------------------------
 *
 * Nothing has been bowled, so there are no scores, and `ladderFor` ranks
 * on three visible keys and no hidden ones:
 *
 ...
 *      under a standing offer, which is real enough to keep somebody
...
export interface BoardTotals {
  ...
  laneNights: number;
  /**
   * The busiest single night, as a share of the house, WHERE THAT SHARE
   * CAN BE CALCULATED AT ALL. It cannot, so this is always null.
   *
   * The fork this was built from divided by a published floor count.
   * DIME publishes no lane count for any location, so the denominator
   * does not exist and a percentage without
   * one is an invention with a per cent sign after it. Null rather than
   * zero, because zero is a reading and this is the absence of one, and
   * every surface that shows it has to render the withheld sentence
   * instead of a number.
   */
  peakNightLaneSharePct: null;
...
export interface LeagueView {
  ...
  /**
   * Share of the house this league holds on its night, which is null
   * because DIME publishes no lane count to take a share of. See
   * `BoardTotals.peakNightLaneSharePct` for the whole argument.
   */
  laneSharePct: null;
}
