...
export function mapBoardStats(totals: MapBoardTotals): MapBoardStat[] {
  return [
    ...
    },
    {
      ...
      provenance: "modeled",
      note: "Straight line, not drive time. Measured from the Irvine office by this app's own haversine, so it is modeled rather than published.",
    },
    /*
...
export function mapSummarySentence(totals: MapBoardTotals): string {
  return (
    `${totals.plotted} organisations plotted around the Irvine corporate office, which is not a store. ` +
