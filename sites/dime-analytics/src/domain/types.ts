 ...
 * territory: THE PRICE IS NOT ON THE PAGE.
 *
 * dimeindustries.com publishes one named group offering, the All Inclusive
 * Party, itemises what is in it, and publishes no price for it, no room
 * capacity, no hourly rate, no minimum spend and no lane count for any
 * location. The nearest store trades seven days a week and none of those
 * figures appears on its page either.
 *
 * That is not a smaller version of a sales job. It is a different one.
 * A catalogue with prices on it is sold by a website; a catalogue with
 * one row and no price is sold by a person, and every conversation
 * starts with the question the site refuses to answer. The posting this
 * work sample was written for describes that person's other half too:
 * sourcing promotional product, negotiating terms with vendors and
 * licensors, and reporting sell through back to them.
 *
 ...
 * In a quarter where nothing has signed yet, activity is the only thing
 * there is to report, which is exactly when the temptation to report it
 * AS progress is strongest. Two ledgers, two types, and no function that
 * takes one can be handed the other.
 *
...
export type Provenance =
  ...
  | "user_input"
  /**
   * The fact exists and the operator deliberately does not publish it.
   *
   * THIS VALUE IS NOT A GAP IN THE RESEARCH. It is a finding.
   *
   * DIME publishes the contents of its one party package and no price
   * for it, no room capacity, no hourly rate, no minimum spend, no
   * booking notice and no lane count anywhere. The booking page says to
   * contact the venue. The withheld price is the reason the role exists,
   * so recording it as "unknown" would throw away the most useful thing
   * the research found.
   *
   * A withheld figure renders as the sentence "not published" rather
   * than as a number. Never as an estimate wearing a number's clothes.
   */
  | "withheld";
...
/**
 * The one site this app is anchored on, and its trading state.
 *
 * `openingStatus` is a type rather than a boolean because the three
 * states are commercially different, and the outreach templates branch
 * on it. This operator is "open": it has traded in the United States
 * since 2010 and the nearest store keeps published hours every day of
 * the week, so there is no opening to sell against and no place in line
 * to offer.
 */
export type OpeningStatus =
  /** Announced, with an address and no date. Kept for completeness. */
  | "announced"
  /** A public opening date exists. Deposits become askable. */
  | "date-set"
  /** Trading, with published hours. Where this operator actually is. */
  | "open";
...
export interface Venue {
  ...
  /** Every attraction the operator publishes, chain wide or per store. */
...
export interface VenueAttraction {
  id: string;
  label: string;
  /** Published on a named store's own page, not merely chain wide. */
...
export type PackageFamily =
  ...
  | "buyout"
  /** Fundraisers. A community door-opener before it is a sale. */
  | "fundraiser";
 ...
 * A published package.
 *
 * `pricePerGuest` is `null` wherever the operator does not publish one,
 * and the provenance on that field is "withheld". There is no fallback
 * estimate anywhere. An invented price on a page a hiring manager can
 * check against dimeindustries.com is the fastest way to lose the room.
 */
export interface EventPackage {
  id: string;
  name: string;
  family: PackageFamily;
  /** What the guest actually gets, in the operator's own published terms. */
  inclusions: string[];
  ...
  /**
   * Bowling lanes consumed per 20 guests, where the operator publishes a
   * ratio. DIME publishes none, so this is absent on its one package
   * and any capacity arithmetic that wants it has to say whose rule it
   * is borrowing.
   */
...
export type PitchStatus =
  /** Never contacted about this package. The default everywhere. */
  | "unworked"
 ...
 *
 * A desk starting a territory from nothing has no system of record to
 * read from, and a live feed would be a lie. Naming the source is more
 * credible than faking the feed.
 */
 ...
 * there are two ledgers is that a young pipeline report is where
...
/**
 * A standing offer. What you can put on the table when the price itself
 * is not yours to publish.
 *
 * Note what is NOT here: a discount off a price the operator has never
 * published. You cannot discount a secret. The leverage available here
 * is PRIORITY and CERTAINTY, a date held in writing, a rate held across
 * a fiscal boundary, a quote back the same week, all of which cost
 * nothing in cash and are worth something because the alternative is a
...
export interface Offer {
  ...
  name: string;
  what: string;
  /** Why this is credible from an operator that publishes no price. */
  rationale: string;
  eligibleLanes: Lane[];
