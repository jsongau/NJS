 ...
 * a per head price in every cell, a green tick where DIME is cheaper.
 * That grid cannot be built. Lucky Strike Fullerton, Lucky Strike
 * Orange, DIME, Dave and Buster's Orange, The Phoenix Club and La
 ...
 * The register in `data/objections.ts` sorts into three classes and the
 * sort is the finding. Most of them are the operator's own shape: no
 * published price, a competitor that publishes what this one does not,
 * and no store at the address the buyer is being written to from. Two
 * are somebody else's calendar: a three year hotel contract, a budget
 * that does not open until next fiscal year. One names a competitor,
 * and the register's own recommended answer is not to argue with the
 * incumbent at all.
 *
 ...
 * and a brokerage that booked in July before anybody here had asked
 * them. None of them to a bowling house. None of them to a price.
 *
 * That changes what a rep does on Monday, which is the test this
 * screen was built against. "Dave and Buster's exists and it is up the
 * road" changes nothing. "Almost every objection is structural and not
 * one loss on record went to a rival's price, so the play is the
 * calendar" changes the whole approach: find out when the incumbent
 * contract ends, take the second occasion nobody is defending, and get
 * into the conversation before July rather than after it.
 *
...
> = {
  ...
  "same-parent": {
    ...
    glyph: "◈",
    cssVar: "var(--info)",
    note: "Two rows in this register that are one company. Buyers name both and are naming one operator twice.",
  },
  "banquet-room": {
      ...
      <div className={styles.inner}>
          ...
          {named ? (
            <div className={styles.punchline}>
              <p className={styles.punchHead}>
                And the one that names a competitor is not answered by
                arguing with it
              </p>
              <p className={styles.punchBody}>
                {named.short}. The buyer chose that night themselves and has
                enjoyed it for years, so the argument against it cannot be
                won and does not need to be had. The register's own
                recommended answer is to sell a different evening and then
                ask which second occasion is not already spoken for.
        ...
        <section className={styles.register} aria-labelledby="register-h">
          ...
          {/*
            ...
            That component prints "DIME does not publish this",
            which is correct on every other screen in this application
            and wrong here. The figure missing from this section is not
            DIME's, it is six other companies', and putting DIME's
            name over it would misattribute the silence.
          ...
          */}
          <div className={styles.noPrice}>
            ...
            <p className={styles.noPriceBody}>
              Every venue below was opened and read on its own stated date
              and not one carries a per head group figure. National chains, a
              regional operator and local venues, and all of them route to a
              form, a planner or a telephone. One of them publishes a
              fundraiser percentage and two guest minimums and still puts no
              per head price on a page. It is the same gate DIME puts on its
              own group pricing and the same gate{" "}
          ...
          {/* The comp set boundary, drawn on the same rule the prospecting
              board runs on. */}
          <div className={styles.boundary}>
            <h3 className={styles.h3}>Where the boundary was drawn, and why it held</h3>
            <p className={styles.boundaryBody}>
              Main Event is the operator that publishes the most about group
              business, and no address for it in this trade area was read. It
              is a category competitor and not a trade area one, and it sits
              in the register for the contrast rather than as the nearest
              threat. That boundary is the same rule the prospecting board
              runs on:{" "}
              <span className="num">{EXCLUDED_FROM_BOARD.length}</span>{" "}
              organisations were researched, found real, and refused a pin
              because the geocoder could not place them or disagreed about
              the street.
            </p>
