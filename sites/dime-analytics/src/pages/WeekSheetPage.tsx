 ...
 * 1. WHO TO CALL ON, per activity. The plan names a corridor and a
 *    number of target conversations. Standing on that street, that is not
 *    a plan, it is a street. So the sheet
 ...
 *    every one of them every week is a register nobody reads twice.
 *
 * 3. WHAT I AM ASKING FOR, lane by lane, from the lane's own hardest
 *    problem and the offer in data/venue.ts that answers it. Where no
 *    price can be quoted the ask is a headcount and a date, not a
 *    signature, and a rep who has that sentence in front of them does not
 *    improvise one.
 *
 * Nothing on this page invents a DIME fact. The office block is the
 * published address and the published support number, the phone number
 * is labelled as a support line rather than as a desk, and every
 * organisation's address was read off a published page and geocoded
 * through the US Census Bureau on 17 August 2026.
 */
...
> = {
  "go-see": {
    ...
    rule: "Ranked by no published email first, then by priority, then by how close they are to the Irvine office. An organisation with a front desk and no inbox can be reached this way and no other.",
    wants: "door",
  },
  tabling: {
 ...
 * fewest lanes. DIME publishes very little that can be offered without
 * anybody's approval, so every offer in data/venue.ts is eligible for
 * every lane and every lane currently gets the same one: the published
 * contents of the All Inclusive Party, and the published notice period
 * for changing a booking. The rule stays because it is the right rule
 * the moment a narrower offer exists.
 */
          ...
          <article className={styles.sheet}>
            <div className={`${styles.masthead} avoid-break`}>
              ...
              {/*
                ...
                asked "what does it cost" at a school reception and guesses
                has created a problem no discount fixes. DIME publishes no
                price, so the sheet says so in words, every week, in the
                place the eye lands first.
              */}
              <p className={styles.noDate}>
                <span aria-hidden="true">▲</span>
                <span>
                  DIME publishes no price for any group package, no room
                  capacity and no lane count. If you are asked today, the true
                  answer is that the number comes from the venue, and the ask
                  is a headcount and a date so a written quote can follow this
                  week.
                </span>
              </p>
            </div>
            ...
            */}
            ...
            <section className={styles.section}>
              ...
              <p className={styles.sectionNote}>
                The most specific offer each lane is eligible for.
              </p>
            ...
            <p className="print-only print-disclaimer">
              DIME. An independent work sample by Nathan J. Song, built for a
              DIME promotion planner application. Not affiliated with,
              endorsed by or connected to DIME Industries, LLC
              Addresses, the published support number and package contents
              were read from published sources on 17 August 2026; hours,
              target conversations and the run lists on this sheet are modeled
              and are labelled as such. Demo build, no message ever leaves the
              browser.
            </p>
          </article>
        ) : null}
        ...
        {week ? (
          <div className={`${styles.method} no-print`}>
            ...
            <ul className={styles.methodList}>
              ...
              <li>
                Addresses read off published pages and geocoded by the US
                Census Bureau, 17 August 2026.
              </li>
              <li>
