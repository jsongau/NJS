 ...
 * GAP. DIME publishes the contents of the All Inclusive Party and no
...
const SEEDED: SentMessage[] = [
  {
    ...
    body: "You asked what a group of about eighty gets. DIME publishes the contents of the All Inclusive Party and I have set them out in full: arcade time-play, bowling with shoe rental, karaoke or a party room, billiards and ping pong, pizza and soda, and a group photo, with a VIP Immersive Lane available at a separate fee. What DIME does not publish anywhere is the price, for this or any package, and I would rather say that plainly than send you a range I would have to correct later. Changes to a booking need three or more days notice, which is worth knowing before a date is set rather than after.",
  ...
  },
  {
    ...
    prospectName: "Irvine High School",
    lane: "schools",
    to: DEMO_RECIPIENT,
...
function nextReference(): string {
  seq += 1;
  return `DIME-DEMO-${String(seq).padStart(4, "0")}`;
}
...
function reviveMessage(raw: unknown): SentMessage | null {
  ...
  return {
    ...
    reference: str(raw.reference) ?? "DIME-DEMO-RESTORED",
  };
}
...
const outboxCodec: SliceCodec<OutboxState> = {
  ...
  decode: (raw, seed) => {
    ...
    /*
      ...
      quote carries DIME-DEMO-0006 for the second time, and two different
    ...
    */
    for (const m of sent) {
      const n = Number.parseInt(m.reference.replace("DIME-DEMO-", ""), 10);
