 ...
 * took the rail, the strip and the lane filter off the screen and left
 * the browser's back button as the only way home. A rep who wanted to
 * check what a school would see lost the board they were working.
 *
 * The route is right and the controls were wrong. A prospect opening a
 * link from an email should get a bare letter; a rep looking at the same
 * letter is looking at a preview, and a preview belongs over the work
 ...
 * gives at length. `?quote=cerritos-high-school` over any route opens
 ...
 * because closing it is what going back means. The headcount and the
 * package ride along beside it, because a quote for 380 guests on the
 * all access pass is a different document from the bare one and the
 * preview has to be the one the rep would actually send.
 *
...
export interface QuotePreviewOptions {
  ...
  packageId?: string;
  /** The headcount discussed. Omitted means the modeled midpoint. */
  guests?: number;
}
...
export function QuotePreviewProvider({ children }: { children: ReactNode }) {
   ...
   * the active element is a control inside the dialog. A rep who opened
   * the preview from the fortieth row of a list belongs back on the
   * fortieth row.
   */
    ...
    <Ctx.Provider value={value}>
      ...
      {openId ? (
        /* Keyed by id so opening a second organisation's quote while the
           ...
           organisation's headcount left in another organisation's
