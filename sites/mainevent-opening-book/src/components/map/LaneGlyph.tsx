 ...
 *   VenueGlyph     the broken ring. Main Event Brea is not open.
 ...
 * The venue, drawn.
 *
 * The ring is broken because the building is not open. There is an
 * address, a phone number and no published opening date, so a solid pin
 * would be the drawing making a claim the company has not made.
 */
export function VenueGlyph({
  ...
  name: string;
  /** The words under the ring. "Not open yet" unless a caller insists. */
  label?: string;
  size?: number;
  decorative?: boolean;
  className?: string;
