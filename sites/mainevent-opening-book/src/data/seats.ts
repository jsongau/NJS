 ...
 * Both titles are published in the Brea posting itself. Its heading is
 * "Sales Manager" and its opening line is "THE EVENT SALES MANAGER:
 * drives event revenue through consultative selling, outbound
 * prospecting, and retaining an existing client base". A parallel Dave
 * and Buster's listing carries the same two sentences. No title on this
 * floor was invented, exactly as no buyer title anywhere in
 * `prospects.ts` was invented.
 *
 ...
 *           auto and finance, hospitality and civic. Main Event publishes
 *           no price for any of it, so the number comes from a person and
 *           that person is the manager. It is also the seat that carries
 *           the referral lane, which converts on a tour rather than on a
 *           call and cannot be handed to somebody in week one.
 *
 ...
 * such wherever they are shown. They are not a claim about how Main
 * Event staffs a floor.
 */

const POSTING =
  "The Brea posting, read from the uploaded Indeed PDF on 14 August 2026, and repeated at builtin.com/job/sales-manager/4062748";
...
export const SEATS: Seat[] = [
  {
    ...
    state: "filled",
    /*
      The first day of the first pre-opening period in `venue.ts`, which
      is 16 weeks out. The book's own header says two contracts twelve
      weeks out is "roughly where a venue gets to once one person has
      worked the trade area for a few weeks", and this is those weeks.
    */
    startedOn: "2026-08-17",
    lanes: ["corporate", "auto-finance", "hospitality-civic"],
    lanesBecause:
      "Every lane here is discretionary, which means there is no event until somebody decides there is one, and Main Event publishes no price for any of the packages they buy. Both halves of that are the manager's job rather than a new starter's.",
    brief:
