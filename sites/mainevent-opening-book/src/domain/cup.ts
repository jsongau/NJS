 ...
 * and finish with two teams on two lanes of a published twenty six.
 *
 * ---------------------------------------------------------------
 * NOBODY HAS BOWLED A COMPETITIVE FRAME AND THE TYPES SAY SO
 * ---------------------------------------------------------------
 *
 * Main Event Brea has not opened. There is therefore no average, no
 * handicap and no won lost record anywhere in this application, and
 ...
 * ---------------------------------------------------------------
 *
 * Main Event publishes no league price and no cup price, and the withheld
 ...
 * it is never presented as a Main Event price. It is also not added to
...
export interface Cup {
   ...
   *
   * ILLUSTRATIVE, ALWAYS, AND NEVER A MAIN EVENT PRICE. Main Event
   * publishes no dollar amount for a league anywhere, and neither Bowlero
   * nor Lucky Strike does either; all three route the question to a form.
   * That makes the missing figure a category habit rather than a gap in
   * the research, so this number is the venue's own, invented for this
   * prototype, and it is not a claim about how Main Event operates.
   *
...
export interface CupEntry {
  ...
  entryNumber: number;
  /** Registered outright, or holding a place under the pre-opening offer. */
