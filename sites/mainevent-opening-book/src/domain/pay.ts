 ...
 *
 * ── WHAT MAIN EVENT ACTUALLY PUBLISHES ────────────────────────────
 * Two phrases and a band. "Competitive salary plus sales commission
 * potential", "Quarterly bonus program", and $75,662.29 to $89,014.40 a
 * year. That is the complete set. No commission rate, no quota, no
 * threshold, no accelerator and no bonus mechanic is published by Main
 * Event or by Dave and Buster's anywhere the research could reach, and
 * the self reported aggregates on the salary sites report pay levels
 * rather than plan design, so none of them is quoted here as a plan.
 *
...
export const BASE_BAND = {
  ...
  high: 89014.4,
  /**
   * The midpoint, rounded to the cent at definition rather than at
   * display. The exact half of the band carries half a cent, and a base
   * that is displayed rounded and computed unrounded produces a quota
   * a reader cannot reproduce with a calculator.
   */
  mid: Math.round(((75662.29 + 89014.4) / 2) * 100) / 100,
  source: "The Brea posting, read from the uploaded Indeed PDF on 14 August 2026",
 ...
 *
 * `PERIODS` in data/venue.ts is four four week pre-opening bands and
 ...
 * August to 6 December 2026, and that is one whole bonus quarter of
 * pre-opening work whose boundaries already exist in the data. A grouping
 * cannot disagree with the thing it groups.
 *
 * ── AND WHAT HAPPENS AFTER THE PERIODS RUN OUT ────────────────────
 * The pre-opening calendar stops at the opening, because a venue that has
 * published no opening date has certainly published no trading calendar.
 * A reader moving the scenario clock past 6 December is standing in a
 * quarter with no periods in it. Rather than invent periods, the
...
export interface Quarter {
  ...
  /** Last day, inclusive. */
  endDate: string;
  /** The periods this quarter groups. Empty past the pre-opening calendar. */
  periodIds: string[];
  /** True where the whole quarter falls before the modelled opening. */
  preOpening: boolean;
  /** Ordinal from the first pre-opening quarter, which is one. */
  ordinal: number;
}
 ...
 *
 * Main Event publishes no opening date. What it publishes is a page that
 * says "Want More Information About The Opening?", and what this
 * application publishes is a pre-opening calendar whose last period is
 * named "4 weeks out to open" and ends on 6 December 2026. The day after
 * that is the only opening date this model contains, and it is used for
 * exactly one decision: whether a quarter is pre-opening. It is not a
 * claim about when the building opens.
 */
