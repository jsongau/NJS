 ...
 * The venue has not opened. There is one Sales Manager and there is no
 * team. A screen showing three staffed desks would be a mock-up. A
 ...
 * the posting describes: "build and manage a high-performing sales
 * team". Building starts before anybody is hired.
 *
 ...
 * staff directories. A seat carries only titles found on published Main
 * Event or Dave and Buster's job listings, with the source on the row.
 * "Sales Manager" and "Event Sales Manager" are both in the Brea posting
 * itself. Two seats share the second title, which is exactly why a seat
 * has an ordinal: two identical titles have to be distinguishable
...
export interface Seat {
  ...
  seatNumber: number;
  /** Published on a Main Event or Dave and Buster's listing. Nothing else. */
  title: string;
  titleSource: string;
  state: SeatState;
