 ...
 * thirty two lanes against a published floor of twenty six.
 */
...
export interface CupNightView {
  ...
  matches: number;
  /** Share of the published lane floor this night takes. */
  laneSharePct: number;
    ...
    .map(([night, rounds]) => {
      ...
      return {
        ...
        laneSharePct: Math.round(
          (lanes / VENUE.bowlingLanesPublishedFloor) * 100,
        ),
        daysAway: daysUntil(
 ...
 *
 * "Five slots free of sixteen" is honest because the field is sixteen for
 * a reason that is a property of the building: twenty six lanes, two
 * lanes to a match, eight matches. The deadline is honest because it
...
export function taleOfTheTape(
  ...
  const rows: TapeRow[] = [
    ...
    row(
      ...
      "none",
      false,
      `Will be ${HANDICAP_FACTOR_PCT} per cent of ${HANDICAP_BASIS} once there are averages to compute one from. The basis is this proposal's own and Main Event publishes no handicap system.`,
    ),
    row(
      "form",
      "Form, last five",
...
export interface CupView {
  ...
  upsets: FixtureView[];
  /** Share of the published lane floor the cup takes on a cup night. */
  laneSharePct: number;
...
export function cupView(cup: Cup, asOf: string = CUP_AS_OF_DATE): CupView {
  ...
  return {
    ...
    upsets: upsetsIn(cup),
    laneSharePct: Math.round(
      (shape.peakLanes / VENUE.bowlingLanesPublishedFloor) * 100,
    ),
