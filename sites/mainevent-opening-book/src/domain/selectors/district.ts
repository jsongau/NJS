 ...
 * `PERIODS` in data/venue.ts is four four week pre opening bands. A
 * reader moving the scenario clock past 6 December is standing outside
 * all four of them, and the fix is the one `domain/pay.ts` already made
...
export function reportPeriodFor(asOf: string): ReportPeriod {
  ...
  if (seeded) {
    return {
      ...
      seeded: true,
      basis:
        "This band is one of the four pre opening bands in data/venue.ts.",
    };
  }
  ...
  return {
    ...
    weeksToOpen: 0,
    seeded: false,
    basis: `Past the pre opening calendar, which ends on ${prettyDate(last.endDate)}. The band keeps the same four week rhythm as the four seeded periods and holds none of them, so nothing is planned into it until somebody plans it.`,
  };
}
...
/**
 * THE RELEASE DATE IS DERIVED FROM MAIN EVENT'S OWN PUBLISHED TERMS.
 *
 ...
 * the published minimum booking notice: five days before the event is
 * the last day the hold can still convert into a booking on Main Event's
 * own terms, so it is the last honest day to be sitting on the date.
 */
...
export interface HeldDate {
  ...
  guests: number | null;
  lanes: number | null;
  /** Share of the published lane floor this hold would consume. */
  ...
  return statuses
    ...
    .map((s) => {
      ...
      return {
        ...
        guests,
        lanes,
        shareOfFloor: lanes ? lanes / VENUE.bowlingLanesPublishedFloor : null,
 ...
 *
 * Sixteen messages in `data/conversations.ts` carry an
 * `offerExtensionId`, and until now nothing in the application read one.
 * They are the record of an obligation made at venue level: a rate
 * honoured for a year, a Spirit Night on published terms, a hard hat
 * tour, a place in a queue. Those outlive whoever made them, and a
 * District Sales Manager who finds one at handover has been badly
 * served.
 *
...
export function exceptions(input: {
  ...
  if (stale.length > 0) {
    ...
    out.push({
      ...
      what: `${names}. Each was held against no deposit, and Main Event's published minimum booking notice of ${STANDARD_TERMS.bookingNoticeDays} days has already passed on every one of them, so none can now be signed on the published terms for the date it is holding.`,
      decision:
  ...
  /* 2. A single hold taking more than half the published lane floor. */
  ...
  for (const h of heavy) {
    out.push({
      ...
      title: `One hold takes ${Math.round((h.shareOfFloor ?? 0) * 100)}% of the published lane floor`,
      what: `${h.name} is holding ${h.targetDate} for ${h.guests} guests, which is ${h.lanes} of the ${VENUE.bowlingLanesPublishedFloor} lanes Main Event publishes as a floor for Brea, at their own published one lane per twenty guests. Nothing is signed against it.`,
      decision:
        "Confirm that a group this size may hold most of a building before an opening date is published, or set the size above which a hold needs the district.",
  ...
  for (const line of typed) {
    ...
    out.push({
      ...
      what: `${name} is signed at $${line.pricePerGuest.toFixed(2)} a guest for ${line.guests} guests. Main Event publishes no price for this package, so that figure came from this desk rather than from a rate card, and it is now the precedent for every group that asks the same question.`,
      decision:
      ...
      wantedByBecause: `The published minimum booking notice is ${STANDARD_TERMS.bookingNoticeDays} days, so this is the last day the contract can move before the group arrives.`,
      provenance: "user_input",
    });
  }

  /* 4. Obligations that outlive the pre opening calendar. */
  const inherited = commitments(period, asOf);
  const lock = inherited.rows.find((r) => r.offerId === "midweek-daytime-lock");
  if (lock) {
    out.push({
      id: "rate-lock",
      title: `A rate honoured for a year has been agreed with ${lock.extendedTo.length} ${lock.extendedTo.length === 1 ? "organisation" : "organisations"}`,
      what: `${lock.extendedTo.map((e) => e.name).join(", ")}. The offer is a midweek daytime rate agreed now and honoured for a year, made while there is no rate card to agree it against, and it binds the building after it opens.`,
      decision:
        "Honour these on opening or reprice them now, while every one of them is still a conversation rather than a contract.",
      wantedBy: MODELLED_OPENING,
      wantedByBecause:
        "The first day the venue could trade is the first day one of these is quoted back at us.",
  ...
  if (spirit) {
    out.push({
      id: "spirit-nights",
      title: `${spirit.extendedTo.length} Spirit Night ${spirit.extendedTo.length === 1 ? "night has" : "nights have"} been offered in the opening months`,
      what: `${spirit.extendedTo.map((e) => e.name).join(", ")}. Main Event's own published programme donates 20% of sales on the night, so each of these is a night of trading with a fifth of it committed before the doors open.`,
      decision:
        "Confirm how many Spirit Nights the opening months will carry, and on which nights of the week.",
      wantedBy: MODELLED_OPENING,
      wantedByBecause:
        "These land in the opening months, so the answer is needed before the opening calendar is set rather than after.",
      provenance: "public",
    });
  }
