...
/**
 * Bowling lanes that group would hold at Main Event's published rule of
 * one lane per twenty guests, against Brea's published floor of more
 * than twenty six lanes.
 *
 ...
 * denominator that means something, and Main Event publishes no target,
 * no capacity plan and no budget for Brea. Inventing one would be the
...
export function mapBoardStats(totals: MapBoardTotals): MapBoardStat[] {
  return [
    ...
    },
    {
      ...
      provenance: "modeled",
      note: "Straight line, not drive time. Measured from 245 W Birch Street by this app's own haversine, so it is modeled rather than published.",
    },
    /*
...
export function mapSummarySentence(totals: MapBoardTotals): string {
  return (
    `${totals.plotted} organisations plotted around Main Event Brea, which is not open. ` +
