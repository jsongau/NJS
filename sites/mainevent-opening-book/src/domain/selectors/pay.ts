...
export interface PayReading {
  ...
  /** Collected over quota. Null in a pre-opening quarter, which has none. */
...
export function payReading(input: {
  ...
  if (quarter.preOpening) {
    /* No revenue quota before there is a venue, so the quarter pays on
       the two leading gates, half each, all or nothing. */
