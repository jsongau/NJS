 ...
 * point: the register says Fairway Ford was lost to a three year hotel
 * contract, and a sceptical reader can go and find the message that
 * says so, in the buyer's own words, with a date on it. Nothing on
 * this screen asks to be believed.
 *
 ...
 *
 * ── THE ONE THING A SMALL FLOOR CANNOT FIX, MEASURED ANYWAY ───────
 * Win and loss practice says the seller should not collect the reason,
 * because a seller leading the interview hears what they want. A venue
 * with one manager has no independent interviewer and never will. So
 * the selector grades the evidence instead: a reason that arrived
 * inbound and unsummarised is the buyer's own words, and a reason
 * summarised after a phone call is the seat's recollection. Both are
 * kept, both are labelled, and the page never adds them together as
...
export function lossRows(): LossRow[] {
  ...
  for (const message of CONVERSATIONS) {
    ...
    /*
      ...
      genuinely different messages: the seat closed Fairway Ford as lost
      at eleven in the morning and the buyer's own sentence arrived at
    ...
    */
    ...
    /* The classification comes from the annotated note, which carries
       ...
       neither it is left as the venue's own gap rather than credited
...
export interface LossReading {
  total: number;
  /** Losses where the buyer named another venue at all. */
  namedAnyone: number;
  /** Losses to a bowling or arcade venue. The point of the reading. */
  toACategoryRival: number;
  /** Reasons that are on file in the buyer's own words. */
  buyerWroteIt: number;
  /** Reasons the seat wrote down afterwards. */
  seatWroteIt: number;
 ...
 * on record went to a bowling house or an arcade. One went to a hotel
 * contract signed three years ago, one to an organisation that owns
 * its own ballroom, and one to a decision made in July before this
 * venue had anybody to ask.
 */
...
/**
 * The seven objections sorted into what actually beats this venue.
 *
 ...
 *
 * Three states rather than two, because the two deadlines are
 * genuinely different and a rep needs to know which has passed. Once
 * the booking date is gone the code cannot be claimed, but every event
 * already claimed under it still runs, which is a month in which a
 * rival is delivering discounted parties and a rep asking about
 * December is competing with the memory of one.
 */
...
> = {
  live: {
    ...
    glyph: "●",
    cssVar: "var(--warn)",
    note: "The code can still be claimed on the date being read. A buyer weighing up two rooms has fifteen per cent in their pocket at one of them.",
  },
  "booking-closed": {
    label: "Closed to new bookings",
    glyph: "◑",
    cssVar: "var(--neutral)",
    note: "Too late to claim it, and the events booked under it are still being delivered. Worth knowing before asking anybody to compare quotes.",
  },
  expired: {
    label: "Expired",
    glyph: "○",
