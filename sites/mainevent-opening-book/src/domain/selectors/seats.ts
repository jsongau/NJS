...
/** Which kinds of work happen outside the building. Same list as the book. */
...
export interface SeatHours {
  ...
  planned: number;
  /** Of those, the ones outside the building. The posting's first line. */
  outside: number;
  /**
   ...
   * hours into a three hour evening.
   */
  intoLanes: number;
}
...
if (LANE_DRIFT.length > 0) {
  console.warn(`The floor and the lanes disagree. ${LANE_DRIFT.join(". ")}.`);
}
