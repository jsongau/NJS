 ...
 * The aria label is what this particular mark MEANS in context ("Brea
 * Olinda High School, schools and districts, calendar locked"), which is
 ...
 *
 * Main Event Brea is not open. A solid pin on a map says "here is a
 * business", which is the one thing this building is not yet, and a
 * reader who takes the venue for an operating site misreads every other
 * pin on the screen as a customer rather than as a prospect nobody has
 * spoken to.
 *
 * So the ring is dashed, the words "Not open yet" sit under it, and
 ...
 * second copy to keep in step, and the venue does not need it. What this
 * mark has to say is "not yet", which is geometry a circle can do.
 */
export function venueMarkScene(name: string, label = "Not open yet"): MarkScene {
  const w = 104;
  const cx = w / 2;
  const cy = 21;
  ...
  return {
    ...
    viewBox: w,
    viewBoxHeight: 60,
    title: `${name}, not open yet`,
    ariaLabel: `${name}, not open yet`,
    shapes: [
      {
        k: "circle",
        cx,
...
export function venueMarkerHtml(name: string, label = "Not open yet"): string {
