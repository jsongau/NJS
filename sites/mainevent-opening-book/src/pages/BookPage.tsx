 ...
 *
 * A pre-opening venue has no client base, no walk-in traffic and no CRM
 * history. For twelve weeks the only thing there is to report is work:
 * tables set up in office lobbies, mixers attended, front desks walked
 * into. That is precisely the situation in which activity gets quietly
 ...
 *
 * 1. HOW MUCH OF THE BOOK RESTS ON A PRICE SOMEBODY TYPED. Main Event
 *    publishes a price for every self-serve product and no price at all
 *    for any corporate or group package, so half of any real pre-opening
 *    book is quoted by a person rather than read off a page. Every
 ...
 *
 * Main Event publishes "a 50% deposit is required to reserve" on the All
 * Access Pass, Fun 101, Level Up, MVP and Happy Hour pages, and says
 * nothing about deposits on several others. A line whose package
 * publishes the figure carries it as public; a line whose package does
 * not carries it as modeled, against the standard terms, with the
 * substitution stated on the cell. Painting all of them public because
 * most of them are would be the kind of small overreach that a hiring
 * manager checks once and then stops trusting the rest of the page.
 */
...
/**
 * A headcount checked against what Main Event actually publishes.
 *
 * Both bounds are published fields on the package rows, so this is a
 * check rather than an opinion: Bowl 'n Fun is published as 12 to 50
 * guests, and a 60-guest booking against it is a promise the venue has
 * not said it can keep. It carries a glyph and a word, never a colour
 * alone.
 */
        ...
        */}
        ...
        <div className={styles.cell}>
          ...
          <dd
            ...
            title={`Main Event publishes one lane per ${GUESTS_PER_BOWLING_LANE} guests. Brea publishes more than 26 lanes, and every capacity figure in this app is computed against that floor.`}
          >
            <Figure
              value={
                line.lanesHeld === 0
                  ? "None held"
                  : `${line.lanesHeld} of 26`
              }
              provenance="modeled"
              compact
            />
      ...
      */}
      {gated ? (
        <div className={styles.gated}>
          <WithheldFigure
            reason={`Main Event publishes no price for ${pkg?.name ?? "this package"}, so there is no published figure to check this line against. The price above was quoted by a person, which is exactly what its entered badge means.`}
          />
        </div>
      ) : null}
          ...
          <nav className={styles.jump} aria-label="Related planning screens">
            ...
            <Link className={styles.jumpCard} to="/calendar">
              ...
              <span>
                ...
                <span className={styles.jumpNote}>
                  What the left column consumes. Brea publishes more than 26
                  lanes, and a 300-guest booking takes fifteen of them.
                </span>
              </span>
            </Link>
          </nav>
          ...
          >
            ...
            */}
            ...
            <section className={styles.typed} aria-labelledby="typed-h">
              ...
              {/* Main Event publishes no price for any corporate or group
                  package, so part of any pre-opening book is always a figure
                  a person quoted. This says which part. */}
            ...
            <p className={styles.colFoot}>
              ...
              Deposits at each line&apos;s own percentage. Main Event publishes
              a {STANDARD_TERMS.depositPercent}% deposit and a{" "}
              {STANDARD_TERMS.bookingNoticeDays}-day booking notice.
            </p>
          </section>
