 ...
 * renders with that stated every time it appears. Main Event publishes no
 * response time anywhere: not on the Brea page, not on the events pages,
 * not on the contact page. Main Event publishes a league programme brand
 * wide and names three locations, all of them in Colorado; nothing about
 * a Brea league is published, so a league ask on this page is answered
 * rather than sold.
 *
...
function contextOf(task: DerivedTask): { role: string; via: string; ask: string } | null {
  ...
  if (league) {
    return {
      ...
      via: "League ask",
      ask: `Asked about a league on ${league.preferredNights.join(" or ")}. Main Event publishes Open Lane Socials brand wide and names three locations, all in Colorado. Brea is not one of them.`,
    };
  }
  return null;
}
    ...
    const unansweredItems: RiskItem[] = [
      ...
      ...unansweredLeague.map((l) => ({
        ...
        detail: `${l.contactRole}. Asked about a league on ${l.preferredNights.join(
          " or ",
        )}, which Brea has not announced. The honest answer is still an answer.`,
    ...
    return [
      ...
      },
      {
        ...
        full: `${
          ...
          "date has",
          "dates have",
        )} no signature and no deposit against them. Main Event publishes a 50% deposit and a five day minimum notice, so a hold that is not converting is a date nobody else can be offered rather than a booking in waiting.`,
        empty:
        ...
        <section className={styles.weekSection} aria-labelledby="week-h">
          ...
          <div className={styles.weekGrid}>
            <div className={styles.weekCard}>
              ...
              <p className={styles.weekText}>
                ...
                <Figure
                  ...
                  compact
                />{" "}
                rests on a price somebody typed rather than one Main Event
                publishes.
              </p>
            </div>
            ...
            <div className={styles.weekCard}>
              ...
              {/* The Colorado fact is a published Main Event fact and it is the
                  reason a league ask is logged rather than sold, so it keeps a
                  home in the tooltip and in the accessibility tree. */}
              <p
                className={styles.weekText}
                title={
                  week.leagueArrived.length === 0
                    ? undefined
                    : "Main Event publishes Open Lane Socials brand wide and names three locations, all of them in Colorado. Brea is not one of them, so a league ask is recorded as midweek demand rather than sold."
                }
              >
                ...
                {week.leagueArrived.length === 0 ? null : (
                  <span className="visually-hidden">
                    {" "}
                    Main Event publishes Open Lane Socials brand wide and names
                    three locations, all of them in Colorado. Brea is not one of
                    them, so a league ask is recorded as midweek demand rather
                    than sold.
                  </span>
                )}
              </p>
            ...
            <div className={styles.weekCard}>
              ...
              {/* Main Event's own Brea form asks for no date, no headcount and
                  no event type. That published fact is why this count exists,
                  so it stays as the tooltip on the count itself. */}
              <p
                className={styles.weekText}
                title="Main Event's Brea enquiry form asks for a name, an email, a phone number, a company and 256 characters. It asks for no date, no headcount and no event type."
              >
                ...
                {(() => {
                  ...
                  return unqualified === 0 ? null : (
                    <>
                      ...
                      <span className="visually-hidden">
                        {" "}
                        Main Event's Brea enquiry form asks for a name, an
                        email, a phone number, a company and 256 characters. It
                        asks for no date, no headcount and no event type.
                      </span>
                    </>
                  );
                })()}
