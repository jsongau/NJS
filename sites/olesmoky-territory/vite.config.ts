...
/**
 * base is "/molsoncoors/" because the built output is committed into
 * njs-site/olesmoky/distribution/ and served from nathanjsong.com/olesmoky/distribution.
...
export default defineConfig({
  base: process.env.VITE_PREVIEW === "1" ? "./" : "/olesmoky/distribution/",
  plugins: [react()],
  resolve: {
    alias: {
  ...
  build: {
    ...
    rollupOptions: {
      output: {
         ...
         * land on the order desk and never open the map, so 90 KB of
