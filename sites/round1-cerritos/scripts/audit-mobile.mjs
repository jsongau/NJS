...
/**
 * The screens to audit, read out of the route emitter's own typed list.
 *
 * There is one reading of each screen in this application, so the audit
 * targets are exactly the pages the emitter writes a stub for, minus the
 * per-prospect and per-league stubs, which are one template each and are
 * covered by auditing a single example.
 *
 * It is parsed out of emit-route-stubs.mjs rather than typed again here,
 * because a list typed twice is a list that goes out of date in one
 * place and reports a clean audit over a page nobody looked at.
 */
function consolePaths() {
  const here = path.dirname(new URL(import.meta.url).pathname);
  const src = fs.readFileSync(path.join(here, "emit-route-stubs.mjs"), "utf8");
  const start = src.indexOf("const ROUTES = [");
  const end = src.indexOf("];", start);
  if (start === -1 || end === -1) {
    console.error("Could not find the ROUTES array in emit-route-stubs.mjs. Fix this before trusting an audit that skipped every screen.");
    process.exit(1);
  }
  const out = new Set(["/"]);
  for (const m of src.slice(start, end).matchAll(/^\s*"([a-z0-9/-]+)",$/gm)) {
    out.add(`/${m[1]}`);
  }
  if (out.size < 10) {
    console.error(`Only ${out.size} routes parsed. The shape changed; fix this before trusting the result.`);
    process.exit(1);
  }
  return [...out].sort();
}
