...
import { PERIODS, VENUE } from "@/data/venue";
 ...
 * has exactly one user, the promotion planner, and a switcher offering a view
    ...
    items: [
      {
        ...
        label: "Packages",
        sec: "packages",
        hint: "Every package Round1 publishes, and every price it does not",
      },
      {
        to: "/coaching",
        label: "Coaching",
...
export function SideRail() {
  ...
  /*
    THE RAIL POINTS AT ITSELF.

    This application ships one reading of each screen, so a destination
    is its own address and nothing rewrites it on the way out. The
    indirection is kept rather than inlined because every row in the rail
    already calls it, and a single function is the one place a future
    second reading would be added back.
  */
  const railHref = (to: string) => to;

  /**
   * WHICH SCREEN THE READER IS STANDING ON.
   *
   * Everything below that asks "which screen is open" reads this one
   * value rather than location.pathname raw, because normalisePath is
   * where a trailing slash and an index path are settled. sectionFor
   * solves the same problem the same way one file over, which is the
   * pattern to follow rather than a new one to invent.
   */
  const here = normalisePath(location.pathname);
  const consoleHere = here;
   ...
   * Round1 publishes no group price, no capacity plan and no budget, so a
   * percentage against one would be a number this app made up and then
   * drew a picture of. What is real and countable is how much of the
  ...
  const markLabel =
    `R1. The dial round the ball shows how far the trade ` +
    `area has been worked: ${worked} of ${PROSPECTS.length} organisations ` +
    `worked so far, ${workedPct} per cent.`;

  /* THE COUNTDOWN THAT USED TO STAND HERE HAS GONE. It counted weeks
     until a venue opened, which was the whole premise of the fork and is
     not a fact about Round1. Round One Entertainment has traded in the US
     since 2010 and the Cerritos office is open for business, so there is
     nothing to count towards and a large figure reading zero would have
     been urgency invented out of an empty field. The period select below
     is what remains, and quarters are what an office plans in anyway. */
      ...
      <div className={styles.head}>
        ...
        */}
        <div className={styles.brand}>
          ...
          */}
          <Link
            ...
            aria-label="R1, go to the desk"
          >
            <span className={styles.brandText}>
              <strong>R1</strong>
              <span className={styles.brandSub}>
                {VENUE.name}, 12900 Park Plaza Dr
              </span>
            </span>
          </Link>
        </div>
        ...
        {/*
          ...
          the one office it is about, and there is nothing to switch
          between because there is nothing else.
        */}
        <p className={styles.contextLabel}>
          New business development promotion planner, Cerritos
        </p>

        <div className={styles.context}>
          {/*
            WHAT USED TO SIT HERE WAS A COUNTDOWN, AND IT HAS GONE. The
            ...
            Round1: the Cerritos office is open, the chain has traded in
            ...
            forward quarters, which is what an office plans in.
          */}
                      ...
                      >
                        {subFilters.map((sub) => (
                          <li key={sub.id}>
                            {sub.to ? (
                              <Link
                                /* Through railHref like every other
                                   row, so the second level and the first
