...
import { DEMO_RECIPIENT, VENUE, OFFER_BY_ID } from "@/data/venue";
...
import { usePipelineDispatch } from "@/state/PipelineProvider";
...
> = {
  ...
  "reserve-party": {
    glyph: "◕",
    label: "Hold a date",
    what: "The date, the headcount and the published change notice.",
  },
  "league-enquiry": {
    glyph: "◇",
...
function ComposeDialog({
  ...
  */
  ...
  /*
    `weeksToOpen` used to travel in here and it has been taken out. The
    periods are ordinary forward quarters now, because Round1 Cerritos is
    an operating corporate office rather than a building waiting to open,
    so the field on `Period` is zero everywhere and a draft that read
    urgency out of it would be reading urgency out of nothing.
  */
  const ctx = useMemo(
    () => ({
  ...
  /*
    THERE IS NO LANE FLOOR TO COMPARE AGAINST ANY MORE. The fork read a
    published lane count off another operator's page and warned whenever
    a headcount went past it. Round1 publishes no lane count for any
    location, so the comparison has no right hand side and the warning
    has been replaced by the withheld sentence rather than by a guess.
  */
                ...
                <fieldset className={styles.fieldset}>
                  ...
                  <div className={styles.lanesBox}>
                    <p className={styles.lanesLead}>
                      ...
                      {lanesHeld === 1 ? "lane" : "lanes"}, at this
                    ...
                    <p className={styles.lanesWarn}>
                      <WithheldFigure reason={`How many lanes ${VENUE.name} has is not published, for this or any location, so nothing here can tell you whether this group fits in one sitting. Ask the store, and put the answer in the record rather than in a letter.`} />
                    </p>
                    {overPackageMax ? (
                      <p className={styles.lanesWarn}>
