 ...
 * carries `laneFit`, the answer to which kinds of buyer the package is
 * written for. Round1 publishes no restriction of that sort on the one
 * package it itemises, so the All Inclusive Party opens every lane and
 * the file says as much beside the field rather than inventing a rule
 * Round1 has not written. Where a package does narrow, that is the
 * narrowing, and it costs the reader nothing to check.
 *
