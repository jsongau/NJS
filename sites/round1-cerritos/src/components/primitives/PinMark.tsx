...
/**
 * THE MARK: a bowling ball in play, inside the dial that shows how much
 * of the trade area has been worked.
 *
 ...
 *
 * ── AND WHY A BALL SAYS MORE THAN A PIN DOES ──────────────────────
 * A Round1 store is bowling, an arcade, karaoke, billiards, darts and
 * ping pong. A pin is the object that has already been knocked over by
 * somebody else. A ball
 * is the object a person picks up, and at this size a ball with a ring
 * round it is also, exactly, an arcade button: a lit bezel with a face in
 * the middle of it waiting to be hit. The three holes are what settle
 * which of the two you are looking at, and they are the reason both
 * readings are available at once rather than one being a stretch. That is
 * the business in one shape, and it is a shape nobody owns. This
 * application is unaffiliated: there is no Round1 wordmark here, no
 * lockup, no sampled brand colour and no red.
 *
 ...
 * rather than sitting next to it. The one solid shape is the ball. The
 * dial is drawn at line weight, and the arc over it is the only other
 * ink in the file.
 *
 * ── THE HOLES ARE CUT, NOT PAINTED ────────────────────────────────
 * The ball and its three holes are ONE path with an even odd fill, so the
 ...
 * the fraction counts, through `title`. A mark that encodes a number
 ...
 * "some of it" becomes "just past the half". The ball is light on the
 ...
 * Two other files import this name and one of them is not this agent's to
 * edit. Renaming an export across an ownership boundary is an API change
...
/**
 * The ball, and the three holes drilled in it.
 *
 * The radius leaves three and a half units of air inside the dial, which
 * is what stops the two shapes fusing into one disc at twenty four
 * pixels. The three holes are the two fingers and thumb a drilled ball
 * actually carries, and the whole grip is set high and to the left rather
 * than centred. Centred, the two round holes over a third read as a face,
 * which is the one thing this mark must not be. Off centre it reads as
 * drilling, and drilling is the only detail that separates a ball from a
 * button at this size.
 */
const BALL_R = 5.9;
...
/** The ball and its drilling, in one path. See the even odd rule in the
 *  stylesheet for how the three subpaths become holes. */
const BALL_PATH = [
