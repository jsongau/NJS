 ...
 * console went with it: no strip, no rail, no lane filter, and the back
 * button as the only way home. This dialog is the fix. The letter is the
 * same letter, rendered from the same component the route renders, and
 * the board stays on screen behind it.
 *
 * ── THE TWO CONTROLS A PREVIEW ACTUALLY NEEDS ─────────────────────
 * Copy the link, and open the real page in a new tab. Nothing else. The
 * first is what a rep does ninety times out of a hundred: the URL goes
 * into a message they are already writing. The second is for the tenth,
 * where they want to stand exactly where the activities director stands,
 * with no dialog around it. The link is shown as well as copied, because
 * a copy button that reports success and copied nothing is a lie a rep
 * only finds out about after they press send.
 *
 * ── WHY THE HOLD BUTTON IS HELD DOWN IN HERE ──────────────────────
 * `interactive={false}` reaches the document and disables one control:
 * the hold request. On the prospect's own page that press writes a row
 * saying the organisation asked for a date. Pressed from inside a
 * preview it would write the same row, and an outbox carrying requests
 * nobody made is worse than no preview at all. The calculator stays live
 * because trying a headcount writes nothing.
 *
...
export interface QuotePreviewModalProps {
  ...
  /** The headcount quoted, or null for the modeled midpoint. */
  guests?: number | null;
  onClose: () => void;
}
