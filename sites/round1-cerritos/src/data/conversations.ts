...
const WORKED: ConversationMessage[] = [
  ...
  // campus behind it, and it took a fortnight to get an answer.
  {
    ...
    subject: "District contact form",
    body: "The district office is about a mile from Park Plaza Drive, which is close enough that this should be a walk rather than a form, and the form is still the only published route. Asked who approves a partner visiting campuses and when the calendar for staff and student events is set.",
    summarised: false,
    effect: {
  ...
  // Cerritos College. Not one customer, hundreds of student
  ...
  },
  {
    ...
    body: "Both, in writing, as asked. The table is straightforward. The prize stock is the part worth doing properly: quantities set to the bracket size rather than to a round number, artwork approved in writing before anything is decorated, and a delivery date that sits a fortnight before the tournament rather than the week of it. On the group side, Round1 publishes what the All Inclusive Party includes and publishes no price, so the contents are yours to circulate and the figure comes from the venue.",
    summarised: false,
    effect: {
  ...
  // Los Cerritos Center. A promotion site before it is an account, and
...
const INBOUND_LED: ConversationMessage[] = [
  ...
  },
  {
    ...
    body: "Half an hour later, because a caller who has already picked a month is not browsing. Three December weeknights that would take a group that size are listed below in the order I would pick them. On cost, the honest answer rather than the comfortable one: Round1 publishes what the All Inclusive Party contains and publishes no price for it, and the page says to contact the venue. So I can tell you today exactly what is included, and the figure has to come from the venue against your headcount and the evening you take.",
    summarised: false,
    effect: {
  ...
  // Premier Workspaces Cerritos Tower. Asked for a price, was told the
...
const SCHOOLS_COLD: ConversationMessage[] = [
  // Cerritos High School. Half a mile away, and they wrote to us.
  {
    id: "msg-cerritoshs-1",
  ...
  },
  {
    ...
    body: "One thing before the slips print. Changes to a booking need three or more days notice, which Round1 publishes, and a school calendar moves more than most. Building that into the date on the slip now costs nothing and saves a conversation in November.",
    summarised: false,
    effect: {
...
const EMPLOYERS: ConversationMessage[] = [
  ...
  // InBody USA. One touch, one chase, nothing back.
  {
    ...
    counterpartyRole: "Office manager",
    subject: "Head office enquiry",
    body: "A US head office under two miles from Park Plaza Drive. Asked about staff events in the fourth quarter and who buys the branded stock for them.",
    summarised: false,
    effect: {
      note: "First touch.",
      movedStatusTo: "reached-out",
...
const FIELD: ConversationMessage[] = [
  ...
  // holds the tournament crowd.
  {
    ...
    counterpartyRole: "Store manager",
    body: "Under a mile from Park Plaza Drive, so this was a walk rather than a drive. Asked what nights the tournaments run, how many people come, and what the shop buys for prizes.",
    summarised: true,
    effect: {
...
const LOCAL: ConversationMessage[] = [
  // Cerritos Library. A third of a mile away, and the programme already
  // exists.
  {
  ...
  // City of Cerritos City Hall. The permit route, and silence.
  {
 ...
 * that catalogue to point at, because Round1 publishes exactly two
