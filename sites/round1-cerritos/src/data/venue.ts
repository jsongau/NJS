...
/** Round One Entertainment, Inc. corporate profile. The address, the entity, the chain-wide attraction list. */
const PROFILE_PAGE = "https://www.round1usa.com/profile";
/** The party booking page. Package contents, no price, the change notice. */
const PARTY_BOOKING_PAGE = "https://www.round1usa.com/book-a-party";
/** The party room activity page. A support number and nothing else useful. */
const PARTY_ROOM_PAGE = "https://www.round1usa.com/activities-list/partyroom";
/** The nearest Round1 store to the office, and the closest thing to a venue this app has. */
const LAKEWOOD_PAGE =
  "https://www.round1usa.com/locations/lakewood-center-mall";

/**
 * THIS RECORD IS AN OFFICE, NOT A VENUE, AND EVERY SCREEN DOWNSTREAM HAS
 * TO BE READ THAT WAY.
 *
 * 12900 Park Plaza Drive, Suite 200 is Round One Entertainment, Inc.'s US
 * corporate headquarters in Cerritos. It is where the role reports, which
 ...
 * and no page on round1usa.com claims there is.
 *
 * SO WHAT ARE THE ATTRACTIONS DOING IN AN OFFICE RECORD. They are what
 * Round1 publishes about its VENUES, chain-wide, on its corporate profile
 ...
 *
 * The published number below is Round1's customer support line. It is not
...
export interface HeadOfficeVenue
   ...
   * Event's own Brea page. Round1 publishes no lane count for any
   ...
   *
   * A wrong lane count in front of a Round1 reader is the single most
...
export const VENUE: HeadOfficeVenue = {
  ...
  postalCode: "90703",
  phone: "855-772-6636",
  phoneLabel: "Round1's published customer support line, not an office line",
  /**
  ...
  /**
   * The office is operating and so is the chain. Round One Entertainment,
   ...
   */
  openingStatus: "open",
  /** Round1 publishes no lane count, anywhere. See the field's own note. */
  bowlingLanesPublished: null,
  attractions: [
    {
      id: "bowling",
    ...
    },
    {
      ...
      label: "Spo-cha",
      storeSpecific: false,
      note: "Published as available at select locations only. Round1 does not publish which, so it is never promised to a group.",
      source: PROFILE_PAGE,
      provenance: "public",
    },
    {
...
/**
 * The nearest Round1 store to the office, as Round1 publishes it.
 *
 ...
 * filling any one of them in would be an invention a Round1 reader could
 ...
 * opened. Round1 Cerritos is an operating corporate office and the chain
...
/**
 * What can honestly be put on the table, which is what Round1 publishes
 * and nothing else.
 *
 ...
 * Event circumstance, and carrying any of them across to Round1 would
 * have been an invention dressed as continuity.
 *
 * What is left is short and it is checkable. Round1 publishes the
...
export const OFFERS: Offer[] = [
  {
    ...
    what: "Round1's published party package: arcade time-play, bowling with shoe rental, karaoke or a party room, billiards and ping pong, pizza and soda, and a group photo. A VIP Immersive Lane can be added at a separate fee. Round1 publishes no price for any of it and the page says to contact the venue.",
    rationale:
      "It is the only package Round1 names and itemises in public, so the contents can be quoted to a buyer today without anybody's approval. The price cannot, and saying so plainly is stronger than a range, because the reader can check both halves of that sentence on round1usa.com in fifteen seconds.",
    /* Every lane. The package is described by contents rather than by
       ...
       it, and inventing one would be a rule Round1 has not written. */
    eligibleLanes: [
      "schools",
      "colleges",
      "corporate",
    ...
    */
    ...
    costToVenue: 0,
    costNote:
      "Zero, because nothing is conceded. This offer quotes what Round1 already publishes and concedes no discount off a price that was never published in the first place.",
    provenance: "public",
  },
  {
    id: "change-notice-three-days",
    name: "Three days notice to change a booking",
    what: "Round1 publishes that changes to a booked party need three or more days notice. Spo-cha parties and parties including other amenities are sold separately, and Round1 publishes no further detail about either.",
    rationale:
      "It is the one booking term Round1 states in public, which makes it the one term a rep can commit to without checking. Saying it early also does the buyer a favour, because a group that finds out about a notice period two days before the night has found out too late.",
    eligibleLanes: [
      "schools",
      "colleges",
      "corporate",
...
/**
 * Round1's own published contact surfaces, which are worth stating
