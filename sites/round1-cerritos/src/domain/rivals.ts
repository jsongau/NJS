 ...
 * because almost nobody in the category publishes a group price. Lucky
 * Strike routes event enquiries to a planner, Dave and Buster's routes
 * them to a planner, and this operator's own party page publishes the
 * contents of one package and no figure. `data/leagues.ts` records the
 ...
 * page is a moving target in a way this operator's own withheld pricing
 * is not. A promotion expires. A location rebrands. A lane count is
...
export type RivalStanding =
  ...
  | "category-only"
  /** Two rows in this register, one company. Named so nobody sells
   *  against a brand that is already the operator next to it. */
  | "same-parent"
...
export type LossCause =
  /**
   * The operator's own silence or geography. No published price, and a
   * store one town over from the office. Nobody else is winning these;
   * they are being lost on their own.
   */
  | "our-own-gap"
  /**
...
> = {
  "our-own-gap": {
    ...
    glyph: "▲",
    cssVar: "var(--risk)",
    note: "These are lost to ourselves. No published price, and a store that is not in the town the office is in. A competitor is not involved and beating one would not fix either of them.",
  },
  "their-calendar": {
  ...
  "a-named-competitor": {
    ...
    glyph: "◆",
    cssVar: "var(--info)",
    note: "The buyer named another operator. Two rows in the register do this, and neither recommended answer competes on brand.",
  },
};
