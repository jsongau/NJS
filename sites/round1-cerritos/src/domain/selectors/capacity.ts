 ...
 * denominator. Round1 publishes no bowling lane count for any location
 ...
 * Round1 lane-hold by it would produce a percentage that looks
 * calculated, reads as authoritative and is about nothing at all, and a
 * Round1 reader would be entitled to assume every other figure in this
...
export const NO_LANE_COUNT_REASON =
  "Round1 publishes no bowling lane count for any location, including Lakewood Center, the nearest store to this office. There is no house total to divide by, so this is a size and not a share. The figure comes from the store, not from a page.";
 ...
 * fact Round1 keeps to itself, and only the first half can be done off
 * a web page.
 */
 ...
 * have put an exact capacity for somebody else's building on a Round1
 ...
 * guest maximum. Round1's one package publishes neither, so this returns
      ...
      return {
        ...
        note:
          lanesAtMax === null
            ...
            : share === null
              ? `At its published maximum this package holds ${lanesAtMax} ${lanesAtMax === 1 ? "lane" : "lanes"}. What share of the house that is cannot be said, because Round1 publishes no lane count for any location.`
              : share > 0.5
