 ...
 * spends hours outside the office, which is the only genuinely scarce
 * thing a desk working a cold territory has.
 *
 ...
 * already sold is a route sheet nobody uses twice.
 *
 ...
 * quarter of a mile the main retail corridor chains into one group of
 ...
 * front desk: a reception to find, a manager to ask for, a card to leave,
 * and the walk between. A cap is also what stops the greedy pass turning
 * the densest corridor into a single group nobody can finish.
 */
 ...
 * end pinned to 12900 Park Plaza Drive, and every distance FROM THE OFFICE
 * on this board still comes from there rather than from here. This
...
export interface GoSeeRun {
  ...
  walkMiles: number;
  /** How far the first stop is from the building. */
  fromVenueMiles: number;
  /** Sum of the midpoints of every modeled headcount range in the run. */
  guests: number;
 ...
 *
 * Start at whichever stop is closest to the building, because that is the
 * one a rep reaches first coming out of Park Plaza Drive, then take the
...
export function goSeeRuns(
  ...
  for (const seed of pool) {
    ...
    runs.push({
      ...
      guests: near.reduce((n, row) => n + headcountMidpoint(row.prospect), 0),
...
export function runSentence(run: GoSeeRun): string {
  return (
    ...
    `nearest ${run.fromVenueMiles.toFixed(1)} miles from the building. ` +
