 ...
 * are counted in plain days, and they are ALL ILLUSTRATIVE. Round1
 * publishes no service level of any kind, so nothing here is a claim
 * about how Round1 operates. They are this desk's own intervals.
 *
  ...
  switch (r.status) {
    ...
    case "qualifying": {
      ...
      return {
        ...
        action: `Send the quote and name a date. ${r.headcount ? `${r.headcount} guests is ${lanesForGuests(r.headcount)} lanes at this desk's own planning rate of one per twenty.` : ""}`.trim(),
      };
    }
    ...
    case "held":
      return {
        ...
        action: `Convert it or release it. No deposit and no minimum booking notice is published, so nothing external will close this for you, and a hold that is not converted is a date lost to everybody.`,
      };
...
function reasonsFor(
  ...
  if (r.headcount !== null) {
    ...
    out.push({
      ...
      points,
      why: `${r.headcount} guests, which is ${lanesForGuests(r.headcount)} lanes at this desk's own planning rate of one per twenty. This is their number rather than a modeled range, which makes it firmer than the headcounts on the outbound desk and still not a contract.`,
    });
  } else {
    out.push({
      label: "Size unknown",
  ...
  if (r.desiredDate) {
    ...
    if (daysOut <= STANDARD_TERMS.changeNoticeDays) {
      out.push({
        label: "Inside the published notice period",
        points: 18,
        why: `Their date is ${daysOut} days from when they wrote, and the only published timing term is that changes need ${STANDARD_TERMS.changeNoticeDays} or more days notice. Answer slowly and nothing about the booking can be altered before they arrive.`,
      });
    }
  }
  ...
  if (unasked.length > 0) {
    out.push({
      ...
      why: `The ${REQUEST_CHANNEL_META[r.channel].label.toLowerCase()} does not ask for the ${unasked
        ...
        .join(" or the ")}. That gap is the form's rather than this sender's haste, and it is recovered by a phone call every single time.`,
    });
  }
...
export function derivedTasks(
  ...
  for (const l of leagueInterest) {
    ...
    tasks.push({
      ...
      status: null,
      because:
        "Asked about a league at an operator that publishes none. There is no programme name, no night, no season length and no price on any page read.",
      action:
        "Answer with the plain fact that no league is published, and treat the ask as what it is, a bowling group with names attached. Record the interest as evidence of midweek demand.",
      dueAt,
      hoursLate,
      score: reasons.reduce((n, c) => n + c.points, 0),
      reasons,
 ...
 * The finding, counted.
 *
 * The inbound routes in `domain/requests.ts` do not all ask the same
 * questions, and one of them asks for no date, no headcount and no event
 * type at all. This function counts what that difference actually costs
 * across a live queue, and separates the fields a sender withheld
