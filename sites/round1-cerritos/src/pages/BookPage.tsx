 ...
 *
 * 1. HOW MUCH OF THE BOOK RESTS ON A PRICE SOMEBODY TYPED. Round1
 ...
 *
 * Round1 publishes no deposit percentage on any page, so no line here
...
/**
 * A headcount checked against what Round1 actually publishes.
 *
 * Both bounds are published fields on the package rows, so this is a
 * check rather than an opinion. Round1 publishes neither bound for the
        ...
        */}
        ...
        <div className={styles.cell}>
          ...
          <dd
            ...
            title={`One lane per ${GUESTS_PER_BOWLING_LANE} guests is this app's own planning ratio. Round1 publishes no lane count for any location, so this says what the group needs and never what is left.`}
          >
            <Figure
              value={
                line.lanesHeld === 0
      ...
      */}
      {gated ? (
        <div className={styles.gated}>
          <WithheldFigure
            reason={`Round1 publishes no price for ${pkg?.name ?? "this package"}, so there is no published figure to check this line against. The price above was quoted by a person, which is exactly what its entered badge means.`}
          />
        </div>
      ) : null}
          ...
          >
            ...
            */}
            ...
            <section className={styles.typed} aria-labelledby="typed-h">
              ...
              {/* Round1 publishes no price for any group package, so every
            ...
            <p className={styles.colFoot}>
              ...
              Deposits at each line&apos;s own percentage. Round1 publishes no
