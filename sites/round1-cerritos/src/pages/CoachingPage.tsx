 ...
 * Round1 publishes the contents of one party package in this file and
 * publishes no price for it. Nothing here sells itself off a phone at
 * eleven at night, because there is no number on the page to sell it
 * with. A rep who spends week one memorising inclusions has learned the
 * part of the job the website already does for free.
 *
...
/** A 300 guest party, in bowling lanes, at this app's own planning ratio. */
...
const RAMP: RampStep[] = [
  {
    ...
    what: (
      <>
        Sort the range into the packages Round1 publishes a price for and
        ...
        round1usa.com. A priced package is one a parent buys alone at night on
        a phone. Every gated one sends the reader to the venue instead. That
        is the whole job, published by the company, in its own words.
  ...
  },
  {
    ...
    /*
      HALF OF THIS STEP HAS GONE, AND THE MISSING HALF IS THE LESSON.

      It used to read: one lane per twenty guests, more than twenty six
      lanes in the house, therefore a 300 guest booking is over half the
      building. The first and third clauses still hold. The second was a
      different operator's figure for a different building, and Round1
      publishes no lane count for any location, so a rep can still work
      out what a group needs and cannot work out what is left. That is
      worse for the rep and it is what is true, and a training step that
      taught the comfortable version would be teaching somebody to
      promise an evening on a number nobody has published.
    */
    what: (
      <>
        A bowling lane holds about{" "}
        <strong className="num">{GUESTS_PER_BOWLING_LANE}</strong> guests, so a
        300 guest party needs{" "}
        <strong className="num">{LANES_FOR_THREE_HUNDRED}</strong> lanes. How
        many lanes the house has, and therefore whether two of those on one
        December Friday is most of the venue or a quiet corner of it, cannot
        be worked out here.
        <span className={styles.inlineProv}>
          <ProvenanceBadge provenance="modeled" compact />
        </span>
        <WithheldFigure reason="Round1 publishes no bowling lane count for any location, including Lakewood Center, the nearest store to this office. A rep can size a group off the ratio and cannot check it against the house, so the lane count is the first thing to get from the store and write down." />
      </>
    ),
    why: "Fifth, and before a rep may hold anything, because this is the only mistake on the list that costs a customer rather than a call. A held date that cannot physically be delivered becomes a refund, an apology from a general manager, and a school that tells every other school in the district. It matters more here than it did at a venue that published its lane count, because the check a rep would run is not available and the discipline has to replace it.",
  },
  {
    ...
    what: (
      <>
        ...
        published price, a competitor who publishes what we do not, no store
        at this address, and a calendar already spoken for. They arrive in
        week one and they arrive from everybody.{" "}
        <Link to="/objections">The register</Link>.
      </>
    ),
    why: "Sixth, because a rep who meets \"your website will not tell me what it costs\" without a prepared answer will invent a figure. That is the most expensive sentence anybody in this office can say, and it gets said out of helpfulness rather than dishonesty, which is exactly why training has to get there before the buyer does.",
  },
  {
...
const CALL_FRAME: Record<OccasionClass, CallFrame> = {
  "calendar-locked": {
    ...
    closes:
      "Certainty about a date, offered earlier than the incumbent will offer one. With no published price to compete on, the currency is queue position, and to a buyer working backwards from a fixed date, queue position is the thing actually worth having.",
    failure:
  ...
  discretionary: {
    ...
    leadWith:
      "The corporate family, where Round1 publishes no price at all. So the first conversation is about fit and date, and the number arrives later, from a person, which is the entire reason the role exists.",
  },
};
...
const AGENDA: AgendaItem[] = [
  ...
  },
  {
    ...
    ask: "Give me the answer to \"you will not tell me what it costs\", and then tell me what that answer costs us.",
    bad: "A fluent answer with no cost attached. Every real answer to an objection about a figure nobody publishes gives something away, and a rep who cannot name what they are giving away will end up giving away the wrong thing.",
  },
  {
    minutes: 6,
      ...
      <div className={styles.inner}>
        <header className={styles.head}>
          ...
          <p className={styles.lede}>
            The job posting asks for a manager, at manager or senior manager
            level, who keeps supplier and licensor relationships strong while
            scouting new vendors. Every other screen here argues that the trade
            area has
        ...
        <section className={styles.section} aria-labelledby="ramp-h">
          ...
          <div className={styles.prose}>
            ...
            <p>
              ...
              <strong className="num">{GATED}</strong> behind a page that says
              to contact the venue. A priced package sells itself off a phone
              at eleven at night to somebody who has never spoken to anybody,
              and there is not one of those here.
              <span className={styles.inlineProv}>
                <ProvenanceBadge provenance="public" compact />
              </span>{" "}
            ...
            <div className={styles.ledger} data-ledger="booked-revenue">
              ...
              <ul className={styles.metrics}>
                <li className={styles.metric}>
                  ...
                  <p className={styles.metricNote}>
                    Two contracts in a quarter is roughly what a trade area
                    looks like once one person has worked it for a few weeks
                    from a standing start. Treating that as a performance
                ...
                <li className={styles.metric}>
                  ...
                  <p className={styles.metricNote}>
                    ...
                    price a person typed rather than a price Round1
                    published, and that share is the number a manager
        ...
        <p className={styles.foot}>
          ...
          application. The package counts and the one lane per twenty guests
          rule were read off round1usa.com. A lane count was not, because
          Round1 publishes none for any location, so no figure on this page is
          a share of the house. The figures in
