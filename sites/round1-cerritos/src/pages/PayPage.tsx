 ...
 * The posting publishes a salary band and nothing else about how the
 * money works. No rate, no quota, no threshold, no accelerator and no
 * mechanic, and Round1 publishes none of those anywhere either. So every
 * figure here is either that published band or a PERCENTAGE OF IT, and
 * every percentage carries the illustrative badge and the sentence that
 * goes with it. Nothing on this screen is a claim about how Round1 pays
 * anybody.
 *
 ...
 *   ?as-of=2026-09-23  default. The ramp quarter, day 38 of 112.
 ...
 *                      deposits.
 *   ?as-of=2026-11-21  still ramping, one day after the first event.
  ...
  return (
    <section className={styles.section} aria-labelledby="pay-quarter">
      ...
      >
        ...
        {reading.quarterDays}.{" "}
        {quarter.rampQuarter
          ? "It is the first quarter on the calendar, so there is no book behind it to measure attainment against, no revenue quota in it, and the bonus pays on the two leading gates instead."
          : "It is a measured quarter, so the revenue curve is live and the entry gate still has to be cleared before any of it pays."}
      </SectionHead>
      ...
      <div className={styles.readings}>
        ...
        <Reading
          ...
          provenance="illustrative"
          note={
            bonus.state.kind === "ramp"
              ? `The quarterly target is ${money(bonus.target)}, a quarter of ${PLAN.bonusPct}% of base. A ramp quarter pays half of it for each leading gate, and ${bonus.state.gatesMet} of ${bonus.state.gatesTotal} are met.`
      ...
      <p className={styles.para}>
        ...
        gate in the ramp quarter only, because it is standing in for a
        revenue curve there is no book to draw yet; once the desk has one,
        the second gate is the curve itself.
      </p>
      ...
      <ol className={styles.actions}>
        ...
        {!quarter.rampQuarter && !reading.contractFloorMet ? (
          <li className={styles.action}>
            <span className={styles.actionVerb}>
              Collect on{" "}
        ...
        {quarter.rampQuarter ? (
          <li className={styles.action}>
            ...
            Commission runs normally in the ramp quarter, at{" "}
    ...
    <section className={styles.section} aria-labelledby="pay-curve">
      ...
      {attainment === null ? (
        <p className={styles.callout}>
          <span aria-hidden="true">◇</span> There is no reading on this curve
          in the ramp quarter, because there is no book behind the desk to set
          a quota against. Asking for an attainment figure in the first twelve
          weeks of a territory produces a number that is wrong in whichever
          direction makes the next twenty minutes more comfortable, which is
          the argument{" "}
          <Link to="/coaching">the coaching page</Link> already makes about the
          weekly one to one. The revenue curve starts on{" "}
          {formatDate(RAMP_ENDS)}. Until then the gates above are the whole
          bonus and commission runs on anything that collects.
        </p>
      ) : (
        <>
          <ScaleBar
  ...
  return (
    <section className={styles.section} aria-labelledby="pay-plan">
      ...
      >
        ...
        left column is a claim about Round1.
      </SectionHead>
      ...
      <p className={styles.para}>
        ...
        seeded calendar span{" "}
        {quarter ? (
          <span className="num">
            {formatDate(quarter.startDate)} to {formatDate(quarter.endDate)}
          </span>
        ) : null}
        , which is one whole bonus quarter of work whose boundaries
      ...
      <p className={styles.para}>
        Past the seeded calendar the grouping has nothing to group, because
        Round1 publishes no trading calendar of any kind and this application
        invents none. Quarters after{" "}
        {formatDate(RAMP_ENDS)} keep the same rhythm, carry no periods, and say
        so where they are drawn. The ramp end is the day after the first
        quarter closes. It decides one thing only, which is whether a quarter
        is the ramp, and it is not a claim about anything the operator has
        published.
      </p>

      <h3 className={styles.h3}>The ramp quarter pays differently</h3>
      <p className={styles.para}>
        In the first quarter on the calendar the bonus pays on the
...
function PublishedBlock() {
  return (
    <section className={styles.section} aria-labelledby="pay-published">
      <SectionHead
        eyebrow="The part nobody can check, and it says so"
        title="What Round1 publishes about pay, which is nothing"
        id="pay-published"
      >
        One line, and this is the complete set.
      </SectionHead>

      <ul className={styles.published}>
        <li>
          <span className={styles.publishedLabel}>The band on this screen</span>
      ...
      <p className={styles.callout}>
        ...
        <strong>
          ...
          accelerator and no bonus mechanic is published by Round1 anywhere
          this research could reach.
        </strong>{" "}
        The salary sites carry self reported total compensation for titles like
        this one, but those report pay levels rather than plan design and they
        are unverifiable individual submissions, so none of them is quoted here
        as a plan. Every figure on this screen is this application's own
        proposal, badged illustrative, in the same words the league fee and the
        response commitment already use: this desk's own, invented for this
        prototype, and not a claim about how Round1 operates.
      </p>
    </section>
  );
}
