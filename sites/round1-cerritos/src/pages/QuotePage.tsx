 ...
 * conversation, this family is the reason the role exists". True,
 ...
 * top, that Round1 publishes no price for any of this. A prospect finds
...
/**
 * The offer to put in front of this organisation.
 *
 ...
 *
 * THE ORDER IN data/venue.ts IS DOING THE RANKING, and the first offer
 * in that file is the one that answers the objection nobody gets past,
 * which is that the price is not on the page. Sorting here as well would
 * be a second opinion about priority, living in a page, quietly
...
function LinkNotFound({ sheetTag: Sheet = "main", sheetClassName }: SheetShape) {
  return (
    <Sheet className={[styles.sheet, sheetClassName].filter(Boolean).join(" ")}>
      <p className={styles.eyebrow}>Round1, group events</p>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.body}>
          ...
          {VENUE.postalCode}. Round1 publishes {VENUE.phone} as its customer
          support line, which is not an office line.
        </p>
        <p className={styles.sourceLine}>
          <ProvenanceBadge provenance={VENUE.provenance} />
          <a
            className="tap"
            href={SOURCE_LINKS.profile}
            target="_blank"
            rel="noreferrer"
          >
  ...
  return (
    <footer className={styles.footer}>
      ...
      <p className={styles.disclaimer}>
        An independent work sample by Nathan J. Song, built for a Round1
        promotion planner application. Not affiliated with, endorsed by or
        connected to Round One Entertainment, Inc. No Round1 logo, wordmark or
        trade dress appears anywhere in it. Every published figure carries the
        page it was read from on 17 August 2026; everything else is labelled
        as modeled, illustrative or entered.
      </p>
    </footer>
  );
}
  ...
  function requestHold() {
    ...
    dispatch({
      ...
      message: {
        ...
        body: `${prospect.name} asked for a date to be held on the ${pkg.name} for ${guests} guests, which is ${lanesForGuests(guests)} bowling lanes at this desk's planning ratio of one lane per twenty guests. Round1 publishes no lane ratio and no lane count, so that figure sizes the ask and not the building. They asked off their own page. No deposit is taken and no price is quoted, because none is published; the hold converts or releases once a person has priced it. Their window is ${prospect.buyingWindow}.`,
        packageId: pkg.id,
        guests,
      },
    });
    ...
    <Sheet className={[styles.sheet, sheetClassName].filter(Boolean).join(" ")}>
      ...
      <section className={styles.section} aria-labelledby="venue-h">
        ...
        <div className={styles.notice}>
          <p className={styles.noticeHead}>
            <span aria-hidden="true">◔</span>
            <span>
              The price is not on any page, and you are hearing that here
              first
            </span>
          </p>
          <p className={styles.noticeBody}>
            {VENUE.address} in {VENUE.city} is the corporate office rather
            than the venue, and the nearest store is in the same trade area
            as you. Round1 publishes what is in the package, in full, and
            publishes no price for it, no room capacity and no lane count.
            You would find that out in one search, so it is said here first.
            It is also the reason this page is worth reading: nothing on it
            asks you for money, it asks for a headcount and a date so that a
            written quote can follow.
          </p>
          <p className={styles.sourceLine}>
            <ProvenanceBadge provenance={VENUE.provenance} />
            <a
            ...
            rel="noreferrer"
          >
              Round1's own page for the nearest store
            </a>
          </p>
        </div>

        <p className={styles.body}>
          Everything below is published by Round1 chain-wide, on its own
          corporate profile page. Spo-cha is published as available at
          select locations only and Round1 does not publish which, so it is
          never promised to a group.
        </p>
      ...
      {pkg ? (
        <section className={styles.section} aria-labelledby="pkg-h">
          ...
          <p className={styles.body}>
            This is what Round1 publishes as being in it. Not a summary of
            it, and nothing added to it.
          </p>
        ...
        <div className={styles.calc}>
            ...
            <div className={styles.calcRows}>
              ...
              <div className={styles.calcRow}>
                ...
                <span className={styles.calcRowNote}>
                  One lane for every {GUESTS_PER_BOWLING_LANE} guests is the
                  standard way a group is sized against lanes, and it is this
                  desk&apos;s planning ratio rather than a published one.
                  Round1 publishes no lane count for any location, so this
                  says how many lanes your group needs and not how many are
                  left.
                </span>
              </div>
                ...
                <span className={styles.calcRowValue}>
                  {pkg?.pricePerGuest != null ? (
                    ...
                    <WithheldFigure
                      reason={
                        <>
                          Round1&apos;s page for this package tells you to
                          contact the venue. So there is no number here and
                          there is not going to be an invented one. What that
                          person needs from you is below, and it is short.
                        </>
                      }
                    />
                  )}
              ...
              {total !== null ? (
                <div className={styles.calcRow} data-lead="true">
                  ...
                  <span className={styles.calcRowNote}>
                    Your number multiplied by the published per-guest price.
                    It carries "modeled" because the
          ...
          {belowMinimum && pkg?.minGuests != null ? (
            <p className={styles.flag}>
              <span aria-hidden="true">▲</span>
              <span>
                <strong>Under the published minimum.</strong> Round1
          ...
          {aboveMaximum && pkg?.maxGuests != null ? (
            <p className={styles.flag}>
              <span aria-hidden="true">▲</span>
              <span>
                <strong>Over the published maximum.</strong> Round1
      ...
      {/* -------------------------------------------------------
          THE OFFER. What can honestly be put on the table, which
          is what Round1 already publishes plus a date held while a
          person prices it. Both cost nothing. The internal
          reasoning about why that is good for the venue stays on
      ...
      {offer ? (
            ...
            {offer.costToVenue === 0 ? (
              <p className={styles.offerCost}>
                ...
                <span>
                  ...
                  date costs you nothing, and until somebody has priced it
                  there is nothing here for you to be committed to. What it
                  does buy is position: every date held is one nobody else can
                  take.
                  {pkg?.depositPercent != null ? (
                    <>
                      {" "}
            ...
            <p className={styles.sourceLine}>
              ...
              <span className={styles.sourceNote}>
                {offer.provenance === "public"
                  ? "The terms in this offer are published by Round1."
                  : "This offer was written for a work sample and is not a published Round1 programme."}
              </span>
            </p>
          </div>
        </section>
        ...
        <section className={styles.section} aria-labelledby="offer-h">
          ...
          <p className={styles.body}>
            ...
            tell us the headcount and the date you have in mind, and a
            written quote follows the same week. It costs nothing to ask.
          </p>
        </section>
      )}
        ...
        {confirmation ? (
          ...
          >
            ...
            <p className={styles.confirmBody}>
              ...
              has no way to send one. In a live version of this, the person
              who prices these would see the request against your name the
              same morning.
            </p>
          ...
          <>
            <p className={styles.body}>
              Ask us to hold a date. It takes one press, it costs nothing,
              and it commits you to nothing until somebody has priced it and
              you have said yes to the number.
            </p>
            <Button
              variant="primary"
              glyph="◕"
