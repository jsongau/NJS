 ...
 * inside.
 *
 * Inbound is the half that loses money without leaving a mark. An
 * enquiry arrives with a clock already running and the only way to fail
 * it is to do nothing at all. Nobody writes down "did not reply to the
 * youth pastor for four days". There is no report with that line on it.
 * The enquiry simply stops mattering to the person who sent it, and by
 * the time anyone looks, the group has booked with whoever answered.
 *
 ...
 * ROUND1 PUBLISHES NO RESPONSE TIME ANYWHERE. Not on the party booking
 * page, not on the party room page, not on the nearest store's page.
 * There is no service level to quote and this screen does not invent one
 * on their behalf. The four working hour commitment is this desk's own,
 * invented for this prototype, and it says so on screen in the same block
 * as the number
 * rather than in a footnote nobody scrolls to. A countdown that implied
 * a published service level would be exactly the kind of invented figure
 * this whole application exists to avoid.
 *
 * ── THE FINDING THIS SCREEN IS BUILT AROUND ────────────────────────
 * The published routes into a group booking do not ask for the same
 * things. One of them collects a name, an email, a phone number, a
 * company and a block of free text, and asks for no date, no headcount
 * and no event type. Another asks for all three. Every route's field set
 * is recorded on the route rather than assumed.
 *
 * That difference is not a design quibble; it is a commercial hole. A
 * venue cannot quote what its own form did not collect, and an enquiry
 * through the thinner route arrives missing all three of the answers
 * somebody needs before they can price anything. The gap
 * panel below counts what that costs across a live queue, and it keeps
 * "they left it blank" and "the form never asked" in separate columns,
 ...
 * two published forms rather than about the bucket on screen.
 *
 ...
 * A tournament. Round1 publishes no tournament programme anywhere:
 * no format, no entry fee, no bracket, no eligibility, no dates. A
 * tournament registration screen would be invented end to end, and an
 * invented product sitting beside real published packages is how a
 * reader stops believing the real ones. Leagues are here, because Open
 * Lane Socials is a real published brand-wide programme, and what is
 * shown about it is exactly what is published plus an explicit record of
 * what is not.
 */
...
function factsFor(
  ...
  /* What the reading actually consists of, in the vocabulary the rows
     use. A bucket of eight quotes to chase and a bucket of eight
     unanswered enquiries are the same digit and not the same morning.
...
const READING_EMPTY: Record<ReadingId, string> = {
  all: "No request is generating work.",
...
/**
 * A queue row, which is either an enquiry or a league ask.
 *
 * The league asks are on this table rather than in a sidebar for one
 ...
 * because one of them was a league ask filed somewhere else would break
 * the only promise this page makes. They carry no channel and no
...
export function RequestsPage() {
  ...
  const rows = useMemo<QueueRow[]>(() => {
    ...
    for (const l of SEED_LEAGUE_INTEREST) {
      ...
      out.push({
        ...
        channel: null,
        status: null,
        ask: `League ask, ${l.preferredNights.join(" or ")}`,
        haystack: [
          organisation,
          l.contactRole,
          l.note,
   ...
   * league asks without saying so is the same bug as a bucket that does
   * not add up, only quieter.
   */
  const excludedLeague = useMemo(() => {
    if (channel === "all" && status === "all") return 0;
    return rows.filter((r) => r.kind === "league").length;
  }, [rows, channel, status]);
  ...
  const exportCsv = useCallback(() => {
    const csv = toCsv(
      ...
      filtered.map((r) => [
        ...
        r.role,
        LANE_META[r.lane].label,
        r.channel ? REQUEST_CHANNEL_META[r.channel].label : "League ask",
  ...
  const subject: RequestSubject | null = openRow
    ? openRow.kind === "request" && openRow.request
      ...
      : openRow.interest
        ? { kind: "league", interest: openRow.interest, task: openRow.task }
        : null
    : null;
            ...
            <ContextSelect
              ...
              options={[
                /*
                  ...
                  purpose: a closed enquiry is still a row on this table
        ...
        */}
        <header className={styles.head}>
          ...
          <p className={styles.subLede}>
            ...
            >
              <span className="num">{SEED_REQUESTS.length}</span> enquiries,{" "}
              <span className="num">{SEED_LEAGUE_INTEREST.length}</span> league
              asks, read at <span className="num">{formatWhen(NOW)}</span> venue
              time
            </span>
            <span
              className={styles.provenanceInline}
              title="Enquiries written for this work sample. Each row carries the exact field set of the published route it arrived through. Contact addresses sit on the .invalid domain, which RFC 2606 reserves so it can never resolve."
            >
              <ProvenanceBadge provenance="illustrative" />
              <span>Written for this work sample, published field sets</span>
            </span>
          </p>
        </header>
        ...
        <SegmentedFilter
          ...
          segments={READING_ORDER.map((id) => ({
            ...
            /*
              ...
              enquiry is still a row here and it has no clock, so it
        ...
        <WorkingSetLead
          ...
          facts={readingFacts.map((f, i) => ({
            label: f.label,
            value: f.value,
            /* The clock figures are derived from enquiries written for
        ...
        {total === 0 ? (
          /* THE INBOUND QUEUE, WORKED TO NOTHING. Every enquiry with a
        ...
        <section className={styles.queue} aria-labelledby="queue-h" ref={tableRef}>
          ...
          <div className={styles.filters}>
            <label className={styles.filter}>
              ...
              <input
                ...
                type="search"
                value={query}
                placeholder="Organisation, role, what they asked"
          ...
          <p className={styles.counts} aria-live="polite">
            ...
            {excludedLeague > 0 ? (
              <>
                <span className="num">{excludedLeague}</span> league asks out
                of view.{" "}
              </>
            ) : null}
            {selected.length > 0 ? (
                      ...
                      selectedRows
                        .map((r) =>
                          r.request
                            ...
                            : `${r.organisation}\n${r.role}\nLeague ask: ${r.interest?.preferredNights.join(" or ")}`,
                        )
                        .join("\n\n"),
            ...
            <div className={styles.tableWrap}>
              <table className={styles.table}>
                <caption className="visually-hidden">
                  Inbound requests and league asks, {filtered.length} of{" "}
          ...
          <div className={styles.commitmentGrid}>
            <p className={styles.commitmentFigure}>
              ...
              <span className={styles.commitmentUnit}>
                working hours, 9am to 6pm at the venue, weekends included
              </span>
            </p>
          ...
          <div className={styles.recordRow}>
            ...
            <div className={styles.recordFigure}>
              ...
              <span
                ...
                title="Never answered, and counted in neither figure to the left."
              >
                never answered
                <ProvenanceBadge provenance="illustrative" compact />
              </span>
        ...
        <section className={styles.gap} aria-labelledby="gap-h">
          <div className={styles.sectionHead}>
            <h2 className={styles.h2} id="gap-h">
              What the form did not ask for
            </h2>
            {/* Published field sets, read off the routes themselves. This
                is the finding the panel exists to count, so it keeps its
                badge and its shortest true form. */}
            <p
              className={styles.sectionNote}
              title="One published route asks a first name, a last name, an email, a phone number, a company and a block of free text. It asks for no date, no headcount and no event type. The brand-wide events form asks for all three. Every field set is recorded on the route it belongs to."
            >
              The thinner route asks for no date, no headcount and no event
              type. The brand-wide form asks for all three.
              ...
              return (
                <li key={row.field} className={styles.gapRow}>
                  ...
                  <span
                    ...
                    role="img"
                    aria-label={`${row.missing} of ${gap.open} open requests are missing the ${row.label.toLowerCase()}`}
                  >
                    <span
          ...
          <p className={styles.gapHeadline} aria-live="polite">
            <strong>
              ...
              <span className="num">{gap.open}</span> open requests can be
              quoted without a phone call first.
            </strong>{" "}
            {gap.headline}
          </p>
        </section>
    ...
    >
      ...
      <td data-label="Organisation">
        {/*
          ...
          A walk-in with no row in the trade area file has no record to
          open, so its name stays as words.
        */}
        ...
        {row.prospectId ? null : (
          <span className={styles.walkIn}>
            <span aria-hidden="true">○</span> Walk-in, no prospect record
          </span>
        )}
      </td>
        ...
        {row.request ? (
          missing.length > 0 ? (
            <span className={styles.missing}>
              ...
              <span className="num">3</span> qualifying fields missing
              {unasked.length > 0 ? (
                <>
        ...
        {status ? (
          ...
          <TokenChip
            token={{
              ...
              note: "A league ask is not a pipeline row.",
            }}
            size="sm"
          />
        )}
        ...
        {row.task ? (
          ...
          <span className={styles.actionText}>
            {row.request?.status === "lost"
              ? "Nothing to send. The reason is on the record."
