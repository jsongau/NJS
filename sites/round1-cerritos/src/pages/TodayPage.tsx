 ...
 * halves of the job.
 *
 * Every other page in this application is a place you go when you already
 * know what you are looking for. The desk is for choosing who to write
 * to, the queue is for working through what arrived, the book is for
 * checking what is signed. Each of them is honest and each of them is a
 * decision the reader has to make before the screen can help. This one
 * makes the first decision for them and then shows its working.
 *
 * ----- THE LESSON THIS PAGE INHERITS ---------------------------------
 *
 * The owner's other prototype carries a function whose own comment calls
 * it "the next thing you're on the hook for", and it is called in exactly
 * one place: the empty state. The best idea in that codebase is visible
 * only when there is nothing to do. That is not a small mistake, it is
 * the whole failure of dashboards in one line: the most useful thing gets
 * built, and then it gets filed behind a chart of totals.
 *
 * So here the next thing is not a row in a table and it is not a
 * fallback. It is the first thing on the page, at the size of a headline,
 * with its clock, its organisation, its reason and its action. Everything
 * below it exists to say what else is waiting and what is quietly rotting
 * while the top item gets done.
 *
 * ----- THE DISCIPLINE THIS PAGE IS MOST LIKELY TO BREAK ---------------
 *
 * This application has two ledgers that are never added together: signed
 * contracts, which carry money, and hours outside the building, which
 * carry none. A "today" screen is exactly where somebody adds them,
 * because a single number is easier to put in a heading than two.
 *
 ...
 * because nobody is waiting. "Nine things today" made from four enquiries
 * and five go-sees is a number that means nothing: the four have somebody
 * on the other end of them and the five do not. So the two halves sit
 * side by side, each with its own unit named in its own heading, and
 * there is no total of the two anywhere on this page.
 *
 * ----- WHAT THIS PAGE REFUSES TO SAY ---------------------------------
 *
 * The four working hour response commitment is THIS DESK'S OWN and it
 * renders with that stated every time it appears. Round1 publishes no
 * response time anywhere: not on the party booking page, not on the party
 * room page, not on the nearest store's page. Round1 publishes no league
 * programme at any location either, so a league ask on this page is
 * answered rather than sold.
 *
 ...
 * sample read on somebody else's computer in another state, six months
 * after it was written, must show the same overdue count it showed the
 * day it was built. A "today" page is the one screen where that is most
 * tempting to get wrong and most obvious when it is wrong.
 */
 ...
 * A held date shown one day early is not a rounding error on a screen
 * somebody is working a phone call from.
 */
 ...
 *
 * Every timestamp in the request seeds and everything the queue derives
 * from them carries the venue's own offset, so the hour in the string IS
 * the venue-local hour. Reading it off the string keeps the page free of
 * a second timezone conversion that could disagree with the first.
 */
...
/** The Monday on or before a calendar date. Activity is planned by week. */
...
/** The role and the route an inbound task came in through, where there is one. */
function contextOf(task: DerivedTask): { role: string; via: string; ask: string } | null {
  ...
  if (league) {
    return {
      ...
      via: "League ask",
      ask: `Asked about a league on ${league.preferredNights.join(" or ")}. Round1 publishes no league at any location, so there is nothing to enrol them in and the ask is recorded as midweek demand.`,
    };
  }
  return null;
}
...
export function TodayPage() {
  ...
  /* Collapsed by default. The argument for the tool is not the tool, and a
     person opening this at nine in the morning wants the queue. */
   ...
   *
   * The reason for it is the one in the job posting: the person running
   * this desk is out of the building for half the week, and a queue that
   * can only be read on a screen behind a rail cannot be worked from a
   * car park. The rank is exported with it, because a list handed over
   * without its order is a list somebody else will reorder by name.
   */
  ...
  const risks = useMemo<RiskGroup[]>(() => {
    ...
    /* ONE. Never answered, and the commitment has already passed. The
       only failure on this page that belongs entirely to the venue: no
       group said no, nobody chose anything, the clock simply ran. */
    ...
    const unansweredItems: RiskItem[] = [
      ...
      ...unansweredLeague.map((l) => ({
        ...
        lane: l.lane,
        detail: `${l.contactRole}. Asked about a league on ${l.preferredNights.join(
          " or ",
        )}, which Round1 has not announced anywhere. The honest answer is still an answer.`,
    ...
    /* THREE. A date held against no deposit is a date nobody else can be
       offered, so a hold that is not converting is costing the next
       caller rather than the person holding it. Both halves of the app
       can create one, so both halves are counted here. */
    ...
    const heldRequests: RiskItem[] = SEED_REQUESTS.filter(
      ...
      return {
        ...
        clock: r.desiredDate
          ? `${formatDay(r.desiredDate)} held, nothing signed`
          : "Date held, nothing signed",
        to: "/requests",
    ...
    const heldProspects: RiskItem[] = PROSPECTS.filter(
      ...
      name: p.name,
      lane: p.lane,
      detail: `${p.decisionMakerTitle}. Date held on the outbound board with no line in the book against it.`,
      clock: "Held, nothing signed",
      to: `/map?prospect=${p.id}`,
      toLabel: "Open the organisation",
    }));

    /* FOUR. The window is open now and nobody has ever written to them.
       A calendar-locked buyer whose window closes unanswered is not a
       slower deal, it is a year gone: a graduating class graduates once. */
    const untouched: RiskItem[] = PROSPECTS.filter(
      ...
      detail: `${p.decisionMakerTitle}. ${
        LANE_META[p.lane].occasionClass === "calendar-locked"
          ? "Calendar-locked, so the occasion happens whether or not anybody calls."
          : "Discretionary, so somebody there has to decide there will be an event at all."
      }`,
      clock: `Buys in ${p.buyingWindow}`,
      to: `/map?prospect=${p.id}`,
      toLabel: "Open the organisation",
    ...
    /* FIVE. The two ledgers disagreeing about whether something was
       ...
       that exists on only one of the two is not a win anybody can
       report. */
    ...
    return [
      {
        ...
        denominatorLabel: "enquiries and league asks on the board",
        full: `${unansweredItems.length} ${plural(
          unansweredItems.length,
          "enquiry has",
          "enquiries have",
        )} sat past the four working hour commitment with nobody having replied. Nobody has said no to any of them. This is the only failure on the page that belongs entirely to the venue, which is why it is first.`,
        empty:
          "Every enquiry that has arrived has had a human reply inside the four working hour commitment. That is the state to hold rather than a state to celebrate, because it lasts exactly until the next form is submitted.",
        route: "/requests",
      ...
      },
      {
        ...
        denominatorLabel: "organisations in the trade area",
      ...
      {
        id: "held",
        label: "Dates held against nothing",
        glyph: "◕",
        cssVar: "var(--accent)",
        denominator: openRequests + liveProspects.length,
        denominatorLabel: "open enquiries and live conversations",
        full: `${
          ...
          "date has",
          "dates have",
        )} no signature and no deposit against them. Round1 publishes no deposit and no minimum booking notice, so nothing external releases a stale hold and a date held against nothing is a date nobody else can be offered.`,
        empty:
          "No date is being held against nothing. Every hold on the board has either converted or been released, which is the only two things a hold is allowed to do.",
        route: "/calendar",
        routeLabel: "Dates and capacity",
      ...
      },
      {
        ...
        full: `${untouched.length} ${plural(
          ...
          "has",
          "have",
        )} never been contacted. Nothing is late here yet, which is exactly why it goes unnoticed until the window shuts.`,
        empty:
      ...
      },
      {
        ...
        denominatorLabel: "enquiries on the board",
        full: `${disagreements.length} ${plural(
          disagreements.length,
          "enquiry is",
          "enquiries are",
        )} marked won with no matching line in the book. One of the two records is wrong, and a win that exists on only one of them is not a win anybody can report.`,
        empty:
          ...
          <div className={styles.built}>
            ...
            {builtOpen ? (
              <div id="built-body" className={styles.builtBody}>
                <p>
                  A territory nobody has worked has no list, no history and
                  no inbound habit, and the only way to find the schools,
                  offices, printers and shops worth a conversation is to go out
                  and look. So I built the list first: every real organisation
                  within reach of the Cerritos office, what each one publishes,
                  and when to go back.
                </p>
                <p>
                  It runs one loop. Rank what is closest to being lost, work it,
                  write down what came back, and set the next date. Every figure
                  on it carries where it came from, because the ones I could
                  not source honestly are the whole reason a sales manager is
                  the person who prices this and not a web page.
                </p>
              </div>
            ) : null}
          </div>
          ...
          <div className={styles.halves}>
            <section className={styles.inbound} aria-labelledby="inbound-h">
              ...
              <p className={styles.halfFoot}>
                <Link className="tap" to="/requests">
                  The inbound queue, all {count(SEED_REQUESTS.length)} enquiries
                </Link>
              </p>
            </section>
              ...
              {thisWeek === null ? (
                ...
                <>
                  <p
                    ...
                    title="A call block from a desk counts as outbound work but not as outside hours."
                  >
                    Week commencing{" "}
                    <span className="num">{formatDay(WEEK_START)}</span>:{" "}
                    {hoursText(outsideThisWeek)} of {hoursText(hoursThisWeek)}{" "}
                    planned hours outside the building.
                  </p>
        ...
        <section className={styles.weekSection} aria-labelledby="week-h">
          ...
          <div className={styles.weekGrid}>
            <div className={styles.weekCard}>
              ...
              <p className={styles.weekText}>
                {week.won.length + week.lost.length === 0
                  ...
                  : `${week.won.length} signed, ${week.lost.length} lost${
                      week.lost.length === 0
                        ? ""
              ...
              <p className={styles.weekText}>
                ...
                <Figure
                  ...
                  compact
                />{" "}
                rests on a price somebody typed rather than one Round1
                publishes.
              </p>
            </div>
            ...
            <div className={styles.weekCard}>
              ...
              <p className={styles.weekUnit}>
                enquiries and league asks
                <ProvenanceBadge provenance="illustrative" compact />
              </p>
              {/* Round1 publishing no league anywhere is the reason a league
                  ask is logged rather than sold, so it keeps a home in the
                  tooltip and in the accessibility tree. */}
              <p
                className={styles.weekText}
                title={
                  week.leagueArrived.length === 0
                    ? undefined
                    : "Round1 publishes no league programme at any location, so a league ask is recorded as midweek demand rather than sold."
                }
              >
                {week.arrived.length + week.leagueArrived.length === 0
                  ...
                  : `${week.arrived.length} ${plural(
                      ...
                      "enquiry",
                      "enquiries",
                    )}, ${week.leagueArrived.length} league ${plural(
                ...
                {week.leagueArrived.length === 0 ? null : (
                  <span className="visually-hidden">
                    {" "}
                    Round1 publishes no league programme at any location, so
                    a league ask is recorded as midweek demand rather than
                    sold.
                  </span>
                )}
              </p>
            ...
            <div className={styles.weekCard}>
              ...
              <p className={styles.weekUnit}>
                of {count(SEED_REQUESTS.length)} enquiries
                <ProvenanceBadge provenance="illustrative" compact />
              </p>
              {/* The thinner of the two published routes asks for no date,
                  no headcount and no event type. That published fact is why
                  this count exists, so it stays as the tooltip on the count
                  itself. */}
              <p
                className={styles.weekText}
                title="The thinner published enquiry route asks for a name, an email, a phone number, a company and a block of free text. It asks for no date, no headcount and no event type."
              >
                {openRequests.length === 0
                  ? "All closed."
                  : `${gap.qualified} of ${gap.open} carry all three qualifying fields.`}
                {(() => {
                  ...
                  return unqualified === 0 ? null : (
                    <>
                      ...
                      <span className="visually-hidden">
                        {" "}
                        The thinner published enquiry route asks for a name,
                        an email, a phone number, a company and a block of free
                        text. It asks for no date, no headcount and no event
                        type.
                      </span>
                    </>
                  );
            ...
            <div className={styles.weekCard}>
              ...
              <p className={styles.weekText}>
                {plan.outsideHours === 0
                  ? "No outside hours planned this period."
          ...
          {/* -----------------------------------------------------
              ...
              measures the desk rather than the trade area, and the
          ...
          <section className={styles.record} aria-labelledby="record-h">
            ...
            {/* The disclosure is the honest label on the whole panel: no
                response time is published anywhere. It never leaves. */}
