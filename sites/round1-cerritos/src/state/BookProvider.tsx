 ...
 *
 * Early in a territory, activity is most of what there is to report,
 * which is exactly when the temptation to report it AS progress is
 * strongest. Every thin status update in the world says "we made 400
 * calls" because 400 is a bigger number than 3. So the two live in
 ...
 * shows it as one: hours out of the building per thousand dollars
 * booked. That number gets better as the pipeline matures and it is the
...
function reducer(state: BookState, action: BookAction): BookState {
  switch (action.type) {
    case "ADD_BOOKING": {
       ...
       *
       * A rep who re-sends a corrected quote has not sold the party
       * twice. Getting this wrong is how a pipeline report ends up
       * double counting the only three bookings it has, which is a very
       * visible way to be wrong in front of a GM.
       */
        ...
        book: state.book.map((l) =>
          l.id === action.id
            ? {
                ...
                /* Lanes held are derived from headcount, never typed.
                   A booking whose guest count changes and whose lane
                   count does not is a capacity chart that lies. */
 ...
 *
 * A guest count typed on a seeded line is a different case, and it is the
...
export interface ActivityTotals {
  ...
  /** Hours committed outside the building, specifically. */
  outsideHours: number;
}
 ...
 *
 * The job posting's first daily responsibility is outbound activity
 * "outside the building". A call block from a desk is outbound work and
 * it is counted, but it is not the thing the posting asked for, and a
 * plan that quietly meets its hours target from a chair has not met it.
 */
...
/**
 * Hours out of the building per thousand dollars booked.
 *
 * The one number that makes activity legible without letting it pretend
 * to be revenue. It starts terrible, as every new territory does,
 * because the first bookings cost the most work, and it improves as
 * referral partners start sending people. Returns null rather than
